...
activate_venv() {
    ...
    python -m pip install "connectors-sdk @ git+https://github.com/OpenCTI-Platform/connectors.git@${RELEASE_REF:-master}#subdirectory=connectors-sdk"
...
do
  if [ -d "$connector_directory_path" ]; then
    ...
    if [ "$CIRCLE_BRANCH" = "${RELEASE_REF:-release/6.9.x}" ]; then
      directory_has_changed=$(git diff HEAD~1 HEAD -- "$connector_directory_path")
    else
      directory_has_changed=$(git diff $(git merge-base origin/"${RELEASE_REF:-release/6.9.x}" HEAD) HEAD "$connector_directory_path")
    fi
