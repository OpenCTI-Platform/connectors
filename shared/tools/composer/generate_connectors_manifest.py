...
class ManifestGenerator:
    ...
    def get_connectors_contracts(self):
        ...
        for __metadata___files_paths in self.get_connector___metadata___files_path():
            connector_manifest_file_path, connector_config_json_schema_file_path = (
                __metadata___files_paths
            )
            ...
            if connector_config_json_schema_file_path:
                with open(
                    connector_config_json_schema_file_path, encoding="utf-8"
                ) as file:
            ...
            if connector_config_json_schema and is_manager_supported:
                ...
                for config_var in __EXCLUDED_CONFIG_VARS__:
                    ...
                    defaults = connector_contract["config_schema"].get("default", {})
                    defaults.pop(config_var, None)

                    properties = connector_contract["config_schema"].get(
                        "properties", {}
                    )
                    properties.pop(config_var, None)

                    required_properties = connector_contract["config_schema"].get(
                        "required", {}
                    )
