...
URLs) and malware-family attributions, MITRE ATT&CK techniques
observed at runtime, and the AssemblyLine submission link as an
external reference on the enriched observable.
...
- Optionally: STIX `Indicator` objects for every malicious IOC
  AssemblyLine extracted (domains, IPs, URLs) and `Malware` SDOs
  for every malware family attributed to the sample.
- Optionally: matching `Observable` objects linked to the
  indicators via `based-on` relationships
  (`ASSEMBLYLINE_CREATE_OBSERVABLES=true`).
- Optionally: `Attack-Pattern` objects for every MITRE ATT&CK
  technique observed at runtime, linked to the generated
  indicators with `related-to` relationships.
...
| Submission Classification      | `classification`            | `ASSEMBLYLINE_CLASSIFICATION`           | `TLP:C`                  | No        | AssemblyLine classification marker attached to every submission.                                                                                                         |
...
| Sequential Mode                | `sequential_mode`           | `ASSEMBLYLINE_SEQUENTIAL_MODE`          | `true`                   | No        | When `true`, the connector waits for AssemblyLine to be idle (no in-flight submission) before sending a new one — this prevents overloading the platform. The wait is capped by `ASSEMBLYLINE_TIMEOUT`.|
...
| Max TLP                        | `max_tlp`                   | `ASSEMBLYLINE_MAX_TLP`                  | `TLP:AMBER`              | No        | Observables marked above this TLP are skipped — the connector will not submit their file content to the external AssemblyLine deployment.                                |
