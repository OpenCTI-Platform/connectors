...
class URLIPv6Enricher(IPv6Enricher):
    """
    ...
    :param ipv6: the IPv6 to be enriched
    """

    def __init__(self, helper, stix_entity, ipv6):
        super().__init__(helper, stix_entity)
        self._stix_entity["value"] = ipv6
        self._helper.log_debug(f"self._stix_entity: {self._stix_entity}")

    def _build_ip(self):
        """
        Adds IPv6 enriched data to the stix bundle with a relationship to the URL
        """
