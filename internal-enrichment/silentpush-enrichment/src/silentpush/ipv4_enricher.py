...
class IPv4Enricher(IPEnricher):
    ...
    def _build_ip(self):
        """
        Adds IPv4 enriched data to the stix bundle
        """
    ...
    def _do_request(self):
        """
        Calls Silent Push API to enrich IPv4
        """
        ...
        enrich_uri = enrich_uri.format(type="ipv4", ioc=self._sanitized_ip)
        self._helper.log_debug(f"enrich_uri: {enrich_uri}")
        return requests.get(
