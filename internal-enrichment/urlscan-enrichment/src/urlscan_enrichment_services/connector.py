...
class UrlscanConnector:
    ...
    def _create_custom_label(self, name_label: str, color_label: str):
        ...
        if new_custom_label is None:
            ...
            self.helper.connector_logger.info(
                "[INFO] The label has been created.", {"name_label": name_label}
            )
    ...
    def _generate_stix_bundle(
        ...
        self.helper.connector_logger.info(
            "[CONNECTOR] The entity has been identified by URLScan and generation of the Stix bundle is in progress.",
            {"entity_type": stix_entity["type"], "entity_value": stix_entity["value"]},
        )
        if is_submission:
            prepared_file_png = (
                self.utils.prepare_file_png(data)
                if self.config.urlscan_enrichment.import_screenshot
                else None
            )
            ...
            if len(reorganized_data) > 0:
                for data_stat in reorganized_data:
                    ...
                    if data_stat["domains"][0] in stix_entity["value"]:
                        if self.config.urlscan_enrichment.create_indicator:
                            stix_indicator = (
        ...
        self.helper.connector_logger.info(
            ...
            {"Entity": stix_entity["value"], "Stix_bundle_length": len(reordered_data)},
        )
    ...
    def _process_message(self, data: Dict) -> str:
        ...
        if entity_type in scopes:
            ...
            if not is_valid_max_tlp:
                raise ValueError(
                    "[ERROR] Do not send any data, TLP of the observable is greater than MAX TLP, the connector does not has access to this observable, please check the group of the connector user"
                )
            ...
            try:
                ...
                if (
                    ...
                    self.client.check_urlscan_user_quota(
                        self.config.urlscan_enrichment.visibility
                    )
                    ...
                    raise ValueError(
                        "[ERROR] This entity type is currently not managed, available_type: Url, Hostname, Domain-Name, IPv4-Addr, IPv6-Addr"
                    )
            except Exception as e:
                raise ValueError(str(e))
        else:
            return self.helper.connector_logger.info(
                "[INFO] The trigger does not concern the initial scope found in the config connector, maybe choose a more specific filter in the playbook",
