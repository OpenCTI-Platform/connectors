...
class TotoConfig(BaseConfigModel):
    ...
    mamama: SkipValidation[int] = legacy_field(new_variable_name="momomo")
    momomo: str = Field(description="momomo")
    interval: SkipValidation[int] = legacy_field(
...
class ConnectorSettings(BaseConnectorSettings):
    ...
    tata: SkipValidation[TotoConfig] = legacy_field(
        deprecated="Use toto",
        new_namespace="toto",
    )
    dummy_enrichment: SkipValidation[DummyConfig] = legacy_field(
    ...
    christmas_old: SkipValidation[ChristmasConfig] = legacy_field(
