...
def fixture_config(monkeypatch: pytest.MonkeyPatch) -> ConnectorSettings:
...
def test_connector(
    config: ConnectorSettings, helper: OpenCTIConnectorHelper, data: dict[str, Any]
) -> None:
    connector = ShodanInternetDBConnector(config=config, helper=helper)
