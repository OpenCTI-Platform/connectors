...
def test_process_message_unsupported_type(mock_dependencies):
    ...
    helper.api.work.to_processed.assert_called_once_with(
        "mock-work-id", "Unsupported observable type"
    )
...
def test_process_message_domain_enrichment_no_data(mock_dependencies):
    ...
    helper.api.work.to_processed.assert_called_once_with(
        "mock-work-id",
        "No threat intelligence data found on WhoisFreaks for example.com.",
    )
...
def test_process_message_exception(mock_dependencies):
    ...
    helper.api.work.to_processed.assert_called_once_with(
        "mock-work-id",
        "Error during processing of example.com: API failure",
        in_error=True,
    )
