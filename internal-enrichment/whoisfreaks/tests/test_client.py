...
def test_client_post_success(client):
    with requests_mock.Mocker() as m:
        m.post(
            "https://api.whoisfreaks.com/v2.0/bulkwhois/live?apiKey=test-api-key&format=json",
            json={"bulk": "success"},
        )
...
def test_historical_whois_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v2.0/whois?apiKey=test-api-key&format=json&domainName=example.com&whois=historical",
            json={"data": "whois"},
        )
...
def test_reverse_whois_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v1.0/whois?apiKey=test-api-key&format=json&keyword=example&whois=reverse",
            json={"data": "rev"},
        )
...
def test_live_dns_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v2.0/dns/live?apiKey=test-api-key&format=json&domainName=example.com&type=all",
            json={"data": "dns"},
        )
...
def test_historical_dns_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v2.0/dns/historical?apiKey=test-api-key&format=json&domainName=example.com&type=all&page=2",
            json={"data": "hist"},
        )
...
def test_reverse_dns_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v2.0/dns/reverse?apiKey=test-api-key&format=json&value=1.2.3.4&page=1&type=a&exact=True",
            json={"data": "rev"},
        )
...
def test_bulk_dns_lookup(client):
    with requests_mock.Mocker() as m:
        ...
        assert client.bulk_dns_lookup(
            domains=["example.com"], ip_addresses=["1.2.3.4"]
        ) == {"data": "bulk"}
...
def test_domain_availability_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v1.0/domain/availability?apiKey=test-api-key&format=json&domainName=example.com&sug=False",
            json={"data": "avail"},
        )
...
def test_bulk_domain_availability_lookup(client):
    with requests_mock.Mocker() as m:
        m.post(
            "https://api.whoisfreaks.com/v1.0/domain/availability?apiKey=test-api-key&format=json",
            json={"data": "bulk_avail"},
        )
        assert client.bulk_domain_availability_lookup(["example.com"]) == {
            "data": "bulk_avail"
        }
...
def test_ssl_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v1.0/ssl/live?apiKey=test-api-key&format=json&domainName=example.com&chain=True&sslRaw=False",
            json={"data": "ssl"},
        )
...
def test_ip_geolocation_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v1.0/geolocation?apiKey=test-api-key&format=json&ip=1.2.3.4",
            json={"data": "geo"},
        )
...
def test_bulk_ip_geolocation_lookup(client):
    with requests_mock.Mocker() as m:
        m.post(
            "https://api.whoisfreaks.com/v1.0/geolocation?apiKey=test-api-key",
            json={"data": "bulk_geo"},
        )
...
def test_subdomains_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v1.0/subdomains?apiKey=test-api-key&format=json&domain=example.com&page=2",
            json={"data": "sub"},
        )
...
def test_ip_reputation_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v1.0/security?apiKey=test-api-key&ip=1.2.3.4",
            json={"data": "rep"},
        )
...
def test_bulk_ip_reputation_lookup(client):
    with requests_mock.Mocker() as m:
        m.post(
            "https://api.whoisfreaks.com/v1.0/security?apiKey=test-api-key",
            json={"data": "bulk_rep"},
        )
...
def test_asn_whois_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v2.0/asn-whois?apiKey=test-api-key&asn=AS15169&format=json",
            json={"data": "asn"},
        )
...
def test_ip_whois_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v1.0/ip-whois?apiKey=test-api-key&ip=1.2.3.4&format=json",
            json={"data": "ip_whois"},
        )
...
def test_domain_reputation_lookup(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v1.0/domain-reputation?apiKey=test-api-key&domainName=example.com&format=json",
            json={"data": "dom_rep"},
        )
...
def test_account_usage(client):
    with requests_mock.Mocker() as m:
        m.get(
            "https://api.whoisfreaks.com/v1.0/whoisapi/usage?apiKey=test-api-key",
            json={"data": "usage"},
        )
...
def test_rotate_api_key(client):
    with requests_mock.Mocker() as m:
        m.post(
            "https://api.whoisfreaks.com/v1.0/api-key/rotate?apiKey=test-api-key",
            json={"data": "rotate"},
        )
