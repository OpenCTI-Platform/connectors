...
class WhoisFreaksStixBuilder:
    ...
    def build_whois_bundle(
        ...
        if registrar_name and str(registrar_name).strip():
            registrar_stix = stix2.Identity(
                id=pycti.Identity.generate_id(
                    name=str(registrar_name).strip(), identity_class="organization"
                ),
    ...
    def build_dns_bundle(
        ...
        if self.is_ip_address(clean_value):
            source_stix = (
                stix2.IPv6Address(value=clean_value)
                if ":" in clean_value
        ...
        for record in dns_records:
            ...
            if record_type == "A":
                ...
                target_stix = stix2.IPv6Address(value=record_value.strip())
                rel_type = "resolves-to"
    ...
    def build_ssl_bundle(
        ...
        if self.is_ip_address(clean_target):
            target_stix = (
                stix2.IPv6Address(value=clean_target)
                if ":" in clean_target
                else stix2.IPv4Address(value=clean_target)
            )
        else:
