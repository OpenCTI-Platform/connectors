...
class ConnectorConfig(ConfigBaseModel):
    ...
    scope: ListFromString = Field(
        description="The scope of the connector, e.g. 'abuseipdb'.",
        default=["abuseipdb"],
    )
    log_level: Literal[
        "debug",
...
class AbuseIPDBConfig(ConfigBaseModel):
    api_key: str = Field(
        description="The API key to connect to AbuseIP DB.",
    )
    max_tlp: Literal[
        "TLP:CLEAR",
        "TLP:WHITE",
        ...
        description="The maximum TLP of OCTI entities to enrich.",
        default="TLP:RED",
    )
...
class ConfigLoader(BaseSettings):
    ...
    Define a complete config for a connector with:
        ...
        - abuseipdb: the config specific to AbuseIP DB
    """
