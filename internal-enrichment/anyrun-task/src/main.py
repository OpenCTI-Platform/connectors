...
from anyrun_task import AnyRunTask, ConnectorSettings
...
if __name__ == "__main__":
    ...
    try:
        ...
        helper = OpenCTIConnectorHelper(
            config=settings.to_helper_config(), playbook_compatible=False
        )

        connector = AnyRunTask(config=settings, helper=helper)
        connector.run()
    except Exception:
        traceback.print_exc()
        exit(1)
