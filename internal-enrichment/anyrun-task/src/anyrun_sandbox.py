...
class AnyRunSandbox:
    ...
    def load_analysis_object(self, opencti_entity: dict) -> None:
        ...
        if opencti_entity["entity_type"] == "StixFile":
            ...
            self._config = Config.update_config(opencti_entity["value"], "Url")
            self._analysis_type = "Url"
        else:
            raise ValueError("Wrong scope! Supported only StixFile and Url observables")
    ...
    def process_analysis(self) -> dict:
        ...
        if self._config.env_os == "windows":
            ...
            raise ValueError(
                "ANY.RUN env_os option value is invalid! Please specify one of the following environments: windows, linux, android"
            )
    ...
    def _get_file_content(self, file: dict) -> tuple[bytes, str]:
        ...
        try:
            ...
            raise ValueError("Error fetching attachment from the observable") from err
    ...
    def _get_submission(
        self, connector: WindowsConnector | LinuxConnector | AndroidConnector
    ) -> dict:
        """
        Process analysis
        ...
        return connector.get_analysis_report(task_uuid, report_format="json")
