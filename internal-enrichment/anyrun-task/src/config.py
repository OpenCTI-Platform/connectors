...
# Mapping from internal parameter name (used as a kwarg of the ANY.RUN SDK)
# to the corresponding environment variable name and YAML path. Keeping the
# table explicit avoids surprises caused by previous `f"ANYRUN_{name.upper()}"`
# generation that did not match the variable names documented in the
# docker-compose / __metadata__ files.
_BASE_PARAMS: dict[str, tuple[str, list[str]]] = {
    "env_locale": ("ANYRUN_ENV_LOCALE", ["anyrun_env", "env_locale"]),
    "opt_network_connect": (
    ...
    "opt_network_fakenet": (
        ...
        ["anyrun_opt", "opt_network_fakenet"],
    ),
    "opt_network_tor": ("ANYRUN_TOR", ["anyrun_opt", "opt_network_tor"]),
    "opt_network_geo": ("ANYRUN_GEO", ["anyrun_opt", "opt_network_geo"]),
    "opt_network_mitm": ("ANYRUN_MITM", ["anyrun_opt", "opt_network_mitm"]),
    "opt_network_residential_proxy": (
        "ANYRUN_RESIDENTIAL_PROXY",
        ["anyrun_opt", "opt_network_residential_proxy"],
    ),
    "opt_network_residential_proxy_geo": (
        "ANYRUN_RESIDENTIAL_PROXY_GEO",
        ["anyrun_opt", "opt_network_residential_proxy_geo"],
    ),
    "opt_privacy_type": (
        "ANYRUN_PRIVACY_TYPE",
        ["anyrun_opt", "opt_privacy_type"],
    ),
    "obj_ext_extension": (
        "ANYRUN_OBJ_EXT_EXTENSION",
        ["anyrun_obj", "obj_ext_extension"],
    ),
}
...
class Config:
    ...
    def _create_base_config(self) -> dict[str, bool | int | None | str]:
        ...
        """
        for param_name, (env_var, yaml_path) in _BASE_PARAMS.items():
    ...
    @classmethod
    def update_config(
        cls, submission_obj: str | tuple[bytes, str], entity_type: str
    ) -> Self:
        """
        ...
        os_type = get_config_variable(
            "ANYRUN_OS_TYPE", ["anyrun_env", "os_type"], config
        )
        ...
        raise ValueError(
            f"Unspecified submission os: {os_type}"
            if os_type
            else "The os_type option must be specified"
        )

    @staticmethod
    def _create_windows_config(
        submission_obj: str | tuple[bytes, str], entity_type: str
        ...
        """
        params: dict[str, bool | int | None | str] = {"env_os": "windows"}

        params["env_version"] = get_config_variable(
            "ANYRUN_ENV_VERSION", ["anyrun_windows_env", "env_version"], config
        )
        ...
        params["env_type"] = get_config_variable(
            "ANYRUN_ENV_TYPE", ["anyrun_windows_env", "env_type"], config
        )
        ...
        if entity_type == "File":
            ...
            params["obj_ext_cmd"] = get_config_variable(
                "ANYRUN_OBJ_EXT_CMD", ["anyrun_windows_env", "obj_ext_cmd"], config
            )
    ...
    def _create_linux_config(
        ...
        """
        params: dict[str, bool | int | None | str] = {"env_os": "linux"}
        ...
        if entity_type == "File":
            ...
            params["obj_ext_cmd"] = get_config_variable(
                "ANYRUN_OBJ_EXT_CMD", ["anyrun_linux_env", "obj_ext_cmd"], config
            )

            params["run_as_root"] = get_config_variable(
                "ANYRUN_RUN_AS_ROOT", ["anyrun_linux_env", "run_as_root"], config
            )
    ...
    def _create_android_config(
        ...
        """
        params: dict[str, bool | int | None | str] = {"env_os": "android"}
        ...
        if entity_type == "File":
            ...
            params["obj_ext_cmd"] = get_config_variable(
                "ANYRUN_OBJ_EXT_CMD", ["anyrun_android_env", "obj_ext_cmd"], config
            )
    ...
    @staticmethod
    def _process_url(
        params: dict[str, bool | int | None | str], submission_obj: str
    ) -> dict[str, bool | int | None | str]:
        """
        ...
        params["obj_ext_browser"] = get_config_variable(
            "ANYRUN_OBJ_EXT_BROWSER", ["anyrun_env", "obj_ext_browser"], config
        )
