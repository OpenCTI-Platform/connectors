...
class AnyRunTask:
    def __init__(self):
        ...
        self.anyrun_url = get_config_variable("ANYRUN_API_URL", ["anyrun", "url"], config, default='https://api.any.run')
        ...
        self.task_timer = int(get_config_variable("ANYRUN_TASK_TIMER", ["anyrun", "timer"], config, default=60))
        self.task_os = get_config_variable("ANYRUN_OS", ["anyrun", "os"], config, default='windows')
        self.task_os_bitness = get_config_variable("ANYRUN_OS_BITNESS", ["anyrun", "bitness"], config, default='64')
        self.task_os_version = get_config_variable("ANYRUN_OS_VERSION", ["anyrun", "version"], config, default='10')
        self.task_os_browser = get_config_variable("ANYRUN_OS_BROWSER", ["anyrun", "browser"], config, default='Google Chrome')
        self.automated_interactivity = get_config_variable("ANYRUN_AUTOMATED_INTERACTIVITY", ["anyrun", "automated_interactivity"], config, default=False)
        self.enable_ioc = get_config_variable("ANYRUN_IOC", ["anyrun", "ioc"], config, default=True)
        self.enable_mitre = get_config_variable("ANYRUN_MITRE", ["anyrun", "mitre"], config, default=False)
        self.enable_processes = get_config_variable("ANYRUN_PROCESSES", ["anyrun", "processes"], config, default=False)
        self.iocs_types_mapping = {'domain': 'Domain-Name',
                                   'url': 'Url',
                                   'ip': 'IPv4-Addr'
                                   }
    ...
    def _process_message(self, data):
        ...
        if opencti_entity['entity_type'] == 'Artifact':
            ...
            try:
                ...
                raise ValueError(
                    "Error fetching artifact from OpenCTI"
                ) from err
            ...
            raise ValueError('Wrong scope! supported only "Artifact" and "Url" observables types')
        ...
        if task.get('error', False):
            ...
            self.helper.log_info(
                'ANY.RUN task started url: https://app.any.run/tasks/{}'.format(task['data']['taskid']))
            ...
            self.helper.api.stix_cyber_observable.add_external_reference(id=opencti_entity['id'],
                                                                         external_reference_id=external_reference_task['id'])

            result = self.wait_for_task(task['data']['taskid'], timer=self.task_timer+20)
            ...
            for tag in result['data']['analysis']['tags']:
                ...
                self.helper.api.stix_cyber_observable.add_label(id=opencti_entity['id'],
                                                                label_id=label["id"])
            # Add score
            ...
            if anyrun_score < opencti_score:
                note = self.helper.api.note.create(authors=[self.organization['id']],
                                                   content="ANY.RUN score: {}".format(str(anyrun_score)))
                self.helper.api.note.add_stix_object_or_stix_relationship(id=note['id'],
                                                                          stixObjectOrStixRelationshipId=opencti_entity['id'])
            else:
                self.helper.api.stix_cyber_observable.update_field(
                    id=opencti_entity['id'],
                    input={"key": "x_opencti_score",
            ...
            if self.enable_mitre:
                for pattern_anyrun in result['data']['mitre']:
                    patterns_opencti = self.helper.api.attack_pattern.list(search=pattern_anyrun['id'])
            ...
            # add malicious iocs
            if self.enable_ioc:
                response = self.call_anyrun_api('GET', 'report/{}/ioc/json'.format(task['data']['taskid']))
                iocs = json.loads(response.text)
                for ioc in iocs:
                    if self.iocs_types_mapping.get(ioc['type']) is None:
                        self.helper.log_warning('Indicator type {} is not supported. value ({})'.format(ioc['type'],
                                                                                                        str(ioc)))
                        continue
                    if ioc['reputation'] == 2:
                        new_observable = self.helper.api.stix_cyber_observable.create(
                            observableData={"type": self.iocs_types_mapping.get(ioc['type']),
                                            "value": ioc['ioc']},
                        ...
                        indicator = self.helper.api.indicator.create(
                            ...
                            description=ioc['category'],
                            pattern_type="stix",
                            pattern="[{}:value = '{}']".format(self.iocs_types_mapping[ioc['type']].lower(),
                                                               ioc['ioc']),
                            x_opencti_main_observable_type=self.iocs_types_mapping[ioc['type']],
                            x_opencti_score=opencti_score,
                            update=True,
                            x_mitre_platforms=[get_config_variable("ANYRUN_OS", ["anyrun", "os"], {}, default='windows')],
                            createdBy=self.organization['id']
                        )

                        if new_observable['id'] != opencti_entity['id']:
                            self.helper.api.indicator.add_stix_cyber_observable(id=indicator['id'],
                                                                                # indicator=indicator,
            ...
            if self.enable_processes:
                ...
                for proc in result['data']['processes']:
                    if proc['scores']['verdict']['score'] > 0 and proc['important'] is True:

                        # self.helper.log_error(str(proc))
                        process = self.helper.api.stix_cyber_observable.create(
                            observableData={
                                "type": "Process",
                                "x_opencti_description": proc['scores']['verdict']['threatLevelText'],
                                "cwd": proc['fileName'],
                                "pid": proc['pid'],
                        ...
                        if proc.get('threatName') is not None:
                            ...
                            self.helper.api.stix_cyber_observable.add_label(id=process['id'],
                                                                            label_id=label["id"])
                        self.helper.api.stix_core_relationship.create(
                        ...
                        procs_links.append({'pid': proc['pid'],
                                            'ppid': proc['ppid'],
