...
class HatchingTriageSandboxConnector:
    ...
    def _process_overview_report(self, observable, overview_dict, sample_id):
        ...
        # Attach IP addresses
        ips = [task_dict["iocs"]["ips"] for task_dict in overview_dict["targets"]]
        for ip in ips[0]:
    ...
    def _process_file(self, observable):
        ...
        self.helper.log_info(sample_json)

        sample_id = sample_json["id"]
        sample_status = sample_json["status"]
        self.helper.log_info(f'Analysis {sample_id} has status "{sample_status}".')

        # Stream events for the sample
