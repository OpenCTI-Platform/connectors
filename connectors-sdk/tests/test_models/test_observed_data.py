...
def test_observed_data_requires_first_observed():
    ...
    input_data = {
        ...
        "number_observed": 5,
        "objects": [IPV4Address(value="1.1.1.1")],
    }
...
def test_observed_data_requires_objects():
    """Test that ObservedData requires objects field."""
    input_data = {
        "first_observed": datetime(2025, 1, 1, 12, 0, 0, tzinfo=timezone.utc),
        "last_observed": datetime(2025, 1, 1, 13, 0, 0, tzinfo=timezone.utc),
        "number_observed": 5,
    ...
    with pytest.raises(ValidationError) as error:
        ObservedData.model_validate(input_data)
        assert "objects" in str(error.value)


def test_observed_data_should_not_accept_empty_objects():
    """Test that ObservedData cannot be created with empty objects list."""
    input_data = {
        "first_observed": datetime(2025, 1, 1, 12, 0, 0, tzinfo=timezone.utc),
        "last_observed": datetime(2025, 1, 1, 13, 0, 0, tzinfo=timezone.utc),
        "number_observed": 5,
    ...
    with pytest.raises(ValidationError) as error:
        ObservedData.model_validate(input_data)
        assert "objects must contain at least one element" in str(error.value)


def test_observed_data_should_not_accept_none_objects():
    """Test that ObservedData cannot be created with None objects."""
    # Given valid input data for ObservedData with objects = None
    input_data = {
        "first_observed": datetime(2025, 1, 1, 12, 0, 0, tzinfo=timezone.utc),
        "last_observed": datetime(2025, 1, 1, 13, 0, 0, tzinfo=timezone.utc),
        "number_observed": 5,
...
def test_observed_data_with_single_object():
    ...
    observed_data = ObservedData.model_validate(input_data)
    assert len(observed_data.objects) == 1
    assert observed_data.objects[0].value == "192.168.1.1"


def test_observed_data_with_multiple_objects():
    """Test that ObservedData can be created with multiple objects."""
    ipv4 = IPV4Address(value="192.168.1.1")
    ...
    observed_data = ObservedData.model_validate(input_data)
    assert len(observed_data.objects) == 2
...
def test_observed_data_to_stix2_object_with_objects(
    fake_valid_organization_author,
    fake_valid_tlp_markings,
):
