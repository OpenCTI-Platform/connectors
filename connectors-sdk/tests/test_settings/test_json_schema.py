...
class TestSanitizingJsonSchema:
    ...
    def test_removes_change_value_from_metadata(self):
        """Test that change_value function is removed from schema metadata."""

        class TestModel(BaseModel):
            field: str = Field(
                default="test",
                json_schema_extra={"change_value": lambda x: x.upper()},
            )

        schema = TestModel.model_json_schema(schema_generator=SanitizingJsonSchema)

        # Check that change_value is not in the generated schema
        properties = schema["properties"]
        assert "change_value" not in str(properties)
    ...
    def test_preserves_other_metadata(self):
        ...
        class TestModel(BaseModel):
            field: str = Field(
                ...
                json_schema_extra={
                    "new_namespace": "ns",
                    "new_variable_name": "var",
                    "change_value": lambda x: x,
                },
            )
        ...
        assert "new_variable_name" in str(schema)
    ...
    def test_handles_nested_models(self):
        ...
        class NestedModel(BaseModel):
            nested_field: str = Field(
                default="test", json_schema_extra={"change_value": lambda x: x}
            )
        ...
        # Should not contain change_value function
        assert "change_value" not in str(schema)
    ...
    def test_generate_inner_with_function_in_metadata(self):
        ...
        # change_value should not appear in the string representation
        schema_str = str(schema)
        assert "change_value" not in schema_str

        # But other metadata should be preserved
        assert "new_variable_name" in schema_str or "deprecated" in schema_str
...
class TestConnectorConfigJsonSchemaGenerator:
    ...
    def test_flatten_config_loader_schema_basic(self):
        ...
        result = ConnectorConfigJsonSchemaGenerator.flatten_config_loader_schema(
            root_schema
        )
                ...
                "old_namespace": {
                    ...
                    "properties": {
                        "field": {
                            "type": "string",
                            "description": "A field",
                            "new_variable_name": "renamed_field",
    ...
    def test_flatten_with_new_variable_name(self):
        """Test flattening with new_variable_name metadata."""
        root_schema = {
            "$schema": "http://json-schema.org/draft-07/schema#",
            "$id": "test",
            "type": "object",
    ...
    def test_flatten_removes_metadata_fields(self):
        """Test that new_variable_name and new_namespace are removed from final schema."""
        root_schema = {
            "$schema": "http://json-schema.org/draft-07/schema#",
            "$id": "test",
            "type": "object",
        ...
        assert "new_variable_name" not in result["properties"]["CONNECTOR_FIELD"]
