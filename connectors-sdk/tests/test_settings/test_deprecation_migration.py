...
from connectors_sdk.utils.deprecations import LegacyField
...
class TestMigrateDeprecation:
    ...
    def test_migrate_deprecated_variable_in_settings(self, monkeypatch):
        ...
        class CustomConnectorConfig(BaseExternalImportConnectorConfig):
            old_field: str | None = LegacyField(
    ...
    def test_migrate_deprecated_variable_with_value_transformation(self, monkeypatch):
        ...
        monkeypatch.setenv("CONNECTOR_NAME", "Test")
        monkeypatch.setenv("CONNECTOR_SCOPE", "test")
        monkeypatch.setenv("CONNECTOR_OLD_DURATION", "5")

        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always")
            settings = TestSettings()

        # Value should be transformed (but timedelta parsing will handle it)
        # We're testing the transformation happens
        assert settings.connector.duration_period is not None
    ...
    def test_error_when_namespace_has_new_variable_name_in_legacy_field(
        self, monkeypatch
    ):
        """Test that ValueError is raised when a deprecated namespace has new_variable_name."""
        ...
        class TestSettings(BaseConnectorSettings):
            old_connector: DeprecatedConfig = Field(
                ...
                json_schema_extra={
                    ...
                    "new_variable_name": "renamed",  # This should trigger ValueError
                },
            )
    ...
    def test_migrate_with_nested_field_metadata(self, monkeypatch):
        ...
        class CustomConnectorConfig(BaseExternalImportConnectorConfig):
            special_field: str | None = LegacyField(
    ...
    def test_migrate_entire_namespace(self, monkeypatch):
        ...
        class TestSettings(BaseConnectorSettings):
            old_connector: SkipValidation[CustomConnectorConfig] = LegacyField(
