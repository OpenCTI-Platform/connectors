...
class TestRateLimitBlock:
    def test_rate_limit_block_sleeps_instead_of_raising(self):
        """When rate_limit_block=True, adapter sleeps instead of raising."""
        adapter = _RateLimitAdapter(
            rate_limit="1/second", rate_limit_key="test", rate_limit_block=True
        )
        mock_request = MagicMock(spec=requests.PreparedRequest)
        mock_request.url = "https://api.example.com/test"
        ...
        with patch.object(requests.adapters.HTTPAdapter, "send") as mock_send:
            ...
            with patch("connectors_sdk.client.rate_limit.time.sleep") as mock_sleep:
                # First call succeeds
                adapter.send(mock_request)
                # Second call should sleep, not raise
                adapter.send(mock_request)
                mock_sleep.assert_called_once()
                # Verify the sleep duration is a short window (< 2s), not a Unix timestamp
                sleep_duration = mock_sleep.call_args[0][0]
                assert (
                    sleep_duration < 2
                ), f"sleep({sleep_duration}) looks like a Unix timestamp, not a duration"
