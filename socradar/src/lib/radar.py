...
class RadarConnector:
    def __init__(self):
        ...
        self.interval = get_config_variable(
            "RADAR_RUN_INTERVAL", ["radar", "radar_run_interval"], config, default=600
        )
    ...
    def _process_feed(self, work_id: str):
        """
        Batched feed ingestion in chunks of 50 items
        """
            ...
            try:
                ...
                for idx, item in enumerate(items, start=1):
                    ...
                    if idx % BATCH_SIZE == 0:
                        ...
                        self.helper.log_info(
                            f"Sent {len(stix_batch)} objects for items {idx - BATCH_SIZE + 1}..{idx}"
                        )
                        stix_batch = []
                ...
                if stix_batch:
                    ...
                    self.helper.log_info(
                        f"Sent {len(stix_batch)} leftover objects for {collection_name}"
                    )
