"""
Unit tests for LLMHelper relation extraction logic
in reportimporter.llmhelper.

Purpose:
Validate that LLMHelper.openai_extract_relations() integrates correctly
...
class DummyConfig(ConfigParser):
    """Minimal configuration stub for LLMHelper testing."""
...
def _run_extract(helper: LLMHelper, text: str) -> dict:
    """
    ...
    with patch.object(LLMHelper, "_call_model_relations", return_value=fake_response):
        return helper.openai_extract_relations(text)


def test_llmhelper_extract_relations_merges_spans_event_loop():
    """
    Ensure openai_extract_relations() produces a valid structured output
    and merges span data without raising exceptions.
    """
    helper = LLMHelper(config=DummyConfig())
