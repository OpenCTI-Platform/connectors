...
def test_scan_structured_iocs_ids_are_deterministic_per_ioc_value():
    """Repeated identical IOCs share an ID; distinct IOCs get distinct IDs."""
    text = "IPs: 1.2.3.4 repeated 1.2.3.4 and 5.6.7.8"
    result = [s for s in scan_structured_iocs(text) if s.type == "IPv4-Addr.value"]

    ids_for_1234 = [s.id for s in result if s.normalized_value == "1.2.3.4"]
    ids_for_5678 = [s.id for s in result if s.normalized_value == "5.6.7.8"]

    assert len(ids_for_1234) == 2
    assert len(set(ids_for_1234)) == 1
    assert len(ids_for_5678) == 1
    assert ids_for_1234[0] != ids_for_5678[0]
