...
def test_file_names_not_detected_as_domains():
    """
    Ensure common file names are not misclassified as domain names.
    """
    text = "Artifacts: apt1-report.zip, report.htm, and IOC example.com"
    spans = list(scan_structured_iocs(text))
    domain_values = {
        s.normalized_value.lower() for s in spans if s.type == "Domain-Name.value"
    }

    assert "example.com" in domain_values
    assert "apt1-report.zip" not in domain_values
    assert "report.htm" not in domain_values
