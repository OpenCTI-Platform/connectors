"""
Unit test for LLMHelper prompt trimming and token-budget control.

Purpose:
Validate that LLMHelper correctly enforces its token budget when constructing
...
def test_trim_hints_to_budget_keeps_under_limit(monkeypatch, tmp_path):
    """
    Verify that LLMHelper trims hints such that the composed prompt
