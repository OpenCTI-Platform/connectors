...
def _normalize_threat_actor_lookup(raw: object) -> str:
    """Normalize threat actor subtype values into OpenCTI lookup keys."""
    value = str(raw or "").strip().lower().replace("_", "-")
    if not value:
        return ""
    if value in {"group", "threat-actor-group"}:
        return "THREAT-ACTOR-GROUP"
    if value in {"individual", "threat-actor-individual"}:
        return "THREAT-ACTOR-INDIVIDUAL"
    if value.startswith("threat-actor-"):
        return value.upper()
    return ""
...
def stix_lookup_type(obj: dict[str, object] | None) -> str:
    ...
    if stix_type == "THREAT-ACTOR":
        for candidate in (
            obj.get("x_opencti_threat_actor_type"),
            obj.get("x_opencti_identity_type"),
        ):
            normalized = _normalize_threat_actor_lookup(candidate)
            if normalized:
                return normalized
        threat_actor_types = obj.get("threat_actor_types")
        if isinstance(threat_actor_types, list):
            for candidate in threat_actor_types:
                normalized = _normalize_threat_actor_lookup(candidate)
                if normalized:
                    return normalized
        return "THREAT-ACTOR"
