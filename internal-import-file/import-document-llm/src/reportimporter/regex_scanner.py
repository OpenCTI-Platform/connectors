...
def _hostname_has_public_suffix(hostname: str) -> bool:
    """
    Validate that 'hostname' looks like a domain name.

    Prefer PSL-backed matches via tldextract, but also accept multi-label internal
    domains when they are not obvious filenames or web assets.
    """
    if not hostname:
        return False
    ...
    # Avoid treating file-like tokens (e.g., payload.zip, readme.htm) as domains.
    if labels[-1].lower() in _NON_DOMAIN_FILE_EXTENSIONS:
        return False
