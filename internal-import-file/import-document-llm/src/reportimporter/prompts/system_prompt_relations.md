...
* `"label"` MUST be the relation verb and MUST match the active OpenCTI schema guidance supplied in the task prompt.
...
* TARGETS: Actor/Intrusion-Set/Campaign/Incident -> Organization, Individual, Sector, Country, Region, Vulnerability.name
...
* LOCATED-AT: Infrastructure/Organization/Individual/Sector -> City, Country, Region
* Observable relations may be emitted when the text explicitly states them and the task prompt confirms they are allowed by OpenCTI.
