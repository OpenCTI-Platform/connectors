...
from .regex_scanner import _NON_DOMAIN_FILE_EXTENSIONS, normalize_stix_value
...
def _cp(cp: dict | None) -> dict:
    """Helper: split common STIX properties from custom_properties when present."""
    if not cp:
        return {}
    cp = dict(cp)
    out: dict[str, Any] = {}
    ...
    if created_by_ref:
        out["created_by_ref"] = created_by_ref
    if cp:
...
def _normalize_domain(domain: str) -> str | None:
    ...
    if len(labels) < 2:
        return None
    if labels[-1] in _NON_DOMAIN_FILE_EXTENSIONS:
        return None
    ...
    if not (ext.domain and ext.suffix):
        ...
        if (
            len(final) < 2
            or len(final) > 24
            or not final.isalpha()
            or final in _NON_DOMAIN_FILE_EXTENSIONS
        ):
            return None
...
def _sanitize_url(value: str) -> str:
    ...
    - Preserve original scheme semantics (do not force a default scheme)
...
def _incident_id(name: str) -> str:
    """Generate a stable incident ID from its name."""
    cleaned = (name or "").strip()
    try:
        return Incident.generate_id(cleaned)
    except TypeError:
        return _make_stix_id("incident", cleaned.lower())
    except Exception:
        return _make_stix_id("incident", cleaned.lower())


def _channel_kwargs(cp: dict | None) -> dict:
    """Split common props for Channel custom object creation."""
    cp = dict(cp or {})
    created_by_ref = cp.pop("created_by_ref", None)
    out: dict[str, Any] = {"custom_properties": cp}
    if created_by_ref:
        out["created_by_ref"] = created_by_ref
    return out
...
def _country_code(name: str) -> str:
    ...
    try:
        ...
        try:
            # Common model error: city/state/province tagged as Country.
            subdivision = pycountry.subdivisions.lookup(key)
            if subdivision:
                _helper.connector_logger.debug(
                    f"Value {name!r} appears to be a subdivision, not a country; using 'XX'."
                )
                return "XX"
        except (LookupError, AttributeError):
            pass
...
def _is_valid_hash(value: str, algo: str) -> bool:
    ...
    # NOTE: MD5/SHA-1 are intentionally supported for CTI interoperability because
    # real-world intel feeds and malware reports still publish these legacy IOCs.
    # This connector only validates/parses these strings; it does not use them for
    # cryptographic trust, authentication, signing, or integrity guarantees.
    patterns = {
        "MD5": r"^[a-fA-F0-9]{32}$",  # nosec B324
        "SHA-1": r"^[a-fA-F0-9]{40}$",  # nosec B324
...
def create_stix_object(
    ...
    if not value:
        _helper.connector_logger.debug(
            f"Skipping empty value for category '{category}'."
        )
        return []
    try:
    ...
    if normalized_value is None or (
        ...
        _helper.connector_logger.warning(
            f"Normalization produced empty value for '{category}', skipping {value!r}."
        )
        return []
...
def compose_indicators_from_observables(
    ...
    for o in observables:
        ...
        if t == "domain-name" and v:
            ...
            md5 = hashes.get("MD5")  # nosec B324
            sha1 = hashes.get("SHA-1") or hashes.get("SHA1")  # nosec B324
...
stix_object_mapping: dict[str, Callable[[str, list[str], dict[str, Any]], Any]] = {
    ...
    "Artifact": lambda v, om, cp: stix2.Artifact(
        ...
        # hash the actual legible content for better deduplication/searchability, but preserve the original value in x_opencti_original_value for reference
        payload_bin=base64.b64encode(v.strip().encode("utf-8")).decode("ascii"),
        allow_custom=True,
        **_mk(om),
        **_sco_cp(cp),
    ),
    ...
    "Email-Message.value": lambda v, om, cp: stix2.EmailMessage(
        **(
            {"message_id": v.strip(), "is_multipart": False}
            if _MESSAGE_ID.fullmatch(v.strip())
            else {"subject": v.strip(), "is_multipart": False}
        ),
        allow_custom=True,
        **_mk(om),
        **_sco_cp(cp),
    ),
    # NOTE: MD5/SHA-1 object mappings are retained for IOC interoperability only.
    # They are not used as secure cryptographic primitives in this connector.
    "File.hashes.MD5": lambda v, om, cp: _create_file_hash(
        _lower_hex(v), om, cp, "MD5"  # nosec B324
    ),
    "File.hashes.SHA-1": lambda v, om, cp: _create_file_hash(
        _lower_hex(v), om, cp, "SHA-1"  # nosec B324
    ),
    ...
    "X509-Certificate.sha1_fingerprint": lambda v, om, cp: stix2.X509Certificate(
        hashes={"SHA-1": v.strip().lower()},  # nosec B324
        allow_custom=True,
        **_mk(om),
        **_sco_cp(cp),
    ),
    ...
    "Threat-Actor": lambda v, om, cp: stix2.ThreatActor(
        id=ThreatActor.generate_id(v, "unknown"),
        name=v.strip(),
        allow_custom=True,
        **_mk(om),
        **_cp({**(cp or {}), "threat_actor_types": ["unknown"]}),
    ),
        ...
        **_cp(
            {
                **(cp or {}),
                ...
                "x_opencti_identity_type": "group",
                "x_opencti_threat_actor_type": "group",
            }
        ),
    ),
        ...
        **_cp(
            {
                **(cp or {}),
                "threat_actor_types": ["unknown"],
                "x_opencti_identity_type": "individual",
                "x_opencti_threat_actor_type": "individual",
            }
        ),
    ),
