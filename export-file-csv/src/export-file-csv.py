...
class ExportFileCsv:
    ...
    def _process_message(self, data):
        ...
        if entity_id is not None:
            ...
            self.helper.api.stix_domain_entity.push_entity_export(entity_id, file_name, csv_data)
            self.helper.log_info(
            ...
            if IdentityTypes.has_value(entity_type):
                if list_args['filters'] is not None:
                    list_args['filters'].append({'key': 'entity_type', 'values': [entity_type]})
                else:
                    list_args['filters'] = [{'key': 'entity_type', 'values': [entity_type]}]
            ...
            do_list = lister.get(
                entity_type.lower(),
                lambda **kwargs: self.helper.log_error('Unknown object type "' + entity_type + '", doing nothing...')
            )
