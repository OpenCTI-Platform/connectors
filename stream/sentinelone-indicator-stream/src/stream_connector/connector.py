...
DEBUG_HEARTBEAT_MESSAGES = ["[<3] I'm alive!", "[<3] I'm still here!", "[<3] I'm not dead yet!"]
...
class IndicatorConnector:
    def __init__(self):
        ...
        self.buffer = IndicatorBuffer(self.helper.connector_logger, self.s1_client.send_indicators)
        self.helper.connector_logger.info("Indicator Connector Initialised Successfully.")

    def _setup_development_environment(self, helper):
        #Force the connector to read from the first REDIS event
        helper.set_state({'start_from': '0-0', 'recover_until': datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")})
    ...
    def _process_message(self, msg: dict) -> None:
        ...
        try:
            ...
            if msg.event == "create":
                ...
                if self.indicator_creation_identifier in message_dict.get("message", ""):
                    if len(self.buffer.buffer) == 0:
                        self.helper.connector_logger.info("Listening for new Indicators...")
                    self.helper.connector_logger.debug("New indicator to process found.")
                    if indicator_payload := self._validate_and_extract_indicator(message_dict):
                        self.buffer.add_item(indicator_payload)
        except SentinelOnePermissionError:
            raise
        except Exception as e:
            self.helper.connector_logger.error(f"Connector cannot process consumed message, error: {e}")
            pass
    ...
    def _validate_and_extract_indicator(self, message_dict: dict) -> Optional[dict]:
        ...
        if not ioc_type or not ioc_value:
            self.helper.connector_logger.debug(f"Unsupported indicator type: {message_dict.get('data', {}).get('pattern', 'Pattern Unknown')}, skipping.")
            self.helper.connector_logger.debug(message_dict.get('data',{}).get('id', "ID Unknown"))
            return None
        indicator_data = message_dict.get("data", {})
        return self._build_indicator_payload(indicator_data, ioc_type, ioc_value)


    def _extract_indicator_info(self, message_dict: dict) -> Tuple[Optional[str], Optional[str]]:
        """
        ...
        if message_dict.get("data",{}).get("pattern_type") == "stix":
            for extension_data in message_dict.get("data", {}).get("extensions", {}).values():
                if observable_values_list := extension_data.get("observable_values", []):
                    for observable_values in observable_values_list: 
                        if ("type" in observable_values.keys() and "value" in observable_values.keys()) or ("hashes" in observable_values.keys()):
                            ioc_type, ioc_value = self._extract_indicator_type_value(observable_values)
                            if ioc_type and ioc_value:
                                return ioc_type, ioc_value

        return None, None

    def _extract_indicator_type_value(self, observable_values: dict) -> Tuple[Optional[str], Optional[str]]:
        """
    ...
    def _build_indicator_payload(self, indicator_data: dict, ioc_type: str, ioc_value: str) -> dict:
        """
        ...
        return {s1_key: ioc_value for s1_key, ioc_value in payload.items() if ioc_value is not None}
...
class IndicatorBuffer:
    ...
    def add_item(self, indicator: dict) -> None:
        ...
        self.last_indicator_appended_time = datetime.now()
        self.logger.debug(f"IOC with OpenCTI ID: {indicator.get('externalId')} added to Buffer. Current size: {len(self.buffer)}")

        if self._should_send_size():
            self.logger.info(f"{self.max_buffer_size} Indicators processed. Preparing to send to SentinelOne.")
    ...
    def _buffer_monitor(self) -> None:
        ...
        while True:
            ...
            self.heartbeat_messages_index = (self.heartbeat_messages_index + 1) % len(self.heartbeat_messages)
            ...
            for counter_name, counter_value in [
                ...
                if counter_value >= self.MAX_COUNTER_VALUE:
                    ...
                    self.logger.warning(f"{counter_name} counter reached maximum value, resetting to 0")

            #Info stats/heartbeat
            if (current_time - last_info_heartbeat).total_seconds() >= self.info_heartbeat_interval:
                self.logger.info(f"Indicator Connector is alive. Session stats:")
                self.logger.info(f"Connector has Sent {self.indicators_sent} Indicators.")
                self.logger.info(f"Connector has Failed to Send {self.buffers_failed} Bundles.")
    ...
    def _should_send_timeout(self) -> bool:
        ...
        """
        return self.buffer and (datetime.now() - self.last_indicator_appended_time).total_seconds() >= self.buffer_timeout
    ...
    def _send_and_clear(self) -> None:
        ...
        self.logger.debug(f"Sending bundle of {len(self.buffer)} Indicators to SentinelOne.")
        if self.send_buffer(self.buffer):
            self.logger.info(f"Bundle of {len(self.buffer)} Indicators sent successfully.")
            self.indicators_sent += len(self.buffer)
        else:
            self.logger.error("Error, unable to send Bundle to SentinelOne. Set logging level to debug for more information.")
            self.logger.debug(f"IDs of Indicators that failed to send: {', '.join([indicator.get('externalId') for indicator in self.buffer])}")
