...
class ZscalerConnector:
    ...
    def authenticate_with_zscaler(self):
        ...
        response = self.handle_rate_limit(requests.post, url, json=payload, headers=headers)
        ...
        if response and response.status_code == 200:
            ...
            logging.info(f"Authenticated successfully with Zscaler. JSESSIONID: {self.session_cookie}")
        else:
            logging.error(f"Failed to authenticate with Zscaler: {response.status_code} - {response.text}")
    ...
    def get_domain_classification_in_zscaler(self, domain):
        ...
        response = self.handle_rate_limit(requests.post, lookup_url, headers=headers, data=payload)
        ...
        if response and response.status_code == 200:
            ...
            if isinstance(lookup_data, list) and len(lookup_data) > 0:
                ...
                logging.warning(f"Unexpected or empty response for domain {domain}: {lookup_data}")
        else:
    ...
    def get_zscaler_blocked_domains(self):
        ...
        if response and response.status_code == 200:
            ...
            logging.error(f"Failed to retrieve blocked domains: {response.status_code} - {response.text}")
            return []
    ...
    def check_and_send_to_zscaler(self, data, event_type):
        ...
        domain = self.is_valid_domain(data["pattern"])  # Use the pattern for extracting and validating the domain
        if domain:
            ...
            if domain in blocked_domains:
                ...
                logging.info(f"The domain {domain} is not blocked. Sending to Zscaler...")
                self.send_to_zscaler(domain, event_type)
        else:
            logging.error(f"Unable to process indicator due to invalid domain pattern: {data['pattern']}")
    ...
    def send_to_zscaler(self, domain, event_type):
        ...
        if event_type == "create":
            ...
            response = self.handle_rate_limit(requests.put, base_url, headers=headers, json=payload)
        ...
        if response and response.status_code == 200:
            ...
            logging.error(f"Failed to send {event_type} event to Zscaler: {response.text}")
    ...
    def activate_zscaler_changes(self):
        ...
        response = self.handle_rate_limit(requests.post, activation_url, headers=headers)
    ...
    def _process_message(self, msg):
        ...
        if data.get("type") == "indicator" and data.get("pattern_type") == "stix":
            ...
            logging.info("Ignoring indicator: unsupported pattern type or indicator type.")
