...
class HarfangLabConnector:
        ...
        if msg.event == "create":
            if data["type"] == "indicator":
                if data['pattern_type'] == "yara":
                    ...
                    self.helper.log_error(f"[CREATE] Unsupported pattern type")
        ...
        if msg.event == "update":
            if data["type"] == "indicator":
                if data['pattern_type'] == "yara":
                    ...
                    #     self.update_observable(entity, msg, "stix", "IOCRule", self.stix_list_id)
                    return
                else:
                    self.helper.log_error(f"[UPDATE] Unsupported pattern type")
            return
        ...
        if msg.event == "delete":
            if data["type"] == "indicator":
                if data['pattern_type'] == "yara":
                    ...
                    self.helper.log_error(f"[DELETE] Unsupported pattern type")
    ...
    def create_observable(self, data, pattern, uri):
        ...
        if data['type'] == 'relationship':
            ...
            if not entity['observables']:
                return self.helper.log_error(f"[CREATE] The indicator has no observables")
            else:
                return self.helper.log_info(f"[CREATE] The process between indicator stix and observables has been completed")
        return
    ...
    def check_and_create_observable_id(self, data, pattern, uri, entity):
        ...
        if get_observable['count'] > 0:
            ...
            if is_enabled_observable is False:
                ...
                return self.helper.log_error(f"[CREATE] A rule with this ID already exists")
        else:
    ...
    def update_indicator(self, data, msg, pattern, uri, source_list_id):
        ...
        if check_patch_path == '/name':
            ...
            response_name_indicator = self._query("get", f"/{uri}/?search={indicator_previous_name}&source_id={source_list_id}")
            ...
            response_name_indicator = self._query("get", f"/{uri}/?search={indicator_name}&source_id={source_list_id}")
