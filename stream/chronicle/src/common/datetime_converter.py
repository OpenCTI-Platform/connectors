...
def iso8601_datetime_utc(utc_date_time: str) -> datetime.datetime:
    ...
    utc_date_time = re.sub(
        r"(\d{2}-\d{2}-\d{2})\s+(\d)", r"\1T\2", utc_date_time
    ).upper()
    ...
    try:
        ...
        return datetime.datetime.strptime(
            f"{utc_date_time}+0000", "%Y-%m-%dT%H:%M:%S.%fZ%z"
        )
    except ValueError:
    ...
    return datetime.datetime.strptime(f"{utc_date_time}+0000", "%Y-%m-%dT%H:%M:%SZ%z")
...
def strftime(utc_date_time: datetime.datetime) -> str:
    ...
    if utc_date_time is None:
        return ""
    return utc_date_time.astimezone(datetime.timezone.utc).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )
