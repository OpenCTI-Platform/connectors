...
| MALANTA_ATTRIBUTION_ACTOR_SEPARATORS | `string` |  | string | `","` | Characters splitting several actors inside a single label. Each character is treated as a separator, so ',;' splits on both. Malanta occasionally emits comma-joined tokens such as 'apt:APT17,APT5'. Deliberately a plain string rather than a list: a comma-separated list type cannot represent a comma as a value. | ```,```, ```,;``` |
