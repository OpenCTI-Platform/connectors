...
import pytest
from connector.connector import FortiEDRConnector


def _make_connector(live_stream_id: str = "live"):
    helper = MagicMock()
    helper.connect_live_stream_id = live_stream_id
    with patch("connector.connector.FortiEDRClient") as client_cls:
        connector = FortiEDRConnector(config=MagicMock(), helper=helper)
