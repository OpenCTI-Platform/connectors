...
class ArcSightConnector:
    """
    Specifications of the ArcSight stream connector.

    This connector listens to an OpenCTI live stream and synchronises indicators
    (IPv4/IPv6, domain names, URLs and file hashes) to an ArcSight ESM Active List
    in real time through the ESM Service Layer REST API.
    """

    def __init__(self, config: ConnectorSettings, helper: OpenCTIConnectorHelper):
        """
        Initialize `ArcSightConnector` with its configuration.
        ...
        if event in ("create", "update"):
            if self.client.add_indicator(data):
                self.helper.connector_logger.info(
                    "[%s] Indicator added to ArcSight Active List" % event.upper()
                )
        elif event == "delete":
            if self.client.remove_indicator(data):
                self.helper.connector_logger.info(
                    "[DELETE] Indicator removed from ArcSight Active List"
                )
