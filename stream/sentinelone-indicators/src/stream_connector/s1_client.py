...
class SentinelOneClient:
    ...
    def send_indicators(self, indicators: list) -> bool:
        ...
        if s1_response and self.config.log_s1_response:
            indicator_uuids = [indicator.get("uuid") for indicator in s1_response.get("data",{})]
            self.logger.info(f"Batch of UUIDs as provided by SentinelOne response: {indicator_uuids}")

        return s1_response


    #TODO: probably more formatting.
    def _send_api_req(self, url: str, request_type: str, payload: dict={}, wait_time: int=1, attempts: int=0) -> Optional[dict]:
        """
        ...
        if response.status_code == 401:
            raise SentinelOnePermissionError("Permissions Error, SentinelOne returned a 401, please check your API key and account ID.")
        ...
        #Rate Limiting requires an exponential backoff as a workaround.
            ...
            if attempts < self.config.max_api_attempts:
                ...
                return self._send_api_req(url, request_type, payload, new_wait_time, attempts + 1)
            else:
                self.logger.error(
        ...
        #Random errors should be logged with context of their origin
            ...
            self.logger.debug(
                f"S1 responded with: {response.text}"
            )
            return False
