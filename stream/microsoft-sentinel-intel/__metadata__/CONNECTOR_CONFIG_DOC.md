...
| MICROSOFT_SENTINEL_INTEL_PUBLISH_IDENTITIES | `boolean` |  | boolean | `false` | Also push STIX Identity objects (e.g. an indicator's author) received from the stream to Microsoft Sentinel, in addition to Indicators, so their display name resolves correctly instead of only showing the identifier. |
