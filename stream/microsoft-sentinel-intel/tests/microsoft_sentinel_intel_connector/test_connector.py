...
def fixture_connector(mocked_api_client: MagicMock) -> MicrosoftSentinelIntelConnector:
    config = ConnectorSettings()
    helper = OpenCTIConnectorHelper(config.model_dump_pycti())
    return MicrosoftSentinelIntelConnector(helper=helper, config=config, client=Mock())
