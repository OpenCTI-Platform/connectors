...
from connector_migrator.utils.ast import get_connector_class_file_path
...
class ConnectorMigrator:
    ...
    def _add_dot_env_sample(self):
        ...
        env_vars = docker_compose_connector_service.get("environment") or []
        if env_vars:
            dot_env_path = self.connector_path / ".env.sample"
            dot_env_content = "\n".join(env_vars) + "\n"
            dot_env_path.write_text(dot_env_content, encoding="utf-8")

    def _add_connector_tmp(self):
        src_subdirectories = [
            entry
            for entry in os.scandir(self.entrypoint_path.parent)
            if entry.is_dir() and not entry.name.startswith(".")
        ]

        src_subdirectory_path = self.entrypoint_path.parent
        if len(src_subdirectories) == 0:
            src_subdirectory_path = self.entrypoint_path.parent / "connector"
            src_subdirectory_path.mkdir(parents=True, exist_ok=True)
        if len(src_subdirectories) == 1:
            src_subdirectory_path = self.entrypoint_path.parent / src_subdirectories[0]

        python_files = []
        for _, _, files in os.walk(self.entrypoint_path.parent):
            for file in files:
                if file.endswith(".py"):
    ...
    def _add_settings_tmp(self):
        ...
        config_variables_path = self._find_file_path(
            "config_variables.py"
        ) or self._find_file_path("config_loader.py")
        if config_variables_path:
            settings_tmp_dir_path = config_variables_path.parent
        else:
            connector_class_path = self._find_file_path("connector.py.tmp")
            if connector_class_path:
                settings_tmp_dir_path = connector_class_path.parent
            else:
                settings_tmp_dir_path = self.entrypoint_path.parent
    ...
    def _add_test_conftest(self):
        ...
        tests_conftest_content = templates_adapters.tests_conftest.get_content(
            self.entrypoint_path, tests_directory_path
        )
    ...
    def _add_tests_settings(self):
        ...
        tests_directory_path = test_requirements_txt_path.parent
        tests_settings_directory_path = tests_directory_path / "tests_settings"
        tests_settings_directory_path.mkdir(parents=True, exist_ok=True)
        test_settings_path = tests_settings_directory_path / "test_settings.py"
        test_settings_path.write_text(test_settings_content, encoding="utf-8")

    def _update_connector_tmp(self):
        connector_tmp_path = self._find_file_path("connector.py.tmp")
        settings_tmp_path = self._find_file_path("settings.py.tmp")
        if not (connector_tmp_path and settings_tmp_path):
            raise RuntimeError("File 'connector.py.tmp' or 'settings.py.tmp' not found")

        connector_tmp_content = templates_adapters.src_connector.get_content(
            connector_tmp_path, settings_tmp_path
        )
    ...
    def migrate_files(self):
        ...
        self._add_connector_tmp()
        self._add_settings_tmp()
        self._add_init_tmp()
        self._add_main_tmp()
        self._update_connector_tmp()
    ...
    def apply_ai_fixes(self):
        ...
        main_tmp_path = self._find_file_path("main.py.tmp")
    ...
    def format_files(self):
        ...
        returncode = subprocess.call(
            (
                f"black {src_path} {tests_path} --quiet "
                f"&& isort --profile black {src_path} {tests_path} -q "
