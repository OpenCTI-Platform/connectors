...
def _get_updated_init_content(
    ...
    all_var_index = next(
        (index for index, line in enumerate(lines) if "__all__ =" in line), None
    )
    if all_var_index is not None:
        ...
        lines.append(f'__all__ = ["{connector_class_name}", "ConnectorSettings"]')
