...
test_connector() {
  ...
  {
    # -qq: Hides both informational and warning messages, showing only errors.
    "$python" -m pip install -qq -r "$test_requirements_file" || return 1
      ...
      if [ -f "$project/$CONNECTOR_METADATA_DIRECTORY/connector_config_schema.json" ]; then
        ...
        (
          ...
          generate_config_json_schema "$project" || {
            echo "❌ $project: Generation of config JSON schema failed, skipping connector"
            rm_venv "$project"
            exit 0
          }

          test_connector "$project" || {
            echo "❌ $project: Tests failed, skipping connector"
            rm_venv "$project"
            exit 0
          }
