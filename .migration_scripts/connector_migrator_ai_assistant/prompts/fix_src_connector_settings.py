...
Instructions:
    ...
    4. in the second model (i.e. the one inheriting from `BaseConfigModel`), add `default` to every fields
        - refer to `README.md`, especially the `Environment variables` table in the column `default`
        - refer to the 5th argument (`default` - `None` by default) of `get_config_variable` calls in the rest of the codebase
        - `default` value MUST be of the type declared for the field (except for `None`)
        - if no default value is specified in the codebase or in the `README.md`, do not set any (i.e. field is required)
    5. in the second model (i.e. the one inheriting from `BaseConfigModel`), update each field's type annotation to reflect if the field is optional or not
