...
Instructions:
    ...
    2. fix the `__init__` method of the connector's main class
        - arguments MUST be `self, config: ConnectorSettings, helper: OpenCTIConnectorHelper` and then the other arguments if they're still needed
        - DO NOT change the order of the arguments
        - `self.config` MUST be assigned to `config` (from arguments)
        - `self.helper` MUST be assigned to `helper` (from arguments)
    ...
    6. fix imports
        - add the import of `OpenCTIConnectorHelper` from `pycti` if missing (for typing)
        - keep the other imports as is (DO NOT add any others and DO NOT remove any)
""".format(
        file_path=file_path
    )
