...
| ABUSEIPDB_TLP_LEVEL | `string` |  | `clear` `green` `amber` `amber+strict` `red` | `"clear"` | TLP marking for imported data (`clear`, `green`, `amber`, `amber+strict`, `red`). |
| ABUSEIPDB_IPVERSION | `string` |  | `4` `6` `mixed` | `"mixed"` | IP version filter: `4`, `6`, or `mixed`. |
| ABUSEIPDB_EXCEPT_COUNTRY | `array` |  | string | `[]` | Comma-separated country codes to exclude (e.g., `RU,CN`). |
| ABUSEIPDB_ONLY_COUNTRY | `array` |  | string | `[]` | Comma-separated country codes to include only. |
