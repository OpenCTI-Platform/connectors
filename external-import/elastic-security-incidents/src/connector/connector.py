...
import stix2
from connector.client_api import ElasticApiClient
...
class ElasticSecurityIncidentsConnector:
    ...
    def __init__(self, config: ConnectorSettings, helper: OpenCTIConnectorHelper):
        ...
        self.config = config
        self.helper = helper
        self.client = ElasticApiClient(self.helper, self.config.elastic_security)
        self.tlp_marking = stix2.TLP_AMBER
        self.converter_to_stix = ConverterToStix(
            self.helper, self.config.elastic_security, self.tlp_marking
        )
    ...
    def _get_last_run_time(self) -> datetime:
        ...
        if self.config.elastic_security.import_start_date:
            start_date = datetime.fromisoformat(
                self.config.elastic_security.import_start_date.replace("Z", "+00:00")
            )
            self.helper.connector_logger.info(
    ...
    def process_message(self) -> None:
        ...
        try:
            ...
            if self.config.elastic_security.import_alerts:
            ...
            if self.config.elastic_security.import_cases:
