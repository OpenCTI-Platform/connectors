...
class FlashpointConnector:
    ...
    def _import_indicators(self, start_date: datetime) -> None:
        """
        :return:
        """
        try:
            ...
            # Small overlap to avoid missing indicators at the boundary
            ...
            try:
                for page_number, indicators_page in enumerate(
                    ...
                    for indicator in indicators_page:
                        ...
                        if len(octi_objects) == 0:
                            continue

                        page_main_indicators.append(octi_objects[0])
                        page_octi_objects.extend(octi_objects[1:])
                        page_indicators_count += 1
                        number_indicators += 1

                    # Results are sorted by modified_at asc, use last item as cursor
                    page_last_modified = self._parse_iso_datetime(
                        indicators_page[-1].get("modified_at")
                    )
                    if page_last_modified is not None:
                        last_modified = page_last_modified

                    if len(page_main_indicators) > 0:
                        # Initiate work on first actual bundle to send
                        if work_id is None:
                            friendly_name = (
                                "Flashpoint Indicators run @ "
            ...
            # Clean up legacy state keys from previous MISP-based version
            for legacy_key in [
                "misp_last_run",
                "misp_last_event",
                "misp_last_event_timestamp",
            ]:
                current_state.pop(legacy_key, None)
            self._set_state(current_state)
    ...
    def process_data(self) -> None:
        ...
        try:
            ...
            if self.config.flashpoint.import_indicators:
                indicators_start = self.config.flashpoint.import_start_date
                if "indicators_last_modified" in current_state:
                    parsed = self._parse_iso_datetime(
                        current_state["indicators_last_modified"]
                    )
                    if parsed is not None:
                        indicators_start = parsed
                elif "misp_last_run" in current_state:
                    # Migrate from legacy MISP-based state
                    parsed = self._parse_iso_datetime(current_state["misp_last_run"])
                    if parsed is not None:
                        indicators_start = parsed
                self.helper.connector_logger.info(
