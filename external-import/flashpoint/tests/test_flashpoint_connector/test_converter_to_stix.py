from unittest.mock import Mock

import pytest
import stix2
from flashpoint_client.models import CompromisedCredentialSighting
from flashpoint_connector.converter_to_stix import ConverterToStix


@pytest.fixture
def mock_converter_to_stix() -> ConverterToStix:
    helper = Mock()
    helper.api = Mock()
    # Fake the creation of Author to reproduce ConverterToStix.create_author() behavior
    helper.api.identity.create = Mock(
        return_value={"standard_id": "identity--50b656b6-8aa0-42e3-acf4-e6a6c9bf0ff1"}
    )

    return ConverterToStix(helper=helper)


def test_converter_to_stix_convert_ccm_alert_to_incident(
    mock_converter_to_stix: ConverterToStix,
):
    compromised_credential_sighting = CompromisedCredentialSighting.model_validate(
        # Typical Compromised Credential Sighting in response to /search endpoint call
        {
            "affected_domain": "www.example.com",
            "affected_url": "https://www.example.com/login/password",
            "basetypes": ["credential-sighting"],
            "body": {
                "raw": (
                    "browser: Google Chrome\r\nprofile: Default\r\n"
                    "url: https://www.example.com/login/password\r\nlogin: test@example.com\r\npassword: 123456"
                )
            },
            "breach": {
                "_header": {},
                "basetypes": ["breach"],
                "breach_type": "credential",
                "created_at": {
                    "date-time": "2025-03-28T06:34:46Z",
                    "timestamp": 1743143686,
                },
                "first_observed_at": {
                    "date-time": "2025-03-28T06:34:46Z",
                    "timestamp": 1743143686,
                },
                "fpid": "HTKfiyCCUrGF9rBOIyW0_A",
                "source": "Analyst Research",
                "source_type": "Credential Stealer",
                "title": 'Compromised Users from Steal C Malware "logs_03_28_2025-063258.zip" Mar282025',
            },
            "credential_record_fpid": "SW8X-ZS7V72VsmHnC7vy7Q",
            "customer_id": "0013l12342FOaNwAAL",
            "domain": "example.com",
            "email": "test@example.com",
            "extraction_id": "Ycf7JZmSXoy5iqWzrwmSDg",
            "extraction_record_id": "EoS0TNPJUtuxlSOb2yK8bA",
            "fpid": "jOyBWZYDXMWRSGrqgQsUOg",
            "header_": {
                "indexed_at": 1743173509,
                "pipeline_duration": 63910392709,
            },
            "heuristics": {
                "heuristics_version": "2.18.0",
                "probable_enterprise_host": False,
            },
            "infected_host_attributes": {
                "fpid": "p_AdFteyWvuHz_7ywj0qEw",
                "installed_software": [
                    {"name": "Google Chrome", "version": "134.0.6998.166"},
                    {"name": "McAfee Security Scan Plus", "version": "4.2.707.1"},
                    {
                        "name": "Microsoft SQL Server Management Studio",
                        "version": "19.1.56.0",
                    },
                    {
                        "name": "Microsoft Visual C++ 2015-2022 Redistributable (x64)",
                        "version": "14.36.32532.0",
                    },
                    {
                        "name": "Microsoft Visual Studio Tools for Applications 2019",
                        "version": "16.0.31110",
                    },
                    {
                        "name": "Python 3.10.9 ",
                        "version": "64-bit",
                    },
                ],
                "isp": {
                    "autonomous_system_number": 1234,
                    "autonomous_system_organization": "Test AS organization",
                    "connection_type": "Cable/DSL",
                    "isp": "Test ISP",
                    "organization": "Test organization",
                },
                "location": {
                    "accuracy_radius": 20,
                    "city_name": "Camaçari",
                    "continent_name": "South America",
                    "country_name": "Brazil",
                    "latitude": -12.619,
                    "location": {"lat": -12.619, "lon": -38.2057},
                    "longitude": -38.2057,
                    "subdivision_1_name": "Bahia",
                    "subdivision_2_name": None,
                },
                "machine": {
                    "cpu": ["AMD Ryzen 5 4600G with Radeon Graphics"],
                    "extra": [
                        {"key": "cpu cores", "value": "6"},
                        {"key": "cpu threads", "value": "12"},
                        {"key": "cpu vendor", "value": "amd"},
                        {"key": "computer", "value": "DESKTOP-1234"},
                        {"key": "country", "value": "VN"},
                        {"key": "hostname", "value": "DESKTOP-1234"},
                        {"key": "lid", "value": "Russia 34"},
                        {"key": "netbios", "value": "DESKTOP-1234"},
                        {"key": "path", "value": "C:\\WINDOWS\\SysWOW64\\explorer.exe"},
                    ],
                    "gpu": ["Radeon RX 580 Series"],
                    "language": ["pt-BR"],
                    "os": "Windows 11 Pro (10.0.22631) x64",
                    "resolution": "1920x1080",
                    "user": "Test Username",
                },
                "malware": {
                    "family": "Test malware",
                    "version": "12.34.5",
                },
            },
            "is_fresh": True,
            "last_observed_at": {
                "date-time": "2025-03-28T06:34:46Z",
                "timestamp": 1743143686,
            },
            "password": "123456",
            "password_complexity": {
                "has_lowercase": False,
                "has_number": True,
                "has_symbol": False,
                "has_uppercase": False,
                "length": 8,
                "probable_hash_algorithms": [
                    "Adler-32",
                    "CRC-32B",
                    "FCS-32",
                    "GHash-32-3",
                    "GHash-32-5",
                    "FNV-132",
                    "Fletcher-32",
                    "Joaat",
                    "ELF-32",
                    "XOR-32",
                    "CRC-32",
                    "Cisco Type 7",
                    "Microsoft Outlook PST",
                    "Dahua",
                ],
            },
            "times_seen": 1,
            "username": "test@example.com",
        }
    )

    stix_objects = mock_converter_to_stix.convert_ccm_alert_to_incident(
        alert=compromised_credential_sighting
    )

    assert any(
        [
            stix_object
            for stix_object in stix_objects
            if isinstance(stix_object, stix2.Incident)
        ]
    )
    assert any(
        [
            stix_object
            for stix_object in stix_objects
            if isinstance(stix_object, stix2.EmailAddress)
        ]
    )
    assert any(
        [
            stix_object
            for stix_object in stix_objects
            if isinstance(stix_object, stix2.DomainName)
        ]
    )
    assert any(
        [
            stix_object
            for stix_object in stix_objects
            if isinstance(stix_object, stix2.URL)
        ]
    )
    assert any(
        [
            stix_object
            for stix_object in stix_objects
            if isinstance(stix_object, stix2.UserAccount)
        ]
    )
    assert any(
        [
            stix_object
            for stix_object in stix_objects
            if isinstance(stix_object, stix2.Malware)
        ]
    )
    assert any(
        [
            stix_object
            for stix_object in stix_objects
            if isinstance(stix_object, stix2.MarkingDefinition)
        ]
    )
