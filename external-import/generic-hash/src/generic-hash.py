...
class GenericCSVHashImport:
    def __init__(self):
        ...
        self.csv_url = os.getenv("TARGET_URL") or get_config_variable(
            "GENERIC_CSV_URL", ["genericcsvhashimport", "csv_url"], config
        )
        score_value = os.getenv("SCORE") or get_config_variable(
            "GENERIC_CSV_SCORE", ["genericcsvhashimport", "score"], config, True
        )
        self.score = int(score_value)

        interval_value = os.getenv("INTERVAL") or get_config_variable(
            "GENERIC_CSV_INTERVAL", ["genericcsvhashimport", "interval"], config, True
        )
    ...
    def run(self):
        self.helper.log_info("Démarrage du connecteur Generic CSV Hash Import...")
        # Définir le regex pour une chaîne de 40 caractères hexadécimaux (ex : SHA-1)
        hash_validator = re.compile(r"^[0-9A-F]{40}$", re.IGNORECASE)
        ...
        while True:
            try:
                ...
                if last_run is None or (
                    ...
                    friendly_name = "Generic CSV Hash Import run @ " + now.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    ...
                    try:
                        ...
                        # Chaque ligne doit contenir un hash
                        hash_list = content.splitlines()

                        # Préparation de la référence externe associée à ce flux
                        external_reference = stix2.ExternalReference(
                            source_name="Generic CSV Hash Import",
                            url=self.csv_url,
                            description="Flux CSV contenant des hash",
                        )
                        ...
                        for hash_value in hash_list:
                            ...
                            if hash_validator.match(hash_value):
                                # Création du pattern pour l'indicateur
                                pattern = "[file:hashes.'SHA-1' = '" + hash_value + "']"
                                # Création de l'indicateur STIX
                                stix_indicator = stix2.Indicator(
                                    id=Indicator.generate_id(pattern),
                                    name=hash_value,
                                    description="Hash (SHA-1) importé depuis le flux CSV",
                                ...
                                # Création de l'observable de type File avec le hash
                                stix_file = stix2.File(
                                    ...
                                    object_marking_refs=[stix2.TLP_WHITE],
                                    custom_properties={
                                        "x_opencti_description": "Hash (SHA-1) importé depuis le flux CSV",
                                        "x_opencti_score": self.score,
                                        "created_by_ref": self.identity["standard_id"],
                                        "external_references": [external_reference],
                                    },
                                ...
                                self.helper.log_error(
                                    "La valeur n'est pas un hash valide : " + hash_value
                                )
                                continue
                            ...
                            stix_relationship = stix2.Relationship(
                                id=StixCoreRelationship.generate_id(
                                    "based-on", stix_indicator.id, stix_file.id
                                ),
                            ...
                            bundle_objects.extend(
                                [stix_indicator, stix_file, stix_relationship]
                            )
...
if __name__ == "__main__":
    try:
        connector = GenericCSVHashImport()
        connector.run()
    except Exception as e:
