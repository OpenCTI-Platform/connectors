...
| MICROSOFT_DEFENDER_INCIDENTS_IMPORT_START_DATE | `string` |  | Format: [`date-time`](https://json-schema.org/understanding-json-schema/reference/string#built-in-formats) | `"P30D"` | Start date for importing incidents in ISO 8601 format (e.g. '2025-01-01T00:00:00Z' or a timedelta ie: 'P30D'). Used only on the first run; subsequent runs use the stored state. |
