...
class Indicator(_BaseIndicator):
    ...
    def _create_pattern(self, pattern_name: str | list[str]) -> Any:
...
class FileHash(_BaseIndicator):
    def __init__(
        ...
        name: list[str],
        c_type: str,
        tlp_color: str | None = "white",
        labels: list[str] | None = None,
        risk_score: str | int | None = None,
    ) -> None:
        super().__init__(name, c_type, tlp_color, labels, risk_score)

    def _create_pattern(self, pattern_name: str | list[str]) -> Any:
        return f"[file:hashes.'{self.determine_hash_algorithm_by_length(str(pattern_name))}' = '{pattern_name}']"
...
class IPAddress(_BaseIndicator):
    ...
    def _create_pattern(self, pattern_name: str | list[str]) -> Any:
        if self.is_ipv4(str(pattern_name)):
            return f"[ipv4-addr:value = '{pattern_name}']"
        elif self.is_ipv6(str(pattern_name)):
...
class URL(_BaseIndicator):
    ...
    def _create_pattern(self, pattern_name: str | list[str]) -> Any:
        pattern = f"[url:value = '{self.stix_escape(str(pattern_name))}']"
        return pattern
