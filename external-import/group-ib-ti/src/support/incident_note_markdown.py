...
def markdown_compromised_bank_card_group(
    *,
    ...
    nb.h2("Card Info")
    nb.kv("ID", item_id)
    nb.kv("Number", card_number)
    nb.kv("Type", card_type)
    nb.kv("Category", card_category)
    nb.kv("System", card_system)
    nb.kv("BIN", ", ".join(str(b) for b in card_bin) if card_bin else None)
    nb.kv("Issuer", card_issuer)
    nb.kv("Issuer country", card_issuer_country)
...
def markdown_compromised_access(
    *,
    ...
    acc_nb.h2("Target")
    acc_nb.kv("Host", target.get("host"))
    acc_nb.kv("Domain", target.get("domain"))
    acc_nb.kv("Provider", target.get("provider"))
    acc_nb.kv("Country", target.get("country"))
    acc_nb.kv("Device OS", target.get("device_os"))
    acc_nb.kv("Browser", target.get("device_browser"))
    ...
    *,
    ...
    if ta.get("name"):
        nb.h2("Attribution").kv(
            "Threat actor",
            f"{ta.get('name')} (id={ta.get('id') or '—'}, isAPT={ta.get('is_apt')})",
        )
    return nb.build()
...
def markdown_osi_git_repository(
    *,
    ...
    unk = "<unknown>"
    git_nb = MarkdownNote()
    git_nb.raw("## Git repository leak")
    git_nb.kv("ID", repo_id or unk)
    git_nb.kv("Name", name or unk)
    git_nb.kv("Source", payload.get("source") or unk)
    git_nb.kv("Date detected", date_detected or unk)
    git_nb.kv("Date created", date_created or unk)
    if files_list:
        frows: list[list[Any]] = []
...
def markdown_compromised_masked_card(
    *,
    ...
    mnb = MarkdownNote()
    mnb.raw("## Compromised masked card")
    mnb.kv("ID", item_id)
    mnb.kv("baseName", masked_card.get("baseName"))
    mnb.kv("isMasked", masked_card.get("isMasked"))
    mnb.kv("isDump", masked_card.get("isDump"))
    mnb.kv("isExpired", masked_card.get("isExpired"))
    mnb.h2("Card")
    mnb.kv("Number", card_number)
    mnb.kv("BIN", ", ".join(str(b) for b in card_bins) if card_bins else None)
    mnb.kv("System", card_system)
    mnb.kv("Type", card_type)
    mnb.kv("Issuer", card_issuer)
    mnb.kv("Issuer country", card_issuer_country_name or card_issuer_country_code)
    mnb.kv(
        "Valid thru",
    ...
    mnb.h2("Threat / Malware / Source")
    mnb.kv(
        "Malware",
        f"{mal_name or '—'} (id: {(malware_obj or {}).get('id') or '—'})",
    )
    actor_names_text = ", ".join(threat_actor_names) if threat_actor_names else None
    mnb.kv("Threat actors", actor_names_text)
    mnb.kv("Source type", source_type)
    mnb.kv("Source link", source_link)
    mnb.h2("Owner (as reported)")
    mnb.kv("Name", owner_obj.get("name"))
    mnb.kv("Phone", owner_obj.get("phone"))
    mnb.kv("Address", owner_obj.get("address"))
    mnb.kv("State", owner_obj.get("state"))
    mnb.kv("ZIP", owner_obj.get("zip"))
    mnb.kv("Country", owner_obj.get("country_code"))
