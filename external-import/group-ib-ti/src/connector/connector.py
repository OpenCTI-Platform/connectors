...
class ExternalImportConnector:
    def __init__(
        ...
        self.proxies = {
            "proxy_ip": getattr(self.cfg, "ti_api_proxy_ip", None),
            "proxy_port": getattr(self.cfg, "ti_api_proxy_port", None),
            "proxy_protocol": getattr(self.cfg, "ti_api_proxy_protocol", None),
            "proxy_username": getattr(self.cfg, "ti_api_proxy_username", None),
            "proxy_password": getattr(self.cfg, "ti_api_proxy_password", None),
        }
        ...
        self.ti_adapter = build_ti_adapter(
            ti_creds_dict={
                "api_key": getattr(self.cfg, "ti_api_token", None),
                "username": getattr(self.cfg, "ti_api_username", None),
            },
            proxies=self.proxies,
            config_obj=self.cfg,
            api_url=getattr(self.cfg, "ti_api_url", None),
        ...
        self.MITRE_MAPPER: dict[str, str] | None = None
        self.helper.connector_logger.info(
        ...
        for count, event in enumerate(parsed_portion, 1):
            ...
            try:
                bundle_objects = self._collect_intelligence(
                    ...
                    ttl=self.ttl,
                    event=event,
                    mitre_mapper=self.MITRE_MAPPER or {},
                    config=self.cfg,
