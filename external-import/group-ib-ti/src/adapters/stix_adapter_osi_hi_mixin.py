...
class OsiHiMixin(SdoMixin):
    ...
    def generate_hi_open_threats(
        ...
        country_codes = [
            ...
            if isinstance(c, dict) and (code := c.get("countryCode"))
        ]
        ...
        def _iter_scored(raw: Any, field_key: str) -> list[tuple[str, int | None]]:
            ...
            for entry in self._normalize_list(raw):
                if isinstance(entry, str):
                    ...
                    field_val = entry.get(field_key)
                    if isinstance(field_val, str) and field_val.strip():
                        out.append(
                            (field_val.strip(), _coerce_score(entry.get("riskScore")))
                        )
            return out

        ioc_objects: list[Any] = []
