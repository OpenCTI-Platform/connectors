...
class AdapterCoreMixin:
    ...
    def _log_skipped(
        ...
        """
        self.helper.connector_logger.info(
            f"{self.collection}: skipped {kind} ({reason}): {str(value)[:256]!r}"
        )
    ...
    def _retrieve_link(
        ...
        return portal_external_ref_rows(obj or [])

    def _retrieve_date(
        self, obj: dict[str, Any] | None, key: str, alter_key: str | None = None
    ) -> datetime:
        self.helper.connector_logger.debug(
            f"Retrieving date for key: {key}, alternate key: {alter_key}"
        )
        date_raw = (obj or {}).get(key, "")
        if not date_raw and alter_key:
            date_raw = (obj or {}).get(alter_key, "")
    ...
    def _retrieve_ttl_dates(
        ...
        ttl = obj.get("ttl") if obj else None
        if not ttl:
        ...
        date_modified_raw = (obj or {}).get("date-modified", "")
        date_created_raw = (obj or {}).get("date-created", "")
