...
from connector.settings import (
    ACTOR_PROFILE_COLLECTIONS,
    MALWARE_DESC_PLACEHOLDER,
    REPORT_NOTE_COLLECTIONS,
...
class SdoMixin(AdapterCoreMixin):
    ...
    def generate_stix_targeted_entities(
        ...
        if not obj:
            return []
        if self.collection not in REPORT_NOTE_COLLECTIONS:
            return []
    ...
    def generate_stix_attack_pattern(self, obj: dict[str, Any]) -> list[Any]:
        ...
        _events = obj.get("mitre_matrix_list") or []
    ...
    def generate_stix_threat_actor(
        ...
        if _threat_actor_name and len(_threat_actor_name) > 2:
            ...
            if self.collection in ACTOR_PROFILE_COLLECTIONS:
    ...
    def generate_stix_report(
        ...
        if self.collection in REPORT_NOTE_COLLECTIONS and _id:
