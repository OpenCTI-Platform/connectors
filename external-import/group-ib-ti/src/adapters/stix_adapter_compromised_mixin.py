...
class CompromisedMixin(SdoMixin):
    ...
    def generate_compromised_bank_card_group(
        ...
        if not created_time and events_table:
            ...
            known_event_dates = [d for d in event_dates if d is not None]
            if known_event_dates:
                created_time = min(known_event_dates)
        if not created_time:
            self.helper.connector_logger.error(
    ...
    def generate_compromised_spd(
        ...
        value_obj: dict[str, Any] = raw_value if isinstance(raw_value, dict) else {}
