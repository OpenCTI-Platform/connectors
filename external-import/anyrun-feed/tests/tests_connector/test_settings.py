...
def test_settings_should_migrate_deprecated_feed_fetch_interval():
    """
    Test that the deprecated `ANYRUN_FEED_FETCH_INTERVAL` field is migrated to
    ...
    assert settings.connector.duration_period == timedelta(minutes=120)
    warning_messages = [str(warning.message) for warning in w]
    assert any(
        "feed_fetch_interval" in msg.lower() or "interval" in msg.lower()
