...
import pytest
from connector import AnyrunFeed, ConnectorSettings
...
def test_opencti_connector_helper_is_instantiated(mock_opencti_connector_helper):
    ...
    assert helper.connect_name == "ANY.RUN TI Feed"
    assert helper.connect_scope == "stix2"
    assert helper.log_level == "ERROR"
    assert helper.connect_duration_period == "PT2H"
...
def test_connector_is_instantiated(mock_opencti_connector_helper):
    ...
    connector = AnyrunFeed(config=settings, helper=helper)
