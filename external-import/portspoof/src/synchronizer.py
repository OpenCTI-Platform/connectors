...
def _build_tlp_clear_marking() -> MarkingDefinition:
    """Build the OpenCTI ``TLP:CLEAR`` marking definition object.

    OpenCTI represents ``TLP:CLEAR`` as a custom statement-type marking that
    is distinct from the legacy ``stix2.TLP_WHITE`` constant. Generating it
    via ``pycti.MarkingDefinition.generate_id`` keeps the ID consistent with
    the platform so it is reused on every ingest.
    """
...
def build_config_from_env() -> Dict[str, Any]:
    ...
    log_level = (os.getenv("CONNECTOR_LOG_LEVEL") or "info").lower()
    ...
    return {
        ...
        "connector": {
            ...
            "name": os.getenv("CONNECTOR_NAME", "PortSpoofPro"),
            "scope": os.getenv(
                "CONNECTOR_SCOPE",
                "Threat-Actor,Observed-Data,IPv4-Addr,IPv6-Addr,Tool,"
                "Attack-Pattern,Report,Relationship,Sighting",
            ),
            "confidence_level": 85,
            "log_level": log_level,
...
class StixSynchronizer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Build the TLP:CLEAR marking object once and reuse it on every
        # bundle. We always include the MarkingDefinition object alongside
    ...
    def _create_ip_observables_and_relationships(
        ...
        if create_targets_relationships:
            ...
            target_ips_for_observables = capped_victims
        else:
            capped_victims = []
            capping_label = None
            # On incremental updates we still surface a small sample of
            # targets, but we do not create targets relationships for them.
            target_ips_for_observables = list(target_hosts[:3])

            logging.info(
                f"Incremental update ({event_type}): Skipping targets relationships"
            )

        # Build the target IP observables exactly once, then reuse the same
        # observable instances for everything else. Building two parallel
        # sets of observables for the same IPs would yield duplicate objects
        # with identical STIX IDs but conflicting labels/properties, which
        # makes ingest non-deterministic in OpenCTI.
        ...
        # Index the freshly built observables by IP so we can map back from
        # capped victim IPs to the matching observable when emitting targets
        # relationships, without rebuilding them.
        target_observables_by_ip = {obs.value: obs for obs in target_ip_observables}

        # ``create_network_traffic_objects`` only needs a small sample of
        # destinations so we keep the original 3-target cap there to avoid
        # blowing up the graph on heavy scans.
        network_traffic_objects = IpObservableManager.create_network_traffic_objects(
            source_ip_observable=source_ip_observable,
            target_ip_observables=target_ip_observables[:3],
            session_id=session_id,
            state=state,
            created_by_ref=self.author_standard_id,
            marking_refs=[self.tlp_clear_stix_id],
        ...
        if create_targets_relationships:
            ...
            for victim_ip in capped_victims:
                victim_observable = target_observables_by_ip.get(victim_ip)
                if victim_observable is None:
                    # Should not happen now that target_ip_observables covers
                    # the full capped_victims list, but stay defensive in case
                    # validation rejected an entry upstream.
                    continue
            ...
            logging.info(
                f"Created {len(target_ip_observables)} victim IP observables, "
...
This indicator represents confirmed malicious activity observed through direct interaction with deception infrastructure.""".strip()
    ...
    def _create_observed_data_and_sighting(
        ...
        if observed_data:
            ...
            sighting_first_seen = parse_iso_datetime(state["session_start_time"])
            sighting_last_seen = parse_iso_datetime(state["last_activity_time"])
            sighting_of_ref = indicator.id if indicator else threat_actor.id
            # Deterministic Sighting ID derived from sighting_of_ref +
            # where_sighted_refs + first/last_seen so re-processing the same
            # session updates the existing Sighting in OpenCTI instead of
            # creating a duplicate every time.
            sighting_id = StixSightingRelationship.generate_id(
                sighting_of_ref,
                self.author_standard_id,
                sighting_first_seen,
                sighting_last_seen,
            ...
            sighting = Sighting(
                ...
                object_marking_refs=[self.tlp_clear_stix_id],
                count=1,
                first_seen=sighting_first_seen,
                last_seen=sighting_last_seen,
    ...
    def _send_bundle(self, bundle_objects: List, work_id: str, session_id: str):
        ...
        # Prepend the actual TLP:CLEAR MarkingDefinition (not stix2.TLP_WHITE)
        # so every marking ref on downstream objects resolves to an object
        # that is present in this bundle.
        bundle_objects_with_refs = [
            author_identity,
            self.tlp_clear_marking,
        ] + bundle_objects
    ...
    def _handle_sync_error(self, work_id: str, session_id: str, error: Exception):
        ...
        except Exception as work_exc:  # noqa: BLE001 - best-effort cleanup path
            logging.debug(
                f"Could not mark work {work_id} as errored: "
                f"{type(work_exc).__name__}: {work_exc}"
            )
        logging.error(
            f"Failed to sync session {session_id}: {type(error).__name__}: {error}",
            exc_info=True,
