...
def _resolve_log_level() -> str:
    ...
    The OpenCTI deployment convention is ``CONNECTOR_LOG_LEVEL`` (documented
    in README and compose). ``LOG_LEVEL`` is kept as a legacy fallback so
    existing deployments are not broken. Values are normalised to upper case
    so that values like ``info`` (the OpenCTI default casing) resolve to a
    valid ``logging`` level.
    """
    raw = os.getenv("CONNECTOR_LOG_LEVEL") or os.getenv("LOG_LEVEL") or "INFO"
    return raw.upper()
...
# dead-lettered, so using it to gate basic_nack(requeue=True) retries would
# never increment and a persistently failing message would loop forever.
RETRY_COUNT_HEADER = "x-portspoof-retry-count"
...
class FullSessionState(BaseModel):
    ...
    def _validate_source_ip(cls, value: str) -> str:
        # Reject malformed addresses early so they never reach STIX indicator
        # patterns or IPv4/IPv6 observable construction downstream.
        try:
...
class OpenCTIConsumer:
    ...
    def on_message(self, ch, method, properties, body):
        ...
        # Use a custom header we control rather than ``x-death`` so the
        # counter survives ``basic_nack(requeue=True)``. ``x-death`` is only
        # mutated by RabbitMQ on actual dead-lettering, so it would never
        # increment for in-queue retries.
        delivery_count = 0
        headers = properties.headers or {}
        try:
            delivery_count = int(headers.get(RETRY_COUNT_HEADER, 0))
        ...
        try:
            ...
            if error_type == ErrorType.PERMANENT:
                ...
                if should_requeue:
                    ...
                    logging.warning(
                        f"Transient error processing message (attempt {next_attempt}/{MAX_RETRIES}): {type(e).__name__}: {e}"
                    )
                    try:
                        # Republish with an incremented retry counter so the
                        # next consumer instance can see how many times we
                        # have already attempted this message. We ack the
                        # original delivery so it does not get redelivered
                        # outside of our counter.
                        new_headers = dict(headers)
                        new_headers[RETRY_COUNT_HEADER] = next_attempt
                        ch.basic_publish(
                            exchange="",
                        ...
                        logging.warning(
                            f"Failed to requeue message for retry (channel likely closed): {nack_error}"
                        )
                        # Channel closed - will reconnect in outer loop
                else:
                    logging.error(
