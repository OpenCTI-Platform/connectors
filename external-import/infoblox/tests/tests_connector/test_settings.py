...
def test_settings_should_accept_valid_input(settings_dict):
    ...
    assert isinstance(settings.infoblox, BaseConfigModel) is True
...
def test_settings_should_migrate_deprecated_interval():
    """
    Test that the deprecated `INFOBLOX_INTERVAL` (hours) is automatically migrated
    ...
    assert settings.connector.duration_period == timedelta(hours=5)
