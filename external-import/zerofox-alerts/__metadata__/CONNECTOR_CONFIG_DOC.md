...
| ZEROFOX_ALERTS_API_BASE_URL | `string` |  | Format: [`uri`](https://json-schema.org/understanding-json-schema/reference/string#built-in-formats) | `"https://api.zerofox.com"` | Base URL of the ZeroFox API. |
| ZEROFOX_ALERTS_MARKING | `string` |  | `clear` `white` `green` `amber` `amber+strict` `red` | `"amber"` | TLP marking level applied to created STIX objects. |
