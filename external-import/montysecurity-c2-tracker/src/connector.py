            ...
            if mapping[m] == n:
                for t in tools:
                    if t["name"] == m:
                        ...
                        opencti_api_client.stix_core_relationship.add_label(
                            id=relationship["id"], label_id=label["id"]
                        )
...
def main():
    ...
    current_opencti_c2_tracker_indicators = opencti_indicator_loop(
        current_c2_tracker_ips, delete_old_iocs=False
    )
