...
class CofenseThreatHQClient:
    ...
    def _build_url(self, endpoint_rest:str, query_parameters:str) -> str:
        ...
        """
        url = f"{self.api_base_url}{endpoint_rest}" + (f"?{query_parameters}" if query_parameters else "")
        return url

    async def _request_data(self, request_method: str, endpoint_rest: str, query_parameters: str=None, expect_binary:bool=False):
        ...
        async def _retry_wrapped():
            async with ClientSession(
                ...
                async with session.request(method=request_method,url=url_built) as response:
                    if expect_binary:
    ...
    async def get_reports(self, next_position:str=None):
        ...
        if self.import_start_date and next_position is None:
            ...
            filters_query_parameter.append(
                f"timestamp={start_date_timestamp}"
            )
        ...
        query_parameters = "&".join(filters_query_parameter) if len(filters_query_parameter) > 0 else None
    ...
    async def get_report_pdf(self, threat_id) -> dict | None:
        ...
        if self.import_report_pdf:
            pdf_binary = await self._request_data(request_method, endpoint_rest, expect_binary=True)
