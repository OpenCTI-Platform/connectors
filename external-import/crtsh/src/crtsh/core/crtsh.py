...
class CrtSHClient:
    def __init__(self, domain, labels='crtsh', marking_refs="TLP:WHITE", is_expired=False, is_wildcard=False):
                ...
                for name in item.get('name_value').split('\n'):
                    try:
                        ...
                        if hasattr(stix_obj, 'id'):
                            relationship = self.stix_relationship(certificate_id, stix_obj.id)
                            if relationship:
