...
def client():
    ...
    return CrtSHClient(marking_refs=DEFAULT_MARKING_DEFINITION, labels=DEFAULT_LABEL, domain=DEFAULT_DOMAIN, is_expired=False, is_wildcard=False)

@pytest.fixture
def client_wildcard():
    """Create a CrtSHClient instance with default parameters."""
    return CrtSHClient(marking_refs=DEFAULT_MARKING_DEFINITION, labels=DEFAULT_LABEL, domain=DEFAULT_DOMAIN, is_expired=False, is_wildcard=True)

@pytest.fixture
def client_expired():
    """Create a CrtSHClient instance with default parameters."""
    return CrtSHClient(marking_refs=DEFAULT_MARKING_DEFINITION, labels=DEFAULT_LABEL, domain=DEFAULT_DOMAIN, is_expired=True, is_wildcard=False)

@pytest.fixture
def client_wildcard_expired():
    """Create a CrtSHClient instance with default parameters."""
    return CrtSHClient(marking_refs=DEFAULT_MARKING_DEFINITION, labels=DEFAULT_LABEL, domain=DEFAULT_DOMAIN, is_expired=True, is_wildcard=True)
...
class TestCrtSHClient:
    ...
    def test_invalid_domain(self):
        ...
        with pytest.raises(ValueError, match="Domain provided failed validation"):
            CrtSHClient(marking_refs=DEFAULT_MARKING_DEFINITION, labels=DEFAULT_LABEL, domain=INVALID_DOMAIN, is_expired=False, is_wildcard=False)
    ...
    def test_request_data_success_wildcard_expired(self, client_wildcard_expired: CrtSHClient, mocker):
        """Test successful fetching of data for valid dataset keys."""
        # Patch the _request_data method to return mock data. The mock will be applied
        # for all iterations in the loop.
    ...
    def test_get_response_wildcard(self, client_wildcard: CrtSHClient, mocker):
        """Test fetching data with an invalid dataset key."""
        FIXTURE_RESPONSE = load_fixture(f"{DEFAULT_DOMAIN}_wildcard.json")
        mocker.patch.object(client_wildcard, "_request_data", return_value=FIXTURE_RESPONSE)
    ...
    def test_get_response_expired(self, client_expired: CrtSHClient, mocker):
        """Test fetching data with an invalid dataset key."""
        FIXTURE_RESPONSE = load_fixture(f"{DEFAULT_DOMAIN}_expired.json")
        mocker.patch.object(client_expired, "_request_data", return_value=FIXTURE_RESPONSE)
        result = client_expired.get_response()
        assert result == FIXTURE_RESPONSE

    def test_get_response_wildcard_expired(self, client_wildcard_expired: CrtSHClient, mocker):
        """Test fetching data with an invalid dataset key."""
        FIXTURE_RESPONSE = load_fixture(f"{DEFAULT_DOMAIN}_wildcard_expired.json")
        mocker.patch.object(client_wildcard_expired, "_request_data", return_value=FIXTURE_RESPONSE)
    ...
    def test_process_certificate(self, client: CrtSHClient, mocker):
        ...
        assert stix_object[0].issuer == "C=US, O=DigiCert Inc, CN=DigiCert TLS RSA SHA256 2020 CA1"
        assert stix_object[0].validity_not_before == convert_to_datetime("2023-01-13T00:00:00")
        assert stix_object[0].validity_not_after == convert_to_datetime("2024-02-13T23:59:59")
    ...
    def test_stix_relationship(self, client: CrtSHClient):
        ...
        relationship = client.stix_relationship(DEFAULT_DOMAIN_STIX.id, DEFAULT_CERTIFICATE_STIX.id)
    ...
    def test_invalid_process_name_value(self, client: CrtSHClient, mocker):
        ...
        assert result is None
        assert len(stix_object) == 0
        result = client.process_name_value(None, stix_object, DEFAULT_CERTIFICATE_STIX.id)
        assert result is None
        assert len(stix_object) == 0
        result = client.process_name_value("", stix_object, DEFAULT_CERTIFICATE_STIX.id)
        assert result is None
        ...
        assert result is None
        assert len(stix_object) == 0
        result = client.process_name_value("test", stix_object, DEFAULT_CERTIFICATE_STIX.id)
        assert result is None
        assert len(stix_object) == 0
        result = client.process_name_value([], stix_object, DEFAULT_CERTIFICATE_STIX.id)
        assert result is None
        assert len(stix_object) == 0
        result = client.process_name_value(FIXTURE_RESPONSE[0], None, DEFAULT_CERTIFICATE_STIX.id)
        assert result is None
        result = client.process_name_value(FIXTURE_RESPONSE[0], "", DEFAULT_CERTIFICATE_STIX.id)
        assert result is None
        result = client.process_name_value(FIXTURE_RESPONSE[0], 0, DEFAULT_CERTIFICATE_STIX.id)
        assert result is None
        result = client.process_name_value(FIXTURE_RESPONSE[0], "test", DEFAULT_CERTIFICATE_STIX.id)
        assert result is None
        result = client.process_name_value(FIXTURE_RESPONSE[0], [], DEFAULT_CERTIFICATE_STIX.id)
        assert result is None
        result = client.process_name_value(FIXTURE_RESPONSE[0], {}, DEFAULT_CERTIFICATE_STIX.id)
    ...
    def test_get_stix_objects(self, client: CrtSHClient, mocker):
        ...
        for item in result:
            assert isinstance(item, X509Certificate) or isinstance(item, DomainName) or isinstance(item, EmailAddress) or isinstance(item, Relationship)
