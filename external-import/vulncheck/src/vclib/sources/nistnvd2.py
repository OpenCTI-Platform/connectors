...
def collect_nistnvd2(conn, config_state: dict) -> list:
    """Collect all data for nist-nvd2
    ...
    """
    conn.helper.connector_logger.info("[NIST NVD-2] Starting collection")
    entities = (
        conn.client.get_nistnvd2()
        if config_state is not None and data_source.NIST_NVD2 in config_state
        else conn.client.get_nistnvd2_from_backup()
    )
    stix_objects = []

    conn.helper.connector_logger.info("[NIST NVD-2] Parsing data into STIX objects")
    ...
    conn.helper.connector_logger.info("[NIST NVD-2] Data Source Completed!")
    return stix_objects
