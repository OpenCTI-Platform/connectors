...
if TYPE_CHECKING:
    ...
    from proofpoint_tap.settings import ConnectorSettings
...
class Connector:
    ...
    def __init__(
        ...
        self._report_processor = ReportProcessor(
            tlp_marking_name=self._config.proofpoint_tap.marking_definition,
        )
    ...
    def _initiate_work(self) -> None:
        ...
        if self._config.proofpoint_tap.export_campaigns:
            self._campaign_since_datetime = _set_acquisition_date(CAMPAIGN_STATE_KEY)

        if self._config.proofpoint_tap.export_events:
    ...
    def _process_events(self) -> bool:
        ...
        try:
            ...
            events: list[EventPort] = self._events.fetch(
                ...
                stop_time=datetime.now(timezone.utc),
                select=self._config.proofpoint_tap.events_type,
            )
            self._logger.info(
    ...
    def start(self) -> None:
        ...
        self._helper.schedule_iso(
            ...
            duration_period=self._config.connector.duration_period,  # type: ignore[arg-type]
        )
