...
|-----------------------------|----------------------------|----------|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `OBSTRACTS_BASE_URL`          | `obstracts.base_url`         | TRUE     | `'https://api.obstracts.com/'`        | Should always be `'https://api.obstracts.com/'`                                                                                                                                                                                                                           |
| `OBSTRACTS_API_KEY`          | `obstracts.api_key`         | TRUE     | n/a         | The API key used to authenticate to Obstracts Web                                                                                                                                                                                                                         |
| `OBSTRACTS_FEED_IDS`         | `obstracts.feed_ids`         | FALSE    | blank        | A list of comma separated feed IDs (e.g. `'feed1,feed2,feed3'`. You can get a feed ID in the Obstracts Web feed view. If left blank, all intelligence for feeds you are subscribed will be ingested (this includes feeds you subscribe to after the connector is configured). |
| `OBSTRACTS_INTERVAL_HOURS` | `obstracts.interval_hours`   | TRUE     | `12`          | How often (in hours) this Connector should poll Obstracts Web for updates.                                                                                                                                                         |
| `OBSTRACTS_DAYS_TO_BACKFILL`  | `obstracts.days_to_backfill` | TRUE     | `90`          | When a new feed is added, this setting determines the number of days to backfill. For example, setting `30` will ingest any posts from the blog within the last 30 days.
