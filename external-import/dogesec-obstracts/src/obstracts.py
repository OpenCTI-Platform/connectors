...
class ObstractsConnector:
    ...
    def get_posts_after_last(self, feed):
        ...
        feed_state = self._get_state()['feeds'].get(feed_id, dict(latest_post_update_time=(datetime.now(UTC)-timedelta(days=self.days_to_backfill)).isoformat()))
        feed_posts = self.retrieve(
            f'v1/feeds/{feed_id}/posts/', 'posts',
            params=dict(
    ...
    def process_post(self, feed_id, post: dict):
        ...
        try:
            objects = self.retrieve(f'v1/feeds/{feed_id}/posts/{post_id}/objects/', list_key='objects')
            bundle = dict(
                type='bundle',
                id=f"bundle--{post_id}",
                objects=objects,
    ...
    def retrieve(self, path, list_key, params:dict=None):
        ...
        self.helper.log_info(f'found {len(objects)} {list_key} at {path} with filters: {params}')
        return objects
    ...
    def _run_once(self):
        ...
        for feed in self.list_feeds():
            ...
            if feed_id not in (self.feed_ids or [feed_id]):
                self.helper.log_info(f'skipping feed with id (`{feed_id}`) not in config.obstracts.feed_ids')
                continue
