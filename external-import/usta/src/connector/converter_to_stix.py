"""
USTA Prodaft STIX 2.1 Converter.
...
class ConverterToStix:
    """
    Converts USTA Prodaft IOC records and ticket data to STIX 2.1 bundles.
    """
    ...
    def __init__(
        ...
        self.author = stix2.Identity(
            ...
            description=(
                "USTA Threat Intelligence Platform by Prodaft. "
    ...
    def _parse_datetime(value: str | None) -> str:
        """Safely parse an ISO datetime string; return current UTC if None."""
        if not value:
            return datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.000Z")
        # Ensure trailing Z for UTC consistency
    ...
    def _create_file_observable(
        ...
        if not hashes:
            raise ValueError("At least one hash value is required to create a File SCO.")
    ...
    def convert_malicious_url(self, record: dict[str, Any]) -> list:
        """
        ...
        indicator_name = f"{url_value}"
        description_parts = [f"Malicious URL indicator reported by USTA Prodaft.\n"]
        if tags:
        ...
        for obs in observable_objects:
            rel = self._make_relationship("based-on", indicator.id, obs.id,
                                          start_time=valid_from, stop_time=valid_until)
        ...
        for tag in tags:
            ...
            rel = self._make_relationship("indicates", indicator.id, malware_sdo.id,
                                          start_time=valid_from, stop_time=valid_until)
            stix_objects.append(rel)

        return stix_objects

    def convert_phishing_site(self, record: dict[str, Any]) -> list:
        """
        ...
        indicator = stix2.Indicator(
            id=PyctiIndicator.generate_id(pattern),
            name=indicator_name,
            description="Phishing site identified by USTA Prodaft threat intelligence.",
            pattern=pattern,
            pattern_type="stix",
            valid_from=created,
            labels=["phishing"],
    ...
    def convert_malware_hash(self, record: dict[str, Any]) -> list:
        """
        ...
        short_hash = display_hash[:16] + "..." if len(display_hash) > 16 else display_hash
        indicator_name = f"{short_hash}"
        if tags:
            indicator_name = f"{tags[0]} - {short_hash}"

        description_parts = ["Malware hash indicator reported by USTA Prodaft."]
        if tags:
        ...
        rel = self._make_relationship("based-on", indicator.id, file_obs.id,
                                      start_time=valid_from, stop_time=valid_until)
        stix_objects.append(rel)
    ...
    def _create_user_account_observable(
        ...
        deterministic UUIDv5 from the account_login to ensure deduplication.
        """
        deterministic_id = "user-account--" + str(
            uuid.uuid5(uuid.UUID("00abedb4-aa42-466c-9c01-fed23315a9b7"), account_login)
        )
    ...
    def convert_compromised_credential(self, record: dict[str, Any]) -> list:
        """
        ...
        STIX mapping:
          ...
          - Indicator SDO       (pattern on user-account)
          - Note SDO            (victim telemetry & password complexity — no raw password)
          - Relationships       (based-on, indicates)
        ...
        if target_url:
            ...
            if hostname and not self._is_ip(hostname):
                ...
                observable_refs.append(domain_obs)
        ...
        if victim_detail:
            ...
            if victim_ip and self._is_ip(victim_ip):
                ...
                observable_refs.append(ip_obs)

            malware_name = victim_detail.get("malware")

        # --- Indicator SDO ---
        pattern = f"[user-account:account_login = '{username}']"
        indicator_name = f"{username}"

        description_parts = [
            f"Compromised credential detected by USTA Prodaft (Ticket #{ticket_id}).",
            f"Source: {source}.",
            f"Company: {company_name}.",
        ]
        if is_corporate:
            description_parts.append("Flagged as corporate credential.")
        if password_complexity:
            score = password_complexity.get("score", "unknown")
            length = password_complexity.get("length", "?")
            description_parts.append(
                f"Password strength: {score} (length: {length})."
            )

        labels = ["compromised-credentials", source.replace("_", "-")]
        if is_corporate:
        ...
        indicator = stix2.Indicator(
            ...
            custom_properties={
                "x_opencti_score": self.confidence,
                "x_opencti_main_observable_type": "User-Account",
            },
        )
        stix_objects.append(indicator)

        # --- Relationships: indicator → observables ---
        for obs in observable_refs:
            rel = self._make_relationship("based-on", indicator.id, obs.id)
            stix_objects.append(rel)
        ...
        if malware_name:
            ...
            rel = self._make_relationship(
                "indicates", indicator.id, malware_sdo.id
            )
            stix_objects.append(rel)
        ...
        if victim_detail:
            ...
            note_abstract = (
                f"Victim telemetry for compromised credential: {username}"
            )

            note = stix2.Note(
                id=PyctiNote.generate_id(
                    created, note_content_parts[0]
                ),
                ...
                created_by_ref=self.author.id,
                object_marking_refs=[self.tlp_marking.id],
                object_refs=[indicator.id, user_account.id],
            )
    ...
    def convert_credit_card_ticket(self, record: dict[str, Any]) -> list:
        """
        ...
        incident_description = (
            f"Credit card fraud ticket reported by USTA Prodaft.\n"
