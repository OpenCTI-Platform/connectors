"""Unit tests for the USTA Prodaft API client — 100 % coverage."""
...
class TestRequest:
    def test_success_absolute_url(self, client):
        ...
        client.session.get.assert_called_once_with("https://other.com/api", params={"a": 1}, timeout=60)
...
class TestCursorPagination:
    def test_single_page(self, client):
        client._request = MagicMock(return_value={
            "results": [{"id": 1}], "next": None, "cursor": "c1"
        })
...
class TestPagePagination:
    ...
    def test_empty(self, client):
        client._request = MagicMock(return_value={"count": 0, "results": [], "next": None})
        assert list(client._fetch_page_paginated("/ep")) == []
...
class TestPublicMethods:
    def _mock_pagination(self, client, method_name):
        ...
        results = [[{"id": 1}]]
        if "cursor" in method_name or method_name in ("get_malicious_urls", "get_malware_hashes"):
