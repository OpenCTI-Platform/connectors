...
class DepConnector:
    ...
    def _send_objects(self, objects: list[stix2._STIXBase21]) -> None:
        if not objects:
            return
        # Every emitted SDO / SRO references ``TLP_AMBER`` via
        # ``object_marking_refs``; with
        # ``cleanup_inconsistent_bundle=True`` enabled the ingestion
        # worker would otherwise drop those references because the
        # marking-definition SDO is not present in the bundle, leaving
        # every object effectively unmarked. Prepend the marking SDO
        # to the deduped object list so the bundle is self-contained
        # and the cleanup pass has nothing to strip.
        #
        # ``update=True`` used to be passed here but the kwarg was
        # removed from the helper's public contract (see the
        # connector-linter ``VC506`` rule); the platform now applies
        # its default update behaviour and a stray ``update=`` kwarg
        # can trigger a ``TypeError`` on newer pycti releases.
        deduped: dict[str, stix2._STIXBase21] = {
            TLP_AMBER.id: TLP_AMBER,
        }
        for obj in objects:
            obj_id = getattr(obj, "id", None)
            if obj_id is None or obj_id in deduped:
                continue
            deduped[obj_id] = obj
    ...
    def _run_cycle(self) -> None:
        ...
        # Track the per-cycle outcome so the Work record is closed with
        # ``in_error=True`` whenever anything went wrong — a fetch
        # failure (no items reached the per-item loop) or any number of
        # per-item processing errors (a subset of the API payload
        # failed to convert). Without this the OpenCTI UI shows the
        # Work as a clean success even when the run silently dropped
        # data, which masks recurring API outages and conversion
        # regressions in production.
        cycle_message = f"DEP connector run completed, last_run: {end.isoformat()}"
        cycle_in_error = False
        try:
            try:
                ...
                cycle_in_error = True
                cycle_message = f"DEP connector fetch failed: {error}"
                return
            ...
            if item_error_count > 0:
                cycle_in_error = True
                cycle_message = (
                    f"DEP connector run completed with {item_error_count} "
                    f"per-item failures, last_run: {end.isoformat()}"
                )
            ...
            # Anything not caught by the inner handlers above
            # (state-write failures, unexpected helper crashes, …) still
            # has to close the Work as in-error rather than leaving it
            # stuck in the running state in the OpenCTI UI.
            self.helper.log_error(f"DEP connector run failed: {error}")
            cycle_in_error = True
            cycle_message = f"DEP connector run failed: {error}"
            raise
        finally:
            self.helper.api.work.to_processed(
                self._current_work_id,
                cycle_message,
                in_error=cycle_in_error,
            )
