...
class StixBuilder:
    ...
    def create_report(self, item: LeakRecord, object_refs: list[str]) -> stix2.Report:
        ...
        # Derive the Report id deterministically from the DEP
        # ``hashid`` (same UUIDv5 namespace as ``create_incident``)
        # instead of ``pycti.Report.generate_id(name, published)``.
        # The latter keys the id on ``name`` (built from
        # ``item.victim or item.victim_domain or "Unknown Victim"``)
        # and ``published`` — so a later DEP update that fills in a
        # previously-unknown victim, or any other change to the
        # display name, would produce a brand new Report SDO rather
        # than upserting the existing announcement. Hash-derived
        # UUIDv5 matches the contract documented in the README
        # ("deterministic primary object IDs based on the DEP
        # ``hashid`` so updates merge cleanly") and gives the report
        # path the same merge-on-update behaviour the incident path
        # already had.
        report_id = f"report--{uuid5(NAMESPACE_URL, f'dep-announcement:{item.normalized_hashid}')}"
        # ``id=`` must appear as an explicit kwarg on every ``stix2.<SDO>``
        # constructor — the repo's custom pylint plugin
        # (``linter_stix_id_generator``) statically AST-walks each call
        # and flags ``W9101 (generated-id-stix)`` when no literal
        # ``id=`` keyword is present, regardless of whether one would
        # be unpacked at runtime via ``**kwargs``. Keep the two
        # branches separate so both shapes pass the lint.
