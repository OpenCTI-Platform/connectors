...
from connector.converter_to_stix import EventConverter
...
class Misp:
    def __init__(self):
        ...
        self.converter = EventConverter(
            ...
            get_author_from_tags=self.config.misp.author_from_tags,
            get_markings_from_tags=self.config.misp.markings_from_tags,
            get_labels_from_tags=self.config.misp.create_tags_as_labels,
            get_associated_files_from_attributes=self.config.misp.import_with_attachments,
            get_observables_from_objects=self.config.misp.create_object_observables,
            ...
            convert_unsupported_objects_to_custom_observables=self.config.misp.import_unsupported_observables_as_text,
            propagate_labels=self.config.misp.propagate_labels,
    ...
    def process_events(self, work_id, events: list[EventRestSearchListItem]):
        ...
        for event in events:
            ...
            if (
                ...
                url = (
                    self.config.misp.reference_url + "/events/view/" + event.Event.uuid
                )
            else:
            ...
            if self.config.misp.create_reports:
                ...
                for note in event.Event.EventReport or []:
                    note = stix2.Note(
                        id=Note.generate_id(
    ...
    def _get_pdf_file(self, attribute: ExtendedAttributeItem):
    ...
    def prepare_elements(
        self, galaxies: list[GalaxyItem], tags: list[TagItem], author, markings
    ):
        elements = {
            "intrusion_sets": [],
            "malwares": [],
        ...
        for tag in tags:
            ...
            if (
                ...
                or tag.name.startswith("misp-galaxy:mitre-mobile-attack-intrusion-set")
    ...
    def resolve_markings(self, tags: list[TagItem], with_default=True):
        markings = []
        for tag in tags:
            tag_name = tag.name
    ...
    def resolve_tags(self, tags: list[TagItem]):
        opencti_tags = []
