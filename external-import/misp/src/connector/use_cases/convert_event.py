...
from .common import ConverterConfig
...
class EventConverter:
    ...
    def __init__(
        ...
        self.config = ConverterConfig(
            ...
            get_author_from_tags=get_author_from_tags,
            get_markings_from_tags=get_markings_from_tags,
            get_labels_from_tags=get_labels_from_tags,
            get_associated_files_from_attributes=get_associated_files_from_attributes,
            get_observables_from_objects=get_observables_from_objects,
            ...
            convert_unsupported_objects_to_custom_observables=convert_unsupported_objects_to_custom_observables,
        ...
        if self.config.guess_threats_from_tags:
            self.threats_guesser = ThreatsGuesser(self.helper.api)
    ...
    def create_report(
        ...
        published_at = datetime.fromisoformat(event.Event.date).astimezone(
            tz=timezone.utc
        )
    ...
    def process(self, event: EventRestSearchListItem) -> list[stix2.v21._STIXBase21]:
        ...
        """
        # Extract STIX report's metadata from MISP event's tags
        author = None
        labels = []
        markings = []

        for tag in event.Event.Tag or []:
            if self.config.get_author_from_tags:
            ...
            markings.append(self.tag_converter.create_marking(tag) or TLP_CLEAR)
        ...
        # Extract STIX report's metadata from MISP event's attributes
        external_references = []
        associated_files = []
        ...
        for attribute in event.Event.Attribute or []:
            ...
            if self.config.get_associated_files_from_attributes:
                associated_file = self.attribute_converter.create_associated_file(
                    attribute
                )
                if associated_file:
                    associated_files.append(associated_file)
        # TODO: to keep or to remove ? check if breaking changes
        ...
        # Extract STIX SCOs, SDOs, SROs from MISP event's galaxies, tags, attributes and objects' attributes
        stix_objects = []
        stix_relationships = []

        for galaxy in event.Event.Galaxy or []:
            galaxy_stix_objects, galaxy_relationship_objects = (
                self.galaxy_converter.process(galaxy, author=author, markings=markings)
            )
            stix_objects.extend(galaxy_stix_objects)
            stix_relationships.extend(galaxy_relationship_objects)
        ...
        threat_level = (
            int(event.Event.threat_level_id) if event.Event.threat_level_id else None
        )
        for attribute in event.Event.Attribute or []:
        ...
        for object in event.Event.Object or []:
            for attribute in object.Attribute or []:
                ...
                stix_objects.extend(attribute_stix_objects)
                stix_relationships.extend(attribute_stix_relationships)
                # TODO: create relationship between attribute indicator/observable and object observable
                # TODO: create relationships between event indicator/observable and threats
        ...
        for stix_relationship in stix_relationships:
            ...
            # TODO: to remove ? what is the purpose of linking relationships ?
            if (
                stix_relationship["source_ref"] + stix_relationship["target_ref"]
