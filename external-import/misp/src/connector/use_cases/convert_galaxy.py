...
class GalaxyConverter:
    ...
    def create_relationships(
        ...
        author: stix2.Identity,
        markings=list[stix2.MarkingDefinition],
    ) -> list[stix2.v21._RelationshipObject]:
        stix_relationships = []
    ...
    def process(
        ...
        if galaxy.namespace == "mitre-attack" and galaxy.name == "Tool":
            for galaxy_entity in getattr(galaxy, "GalaxyCluster", []):
                tool = self.create_tool(galaxy_entity, author=author, markings=markings)
                if tool:
        ...
        stix_relationships.extend(
            self.create_relationships(stix_objects, author=author, markings=markings)
        )
