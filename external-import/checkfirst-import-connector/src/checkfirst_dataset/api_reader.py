...
def _normalize_api_field(data: dict[str, Any], field_name: str) -> str:
    """Normalize a field value from API data.

    Supports different field name formats (camelCase, snake_case, Title Case).
    """
    # Try different field name variations
    variations = [
        field_name,  # As-is
        field_name.replace(" ", "_").lower(),  # snake_case
        field_name.replace(" ", ""),  # Remove spaces
        "".join(word.capitalize() for word in field_name.split()),  # PascalCase
    ]
