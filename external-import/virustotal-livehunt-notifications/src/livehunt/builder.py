...
class LivehuntBuilder:
    ...
    def __init__(
        ...
        *,
        ...
        self._default_bundle = [author.to_stix2_object(), tlp_marking.to_stix2_object()]
    ...
    def create_alert(self, vtobj, external_reference) -> Optional[Incident]:
        ...
        # Extract notification date and convert to timezone-aware datetime
        # This ensures deterministic ID generation via the SDK's computed 'id' field
        notification_date = vtobj._context_attributes["notification_date"]
        created_dt = datetime.datetime.fromtimestamp(
            notification_date, tz=datetime.timezone.utc
        )

        # Create the incident with explicit 'created' timestamp for deterministic ID
        incident = Incident(
            incident_type="alert",
            name=name,
            created=created_dt,
            description=f"Date of the alert on VirusTotal: {created_dt}",
            source=self._SOURCE,
            author=self.author,
        ...
        # Get the deterministic ID generated by the SDK
        incident_id = incident.id

        # Check if alert already exists in OpenCTI
