...
| TLP marking      | matrix.tlp          | `MATRIX_TLP`                | `AMBER`                    | No        | Marking applied to created entities. One of `CLEAR` (modern OpenCTI marking, `x_opencti_definition='TLP:CLEAR'`), `WHITE` (legacy `stix2.TLP_WHITE` constant — distinct marking-definition object from `CLEAR`, kept for backwards compatibility), `GREEN`, `AMBER`, `AMBER_STRICT` (also accepted as `AMBER+STRICT`, `AMBER STRICT`, `AMBER-STRICT`), `RED`. Case-insensitive.                |
