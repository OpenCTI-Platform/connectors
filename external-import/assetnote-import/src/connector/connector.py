...
class AssetnoteImportConnector:
    ...
    def _resolve_status_mapping(self) -> dict[str, str]:
        ...
        configured_statuses = {
            state: name for state, name in status_template_names.items() if name
        }
        ...
        status_id_by_name = {
            status["template"]["name"]: status["id"] for status in statuses
        }

        # If any configured status name isn't attached to the workflow, raise a ValueError
        missing = [
            name
            for name in configured_statuses.values()
            if name not in status_id_by_name
        ]
        if missing:
            raise ValueError(
                f"Case-Incident workflow status template(s) not found: {missing}. Please see the documentation for details on how to attach each to the Case Incident Workflow"
            )
        # Return each configured triage state mapped to its resolved status id
        return {
            state: status_id_by_name[name]
            for state, name in configured_statuses.items()
        }
    ...
    def process_message(self) -> None:
        ...
        try:
            ...
            if current_state is not None and "last_run" in current_state:
                ...
                self.helper.connector_logger.info(f"Connector last run: {since}")
            # If the connector has never run, we should retrieve from the 'first run' timestamp specified in configs
                ...
                since = (
                    self.config.assetnote_import.first_run_retrieval_datetime.strftime(
                        "%Y-%m-%dT%H:%M:%SZ"
                    )
                )
            ...
            if assets_complete and exposures_complete:
                ...
                self.helper.connector_logger.warning(
                    "[CONNECTOR] last_run not update due to error in consumption. Next run will compensate for this"
                )
            self.helper.connector_logger.info(
                f"[CONNECTOR] Imported {asset_count} assets and {exposure_count} exposures"
            )
    ...
    def _process_pages(
        ...
        since: str,
        convert_item: Callable[[dict], list],
        item_name: str,
    ) -> tuple[int, bool]:
        total = 0
        succeeded = True
        ...
        while True:
            self.helper.connector_logger.info(
                f"[CONNECTOR] Fetching {item_name} page {page}"
            )
