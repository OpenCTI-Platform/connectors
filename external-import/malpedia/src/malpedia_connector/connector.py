...
class MalpediaConnector:
    ...
    def __init__(
        ...
        self.config = config
        self.helper = helper
        self.api_client = MalpediaClient(self.helper, self.config.malpedia.auth_key)
        ...
        self.default_marking = getattr(self.config.malpedia, "default_marking", None)
    ...
    def _process_families(self) -> None:
        ...
        for family_name in families_json:
            try:
                ...
                continue
            self.helper.connector_logger.info(
                "[MALWARE] Generate Malware...", {"family_name": family_name}
            )
            malware_id = self._generate_malware_by_family(data_family)
            if not self.config.malpedia.import_yara:
                self.helper.connector_logger.info(
                    "[CONFIGURATION] Due to the configuration, if import yara is false, we skip creating for the yara indicator.",
                    {"family_name": family_name},
                )
            else:
            ...
            if not self.api_client.unauthenticated:
                if not self.config.malpedia.create_indicators and (
                    not self.config.malpedia.create_observables
                ):
                    self.helper.connector_logger.info(
                        "[CONFIGURATION] Due to the configuration, if create indicator and observable are false, we skip creating them for the samples.",
                        {"family_name": family_name},
                    )
                else:
                ...
                self.helper.connector_logger.info(
                    "[AUTHENTICATE] You are an unauthenticated user, you do not have access to retrieve information about the samples, the creation of the samples will be skipped.",
                    {"family_name": family_name},
                )
            if not self.config.malpedia.import_intrusion_sets:
                self.helper.connector_logger.info(
                    "[CONFIGURATION] Due to the configuration, if import intrusion set is false, we skip creating them for the intrusion set.",
                    {"family_name": family_name},
                )
    ...
    def _generate_samples_associated_with_malware(
        ...
        for sample in samples:
            if "sha256" in sample and sample["sha256"] is not None:
                ...
                if self.config.malpedia.create_observables:
                    self.helper.connector_logger.info(
                    ...
                    self.helper.connector_logger.info(
                        "[CONFIGURATION] Due to the configuration, if create observable is false, we skip creating them for the sample.",
                        {"sample_sha256": sample["sha256"]},
                    )
                stix_indicator_id = None
                if self.config.malpedia.create_indicators:
                    self.helper.connector_logger.info(
                    ...
                    self.helper.connector_logger.info(
                        "[CONFIGURATION] Due to the configuration, if create indicator is false, we skip creating them for the sample.",
                        {"sample_sha256": sample["sha256"]},
                    )
                if (
                    self.config.malpedia.create_indicators
                    and self.config.malpedia.create_observables
                ):
                ...
                self.helper.connector_logger.error(
                    "[ERROR-SAMPLE] Error sample data.", {"sample": sample}
                )
                self.helper.metric.inc("error_count")
                continue
    ...
    def _generate_intrusion_set_associated_with_malware(
        ...
        for actor in data_family.attribution:
            ...
            if actor_json["value"] == "" or actor_json["value"] is None:
                ...
                continue
            if self.config.malpedia.import_intrusion_sets:
                try:
                    if (
