...
| Report Type   | cyber_monitor.report_type   | `CYBER_MONITOR_REPORT_TYPE`    |         | No        | Optional STIX `report_types` value(s) attached to imported reports. Accepts a single string (`threat-report`), a comma-separated string (`threat-report,campaign`) or a YAML list (`['threat-report', 'campaign']`). When unset, no report type is emitted. |
| Report Status | cyber_monitor.report_status | `CYBER_MONITOR_REPORT_STATUS`  |         | No        | Optional workflow status mapped to `x_opencti_report_status`. Accepts a default workflow name (`New`, `In Progress`, `Analyzed`, `Closed`) or an integer position. When unset, no status is emitted. |
