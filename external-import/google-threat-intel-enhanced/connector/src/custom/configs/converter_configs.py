...
from connector.src.custom.mappers.gti_reports.gti_campaign_to_stix_campaign import (
...
def set_report_context(report_obj: Any) -> None:
    ...
    report_id = getattr(report_obj, "id", "unknown")
    report_type = getattr(report_obj, "type", "unknown")
    current_refs = len(getattr(report_obj, "object_refs", []) or [])
    logger.info(
        f"{LOG_PREFIX} [Context] SET report context: id={report_id}, type={report_type}, refs={current_refs}"
    )
...
def clear_report_context() -> None:
    ...
    global _current_report_context
    if _current_report_context:
        report_id = getattr(_current_report_context, "id", "unknown")
        final_refs = len(getattr(_current_report_context, "object_refs", []) or [])
        logger = logging.getLogger(__name__)
        logger.info(
            f"{LOG_PREFIX} [Context] CLEAR report context: id={report_id}, final refs={final_refs}"
        )
    _current_report_context = None


def create_report_context_postprocessor() -> Any:
    """Create a postprocessor that sets the report context when converting reports.

    This enables subsequent subentity conversions to add their object IDs to
    the report's object_refs.
    ...
    def postprocess_set_report_context(stix_output: Any) -> Any:
        """Find the report in output and set it as context."""
        logger = logging.getLogger(__name__)

        # Search for report in output
        ...
        if report:
            set_report_context(report)
            logger.info(
                f"{LOG_PREFIX} [Report Postprocessor] Set report context from converted output: "
                f"id={getattr(report, 'id', 'unknown')}"
            )
        else:
            logger.warning(
                f"{LOG_PREFIX} [Report Postprocessor] No report found in conversion output"
            )

        return stix_output

    return postprocess_set_report_context
...
def create_report_linking_postprocessor() -> Any:
    ...
    def postprocess_for_report_linking(stix_output: Any) -> Any:
        ...
        if not parent_report:
            # No report context - this is normal when not processing report subentities
            return stix_output
...
GTI_REPORT_CONVERTER_CONFIG = GenericConverterConfig(
    ...
    validate_input=True,
    to_stix=False,  # Keep as Pydantic models so object_refs can be modified later
    postprocessing_function=create_report_context_postprocessor(),
)
...
GTI_CAMPAIGN_CONVERTER_CONFIG = GenericConverterConfig(
    ...
    mapper_class=GTICampaignToSTIXCampaign,
...
CONVERTER_CONFIGS = {
    ...
    "software_toolkits": GTI_SOFTWARE_TOOLKIT_CONVERTER_CONFIG,
    ...
    "campaigns": GTI_CAMPAIGN_CONVERTER_CONFIG,
}
