...
class ClientAPI:
    ...
    def _create_fetcher_factory(self) -> GenericFetcherFactory:
        ...
        base_headers = {
            ...
            "x-tool": getattr(self.config, "x_tool", "OpenCTI.GTIConnector.v1.0"),
    ...
    def _extract_response_data(self, response: Any) -> tuple[Any, Any]:
        ...
        # Handle Pydantic models with data attribute
    ...
    def _build_campaign_filter_configurations(
        self, initial_state: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        """Build filter configurations for campaigns based on config settings.
        ...
        try:
            ...
            if "All" in campaign_origins:
                campaign_origins = ["All"]

            filter_configs = []

            for origin in campaign_origins:
                current_filter = base_filters
                if origin != "All":
                    current_filter += f" origin:'{origin}'"
                ...
                if origin == "All":
                    ...
                    description = f"campaigns with origin={origin}"
                ...
                self.logger.info(
                    f"{LOG_PREFIX} Configured fetching {description} (from {start_date})"
                )
            ...
            self.logger.error(
                f"{LOG_PREFIX} Failed to build campaign filter configurations: {str(e)}"
            )
            return [
                {
                    "params": {
    ...
    async def fetch_campaigns(
        ...
        """
        filter_configs = self._build_campaign_filter_configurations(initial_state)
        campaign_fetcher = self.fetcher_factory.create_fetcher_by_name(
            "campaigns", base_url=self.config.api_url
        )
        ...
        for filter_config in filter_configs:
            ...
            self.logger.info(
                f"{LOG_PREFIX} Fetching campaigns from endpoint 'campaigns' with filters: {endpoint_params}"
            )
    ...
    async def fetch_campaign_subentities(self, campaign_id: str) -> Dict[str, List[str]]:
        ...
        Returns:
            Dict[str, List[str]]: Dictionary with entity type keys and lists of IDs as values.
                Includes: 'threat_actors', 'malware_families', 'attack_techniques',
                'vulnerabilities', 'software_toolkits', 'domains', 'files', 'urls', 'ip_addresses'.
        ...
        subentities_ids: Dict[str, List[str]] = {}
        ...
        try:
            for subentity_type in subentity_types:
                ...
                # Use report_id param as that's what the relationships fetcher expects
                params = {"report_id": campaign_id, "entity_type": subentity_type}

                try:
                    async for page_data in self._paginate_with_cursor(
                        relationships_fetcher, params, f"campaign {subentity_type}"
                    ):
                    ...
                    self.logger.debug(
                        f"{LOG_PREFIX} Error fetching campaign {subentity_type}: {str(e)}"
                    )

                if all_ids:
                    self.logger.info(
                        f"{LOG_PREFIX} Retrieved {len(all_ids)} {subentity_type} for campaign {campaign_id}"
                    )
            ...
            self.logger.error(
                f"{LOG_PREFIX} Failed to gather subentities for campaign {campaign_id}: {str(e)}"
            )
            return {}

    async def fetch_campaign_subentity_details(
        self, subentity_ids: Dict[str, List[str]]
    ) -> Dict[str, List[Any]]:
        """Fetch detailed data for all campaign subentities including threat actors, malware, TTPs, vulnerabilities, tools, and IOCs.

        Args:
            subentity_ids: Dictionary mapping entity types to lists of IDs.
                Supported types: 'threat_actors', 'malware_families', 'attack_techniques',
                'vulnerabilities', 'software_toolkits', 'domains', 'files', 'urls', 'ip_addresses'.

        Returns:
            Dictionary mapping entity types to lists of fetched entity details

        """
        subentity_details: Dict[str, List[Any]] = {}
        ...
        for entity_type, ids in subentity_ids.items():
            ...
            try:
                ...
                for entity_id in ids:
                    try:
                        result = await fetcher.fetch_single(entity_id=entity_id)
                        if result:
                            details.append(result)
                    except Exception as e:
                        self.logger.debug(
                            f"{LOG_PREFIX} Failed to fetch {entity_type} detail {entity_id}: {str(e)}"
                        )

                if details:
                    subentity_details[entity_type] = details
                    if len(details) < len(ids):
                        self.logger.warning(
                            f"{LOG_PREFIX} Fetched {len(details)}/{len(ids)} {entity_type} details "
                            f"(some entities may be missing or inaccessible)"
                        )
                    else:
                        self.logger.info(
                            f"{LOG_PREFIX} Fetched {len(details)}/{len(ids)} {entity_type} details"
                    ...
                    self.logger.warning(
                        f"{LOG_PREFIX} Failed to fetch any {entity_type} details from {len(ids)} IDs"
                    )
        ...
        return subentity_details

    def _build_threat_actor_filter_configurations(
        self, initial_state: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        """Build filter configurations for standalone threat actor imports.
        ...
        try:
            ...
            for origin in threat_actor_origins:
                current_filter = base_filters
                if origin != "All":
                    current_filter += f" origin:'{origin}'"

                if origin == "All":
                    description = "all threat actors"
                else:
                    description = f"threat actors with origin={origin}"
    ...
    async def fetch_threat_actors(
        ...
        """Fetch threat actors from the API for standalone import.
        ...
        threat_actor_fetcher = self.fetcher_factory.create_fetcher_by_name(
            "threat_actors_list", base_url=self.config.api_url
        )
        ...
        for filter_config in filter_configs:
            ...
            self.logger.info(
                f"{LOG_PREFIX} Fetching threat actors with filters: {endpoint_params}"
            )
    ...
    def _build_malware_filter_configurations(
        self, initial_state: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        """Build filter configurations for standalone malware family imports.
        ...
        try:
            ...
            if "All" in malware_origins:
                malware_origins = ["All"]

            filter_configs = []

            for origin in malware_origins:
                current_filter = base_filters
                if origin != "All":
                    current_filter += f" origin:'{origin}'"

                if origin == "All":
                    description = "all malware families"
                else:
                    description = f"malware families with origin={origin}"
            ...
            self.logger.error(
                f"{LOG_PREFIX} Failed to build malware filter configurations: {str(e)}"
            )
            return [
                {
                    "params": {
    ...
    async def fetch_malware_families(
        ...
        """Fetch malware families from the API for standalone import.
        ...
        """
        filter_configs = self._build_malware_filter_configurations(initial_state)
        malware_fetcher = self.fetcher_factory.create_fetcher_by_name(
            "malware_families_list", base_url=self.config.api_url
        )
        ...
        for filter_config in filter_configs:
            ...
            self.logger.info(
                f"{LOG_PREFIX} Fetching malware families with filters: {endpoint_params}"
            )
    ...
    def _build_vulnerability_filter_configurations(
        self, initial_state: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
        """Build filter configurations for standalone vulnerability imports.
        ...
        try:
            ...
            for origin in vulnerability_origins:
                current_filter = base_filters
                if origin != "All":
                    current_filter += f" origin:'{origin}'"
                ...
                if origin == "All":
                    ...
                    description = f"vulnerabilities with origin={origin}"
            ...
            self.logger.error(
                f"{LOG_PREFIX} Failed to build vulnerability filter configurations: {str(e)}"
            )
            return [
                {
                    "params": {
    ...
    async def fetch_vulnerabilities(
        ...
        """Fetch vulnerabilities from the API for standalone import.
        ...
        """
        filter_configs = self._build_vulnerability_filter_configurations(initial_state)
        vulnerability_fetcher = self.fetcher_factory.create_fetcher_by_name(
            "vulnerabilities_list", base_url=self.config.api_url
        )
        ...
        for filter_config in filter_configs:
            ...
            self.logger.info(
                f"{LOG_PREFIX} Fetching vulnerabilities with filters: {endpoint_params}"
            )
