...
class RetryRequestStrategy(BaseRequestStrategy):
    ...
    async def _handle_api_error(
        ...
        """
        # Log 403/404 at debug level since they're expected and handled gracefully
        if isinstance(error, ApiHttpError) and error.status_code in (403, 404):
            self._logger.debug(
                f"{LOG_PREFIX} HTTP {error.status_code} for {self.api_req.url} - entity blocked by proxy or not found."
            )
        else:
            self._logger.warning(
                f"{LOG_PREFIX} Attempt {attempt + 1}/{self.max_retries + 1} for {self.api_req.url} failed with {type(error).__name__}: {error}."
            )
        # Don't count 403 (proxy blocked) or 404 (not found) toward circuit breaker failures
        # as these are expected and handled gracefully
        if not (isinstance(error, ApiHttpError) and error.status_code in (403, 404)):
            self.breaker.record_failure()
            ...
            self._logger.debug(
                f"{LOG_PREFIX} HTTP {error.status_code} for {self.api_req.url} - Not counting toward circuit breaker failures."
            )

        if isinstance(error, ApiHttpError) and error.status_code < 500:
            # Log 403/404 at debug level since they're handled gracefully
            if error.status_code in (403, 404):
                self._logger.debug(
                    f"{LOG_PREFIX} HTTP {error.status_code} for {self.api_req.url} - skipping entity."
                )
            else:
                self._logger.error(
