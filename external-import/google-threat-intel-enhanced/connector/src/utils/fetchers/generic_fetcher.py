...
class GenericFetcher:
    ...
    async def fetch_multiple(
        ...
        for idx, entity_id in enumerate(entity_ids):
            ...
            except ApiHttpError as http_err:
                # 403/404 are handled gracefully - just skip the entity
                if http_err.status_code in (403, 404):
                    self.logger.debug(
                        f"{LOG_PREFIX} HTTP {http_err.status_code} for {self.config.display_name_singular} {entity_id} - skipping"
                    )
                else:
                    self.logger.warning(
                        f"{LOG_PREFIX} HTTP {http_err.status_code} for {self.config.display_name_singular} {entity_id}: {str(http_err)}"
                    )
                continue
    ...
    async def _make_api_call(
        ...
        try:
            ...
            # Handle 403 (proxy blocked) and 404 (not found) gracefully by skipping
            if http_err.status_code in (403, 404):
                self.logger.debug(
                    f"{LOG_PREFIX} HTTP {http_err.status_code} for {self.config.entity_type} "
                    f"{entity_id or 'unknown'} - skipping (may be blocked by proxy or not found)"
                )
                return None
            # Re-raise other HTTP errors
            self._handle_general_error(http_err, endpoint, entity_id)
            return None
        except Exception as e:
