...
class GenericBatchProcessor:
    ...
    def _send_bundle(self, work_id: str, items: List[Any], batch_num: int) -> None:
        ...
        try:
            ...
            # Handle "Nothing to import" - this is not a failure, just no new objects
            if "Nothing to import" in str(val_err):
                self._logger.info(
                    f"{LOG_PREFIX} Batch #{batch_num} had no new objects to import (all duplicates or already processed)"
                )
                return  # Success - just nothing new to import
            # Re-raise other ValueErrors
            raise self.config.create_exception(
                f"Bundle sending failed for batch #{batch_num}: {str(val_err)}",
                work_id=work_id,
                items_count=len(items),
            ) from val_err
