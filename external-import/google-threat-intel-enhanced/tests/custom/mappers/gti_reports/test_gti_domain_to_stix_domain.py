...
def test_gti_domain_to_stix_minimal_data(
    ...
    # Then only domain observable should be created (no verdict = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    domain_observable = stix_objects[0]
...
def test_gti_domain_to_stix_with_benign_verdict(
    ...
    # Then only domain observable should be created (benign verdict = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    domain_observable = stix_objects[0]
    assert domain_observable is not None  # noqa: S101
...
def test_gti_domain_to_stix_without_attributes(
    ...
    # Then only domain observable should be created (no attributes = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    domain_observable = stix_objects[0]
    assert domain_observable is not None  # noqa: S101
...
def test_gti_domain_to_stix_with_empty_verdict(
    ...
    # Then only domain observable should be created (empty verdict = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    domain_observable = stix_objects[0]
    assert domain_observable is not None  # noqa: S101
...
def test_get_score_with_mandiant_score(
    ...
    # Then score should be None (no verdict in gti_derived mode)
