...
def test_gti_ipv4_to_stix_minimal_data(
    ...
    # Then only IPv4 observable should be created (no verdict = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    ip_observable = stix_objects[0]
...
def test_gti_ipv6_to_stix_minimal_data(
    ...
    # Then only IPv6 observable should be created (no verdict = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    ip_observable = stix_objects[0]
...
def test_gti_ip_to_stix_with_timestamps(
    ...
    # Then only IP observable should be created (no verdict = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    ip_observable = stix_objects[0]
...
def test_gti_ip_to_stix_with_malicious_verdict(
    ...
    _then_stix_indicator_has_type(indicator, IndicatorTypeOV("VERDICT_MALICIOUS"))
...
def test_gti_ip_to_stix_with_benign_verdict(
    ...
    # Then only IP observable should be created (benign verdict = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    ip_observable = stix_objects[0]
    assert ip_observable is not None  # noqa: S101
...
def test_gti_ip_to_stix_with_suspicious_verdict(
    ...
    _then_stix_objects_created_successfully(stix_objects)
    ip_observable, indicator, relationship = stix_objects
    _then_stix_indicator_has_type(indicator, IndicatorTypeOV("VERDICT_SUSPICIOUS"))
...
def test_gti_ip_to_stix_without_attributes(
    ...
    # Then only IP observable should be created (no attributes = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    ip_observable = stix_objects[0]
    assert ip_observable is not None  # noqa: S101
...
def test_gti_ip_to_stix_with_empty_verdict(
    ...
    # Then only IP observable should be created (empty verdict = no indicator)
    assert len(stix_objects) == 1  # noqa: S101
    ip_observable = stix_objects[0]
    assert ip_observable is not None  # noqa: S101
...
def test_determine_indicator_types_with_verdict(
    ...
    assert indicator_types == [IndicatorTypeOV("VERDICT_MALICIOUS")]  # noqa: S101
