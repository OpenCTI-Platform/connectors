...
class OpenCTI:
    ...
    def __init__(self, config: ConnectorSettings, helper: OpenCTIConnectorHelper):
        ...
        urls = [self.config.config.sectors_file_url, self.config.config.geography_file_url, self.config.config.companies_file_url]
    ...
    def retrieve_data(self, url: str) -> dict:
        ...
        """
        try:
            return json.loads(urllib.request.urlopen(url, context=ssl.create_default_context()).read().decode('utf-8'))
        except (urllib.error.URLError, urllib.error.HTTPError, urllib.error.ContentTooShortError) as urllib_error:
    ...
    def process_data(self):
        try:
            ...
            if current_state is not None and 'last_run' in current_state:
                ...
                self.helper.log_info('Connector last run: ' + datetime.fromtimestamp(last_run, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S'))
            else:
                last_run = None
                self.helper.log_info('Connector has never run')
            if last_run is None or timestamp - last_run > self.interval:
                now = datetime.fromtimestamp(timestamp, tz=timezone.utc)
                friendly_name = 'OpenCTI datasets run @ ' + now.strftime('%Y-%m-%d %H:%M:%S')
                ...
                self.helper.log_info('Last_run stored, next run in: ' + str(round(self.interval / 60 / 60 / 24, 2)) + ' days')
            else:
    ...
    def send_bundle(self, work_id: str, data: dict) -> None:
        try:
            self.helper.send_stix2_bundle(json.dumps(data), entities_types=self.helper.connect_scope, update=True, work_id=work_id)
        except Exception as e:
