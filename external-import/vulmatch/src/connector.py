...
class VulmatchConnector:
    ...
    def __init__(self):
        ...
        self.cvss_base_score_min = parse_number(self._get_param('cvss_base_score_min', is_number=True, default_value=-1))
        self.epss_score_min = parse_number(self._get_param('epss_score_min', is_number=True, default_value=-1))
    ...
    def get_vulnerabilities(self, cpes):
        ...
        vulnerabilities = self.retrieve(
            ...
            params=dict(
                ...
                modified_max=datetime.now(UTC).isoformat(), # make sure number of items do not increase while retrieving
            )
        )
    ...
    def process_vulnerability(self, vuln):
        ...
        try:
            objects = self.retrieve(f'v1/cve/objects/{cve_name}/bundle/', list_key='objects')
            ...
            self.helper.log_error(f"process {cve_name} failed", dict(work_id=cve_work_id))
            self.helper.api.work.report_expectation(
                work_id=cve_work_id,
                error={"error": f"[{cve_name}] could not process", "source": "CONNECTOR"},
            )
            self.helper.api.work.to_processed(
                work_id=cve_work_id,
    ...
    def _get_state(self) -> dict:
        state = self.helper.get_state() or dict(last_vulnerability_modified=(datetime.now(UTC) - timedelta(days=self.backfill_days)).isoformat())
        return state
    
    def update_state(self, vulnerability_modified):
        state = self._get_state()
        state.update(
            last_vulnerability_modified=max(vulnerability_modified, state.get('last_vulnerability_modified', vulnerability_modified)),
