...
| ~~SOC Prime Interval~~        | ~~interval_sec~~    | ~~`SOCPRIME_INTERVAL_SEC`~~    | ~~`3600`~~ | ⚠️ Deprecated | ~~The import interval in seconds. This option is **deprecated**, please use `duration_period` instead.~~                                                                                                                                                                                        |
...
| SOC PRIME TLP Level           | tlp_level           | `SOCPRIME_TLP_LEVEL`           | `amber+strict`      | No            | The TLP level to use for the imported elements missing a TLP level. It can be `white`, `green`, `amber`, `amber+strict`, or `red`. Defaults to `amber+strict`.                                                                                                                                         |

The list of possible values for the `siem_type` or `indicator_siem_type` (`SOCPRIME_SIEM_TYPE` or
`SOCPRIME_INDICATOR_SIEM_TYPE`) variables:
...
⚠️ Please be aware that `CONNECTOR_DURATION_PERIOD` default value takes precedence over `SOCPRIME_INTERVAL_SEC` default
value if none of them are set.
