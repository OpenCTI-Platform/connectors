"""Unit tests for connector.settings — Pydantic field validators and typed fields.
...
# ---------------------------------------------------------------------------
# subject_filters validator (Json[list[dict]])
# ---------------------------------------------------------------------------
...
class TestSubjectFiltersValidator:
    def test_valid_filters(self):
        ...
        assert cfg.subject_filters == [{"type": "contains", "value": "Alert"}]
    ...
    def test_all_supported_filter_types(self, ftype):
        ...
        assert cfg.subject_filters[0]["type"] == ftype
...
class TestSubjectRulesValidator:
    ...
    def test_all_supported_match_types(self, match_type):
        ...
        assert cfg.subject_rules[0]["match_type"] == match_type
...
# ---------------------------------------------------------------------------
# start_date (DatetimeFromIsoString | None)
# ---------------------------------------------------------------------------
...
class TestDefaults:
    ...
    def test_invalid_severity_rejected(self):
        with pytest.raises(ValidationError):
            EmailCasesConfig(**_base_kwargs(default_severity="urgent"))

    def test_invalid_priority_rejected(self):
        with pytest.raises(ValidationError):
            EmailCasesConfig(**_base_kwargs(default_priority="P9"))

    def test_optional_credentials_default_to_none(self):
        cfg = EmailCasesConfig(**_base_kwargs())
        assert cfg.imap_host is None
        assert cfg.case_prefix is None
