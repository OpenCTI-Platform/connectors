...
class TestFetchEmails:
    ...
    def test_paginates_across_pages(self):
        # A nextPageToken must trigger a follow-up list call carrying that
        # token, so max_results spanning more than one page is honoured instead
        # of silently truncating to the first 100 results.
        c = _client()
        sess = MagicMock()
        list1 = _resp({"messages": [{"id": "m1"}], "nextPageToken": "PAGE2"})
        list2 = _resp({"messages": [{"id": "m2"}]})
        full1 = _resp(_full_message("m1", "t1"))
        full2 = _resp(_full_message("m2", "t2"))
        sess.get.side_effect = [list1, list2, full1, full2]
        c._session = sess
        c._credentials = MagicMock(expired=False)
        out = c.fetch_emails(sender="alerts@x.com")
        assert len(out) == 2
        assert sess.get.call_args_list[1].kwargs["params"]["pageToken"] == "PAGE2"
...
class TestAttachmentsAndBody:
    ...
    def test_attachment_download_tolerates_unpadded_b64url(self):
        # Gmail attachment payloads are base64url and may omit '=' padding; the
        # download path must decode them without raising binascii.Error.
        c = _client()
        sess = MagicMock()
        raw = b"encrypted-evidence-bytes!"
        unpadded = base64.urlsafe_b64encode(raw).decode().rstrip("=")
        list_resp = _resp({"messages": [{"id": "m1"}]})
        full_resp = _resp(_full_message(with_attachment=True))
        download_resp = _resp({"data": unpadded})
        sess.get.side_effect = [list_resp, full_resp, download_resp]
        c._session = sess
        c._credentials = MagicMock(expired=False)
        out = c.fetch_emails(sender="a@b.com")
        assert out[0].attachments[0].content == raw
