...
class TestLifecycle:
    ...
    def test_disconnect_restores_global_adapter_when_tls_disabled(self):
        # tls_verify=False swaps the process-global HTTP_ADAPTER_CLS; disconnect
        # must restore the prior value rather than leaving TLS verification
        # disabled for every exchangelib user in the process.
        ...
        with (
            ...
            c.connect()
            assert BaseProtocol.HTTP_ADAPTER_CLS is not original
        c.disconnect()
        assert BaseProtocol.HTTP_ADAPTER_CLS is original
