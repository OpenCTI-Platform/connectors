...
def make_config(**overrides):
    """Build a config whose `.email_cases`/`.connector` match the runtime
    settings surface (typed attributes, no ``get_parsed_*`` helpers)."""
    ec = SimpleNamespace(
        ...
        case_prefix=None,
        protocol="imap",
        subject_filters=overrides.get("subject_filters", []),
        labels=overrides.get("labels", []),
        subject_rules=overrides.get("subject_rules", []),
        sender_rules=overrides.get("sender_rules", []),
        start_date=overrides.get("start_date", None),
    )
    for key in (
        "sender_address",
    ...
    connector_ns = SimpleNamespace(duration_period=timedelta(minutes=5))
    return SimpleNamespace(email_cases=ec, connector=connector_ns)
...
class TestImportEmails:
    ...
    def test_off_sender_email_is_dropped(self, conn, helper, monkeypatch):
        # A fuzzy protocol-level filter can surface a message from a different
        # sender; the strict sender check must drop it before subject filtering
        # so it is never turned into a case.
        on_sender = make_email(message_id="<on@example.com>", thread_id="t-on")
        off_sender = make_email(
            message_id="<off@example.com>",
        ...
        monkeypatch.setattr(
            connector_mod,
            "create_email_client",
            lambda cfg: FakeClient([on_sender, off_sender]),
        )
        conn._import_emails()
        assert helper.api.case_incident.create.call_count == 1
        state = helper.set_state.call_args.args[0]
        assert "<on@example.com>" in state["processed_message_ids"]
        assert "<off@example.com>" not in state["processed_message_ids"]
...
class TestTimeoutsAndRun:
    ...
    def test_run_delegates_to_scheduler(self, conn, helper):
        conn.run()
        ...
        assert kwargs["duration_period"] == conn.config.connector.duration_period

    def test_process_skips_cycle_on_connection_failure(self, conn, monkeypatch):
        monkeypatch.setattr(
            conn,
            "_test_connection_with_timeout",
            MagicMock(side_effect=RuntimeError("down")),
        )
        monkeypatch.setattr(conn, "_import_emails", MagicMock())
        conn.process()  # logs and returns, does NOT raise
        conn._import_emails.assert_not_called()
        assert conn._bootstrapped is False

    def test_process_bootstraps_once_then_imports(self, conn, monkeypatch):
        monkeypatch.setattr(conn, "_test_connection_with_timeout", MagicMock())
        monkeypatch.setattr(conn, "_ensure_vocabularies", MagicMock())
        monkeypatch.setattr(conn, "_ensure_connector_author", MagicMock())
        monkeypatch.setattr(conn, "_import_emails", MagicMock())
        conn.process()
        conn.process()
        # Bootstrap (vocab + author) runs once; the import runs every cycle.
        conn._ensure_vocabularies.assert_called_once()
        conn._ensure_connector_author.assert_called_once()
        assert conn._import_emails.call_count == 2
