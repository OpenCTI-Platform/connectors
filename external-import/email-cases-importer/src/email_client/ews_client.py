...
class EwsClient(BaseEmailClient):
    ...
    def __init__(
        ...
        self._account = None
        self._prev_http_adapter_cls = None
    ...
    def connect(self) -> None:
        ...
        if not self._tls_verify:
            # HTTP_ADAPTER_CLS is a process-global in exchangelib; remember the
            # previous value so disconnect() can restore it rather than leaving
            # TLS verification disabled for every exchangelib user in the process.
            self._prev_http_adapter_cls = BaseProtocol.HTTP_ADAPTER_CLS
    ...
    def disconnect(self) -> None:
        ...
        self._account = None
        if self._prev_http_adapter_cls is not None:
            from exchangelib.protocol import BaseProtocol

            BaseProtocol.HTTP_ADAPTER_CLS = self._prev_http_adapter_cls
            self._prev_http_adapter_cls = None
