...
# Hardcoded author identity name for cases with no sender-rule author. Kept
# constant (rather than read from CONNECTOR_NAME) so renaming the connector
# instance never spawns a duplicate author Identity in OpenCTI.
CONNECTOR_AUTHOR_NAME = "Email Cases Importer"
...
class EmailCasesConnector:
    def __init__(self, config: ConnectorSettings, helper: OpenCTIConnectorHelper):
        ...
        self._subject_filters = config.email_cases.subject_filters
        ...
        self._static_labels = config.email_cases.labels
        self._subject_rules = config.email_cases.subject_rules
        self._sender_rules = config.email_cases.sender_rules
        ...
        # OpenCTI internal id of the connector's own author identity, created on
        # the first run (see _ensure_connector_author) and used as the default
        # case author when no sender-rule author applies.
        self._connector_author_id: str | None = None
        # One-time platform bootstrap guard (vocabularies + author identity).
    ...
    def _ensure_connector_author(self) -> None:
        ...
        """
        self._connector_author_id = self._resolve_identity_id(CONNECTOR_AUTHOR_NAME)

    def run(self):
        """Schedule the connector's runs via the SDK scheduler."""
    ...
    def process(self):
        """One scheduled cycle: validate connectivity, bootstrap, import."""
        # Validate email connectivity first. On failure, log and skip this cycle
        # so the scheduler retries next duration_period rather than killing the
        # container on a transient mail-server outage.
        try:
            self._test_connection_with_timeout()
        except Exception as exc:
            self.helper.connector_logger.error(
                "[CONNECTOR] Email connection failed — skipping cycle",
                {"error": str(exc)},
            )
            return

        # One-time platform bootstrap: vocabulary values + author identity.
    ...
    def _import_emails(self):
        ...
        if last_run:
            ...
            configured_start = self.config.email_cases.start_date
            if configured_start:
        ...
        try:
            ...
            with client:
                ...
                # Subject filter stage — applied to the sender-verified list so
                # that off-sender messages surfaced by a fuzzy protocol filter
                # are dropped here instead of being processed.
                subject_matched = [
                    e
                    for e in sender_exact
                ...
                self.helper.connector_logger.info(
                    "[CONNECTOR] Fetch/filter breakdown",
                    {
                        ...
                        "subject_skipped": len(sender_exact) - len(subject_matched),
