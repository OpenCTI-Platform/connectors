...
class StixifyConnector:
    ...
    def get_reports_after_last(self, dossier):
        ...
        self.helper.log_info("processing dossier(id={id}, title='{name}')".format_map(dossier))
        dossier_state = self._get_state()['dossiers'].get(dossier_id, dict(latest_report=''))
        filters = dict()
    ...
    def process_report(self, dossier_id, report: dict):
        ...
        try:
            objects = self.retrieve(f'v1/reports/{report_id}/objects/', list_key='objects')
            bundle = dict(
                type='bundle',
                id=f"bundle--{report_id}",
                objects=objects,
            )
            self.helper.log_info(f'{report_name} sending bundle with {len(objects)} items')
            ...
            report_created = (datetime.fromisoformat(report_created) + timedelta(milliseconds=990)).isoformat()
    ...
    def _run_once(self):
        ...
        for dossier in self.list_dossiers():
            ...
            if dossier_id not in (self.dossier_ids or [dossier_id]):
                self.helper.log_info(f'skipping {dossier_repr} not in config.stixify.dossier_ids')
                continue
    ...
    def run_once(self):
        in_error = False
        try:
            self.work_id = self.helper.api.work.initiate_work(self.helper.connect_id, self.helper.connect_name)
            self._run_once()
        except:
    ...
    def set_dossier_state(self, dossier_id, last_updated):
        ...
        if dossier_id:
            ...
            dossier_state.update(latest_report=max(last_updated, dossier_state.get('latest_report', last_updated)))
