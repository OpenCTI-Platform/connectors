...
def _construct():
    ...
    from config import ConfigConnector as RealConfigConnector
    ...
    with patch("external_import.ConfigConnector", mock_cls), patch(
        "external_import.OpenCTIConnectorHelper", return_value=helper
    ), patch("external_import.setup_file_logging"), patch(
        "external_import.TIAdapter", return_value=ti_adapter
    ):
...
class TestExternalImportConnectorInit:
    ...
    def test_enabled_collections_filtered_via_config(self):
        ...
        mock_cls.COLLECTION_MAP = RealConfigConnector.COLLECTION_MAP
        with patch("external_import.ConfigConnector", mock_cls), patch(
            "external_import.OpenCTIConnectorHelper", return_value=_stub_helper()
        ), patch("external_import.setup_file_logging"), patch(
            "external_import.TIAdapter", return_value=MagicMock()
        ):
            from external_import import ExternalImportConnector
    ...
    def test_ti_adapter_constructed_with_creds(self):
        # Capture the TIAdapter mock class so we can inspect the kwargs the
