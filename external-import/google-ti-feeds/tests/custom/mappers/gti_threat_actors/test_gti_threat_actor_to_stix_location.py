"""Tests for the GTIThreatActorToSTIXLocation mapper."""
...
@pytest.fixture
def threat_actor_with_country_regions() -> GTIThreatActorData:
    """Fixture for GTI threat actor with country regions."""
...
@pytest.fixture
def threat_actor_with_multiple_countries() -> GTIThreatActorData:
    """Fixture for GTI threat actor with multiple countries."""
    return GTIThreatActorDataFactory.build(
        attributes=ThreatActorModelFactory.build(
            targeted_regions_hierarchy=[
                TargetedRegionFactory.build(
...
@pytest.fixture
def threat_actor_without_regions() -> GTIThreatActorData:
    """Fixture for GTI threat actor without regions."""
    return GTIThreatActorDataFactory.build(
        attributes=ThreatActorModelFactory.build(
...
@pytest.fixture
def threat_actor_with_empty_regions() -> GTIThreatActorData:
    """Fixture for GTI threat actor with empty regions."""
    return GTIThreatActorDataFactory.build(
        attributes=ThreatActorModelFactory.build(
            targeted_regions_hierarchy=[],
            source_regions_hierarchy=[],
...
@pytest.fixture
def threat_actor_with_region_only() -> GTIThreatActorData:
    """Fixture for GTI threat actor with region only (no country)."""
    return GTIThreatActorDataFactory.build(
        attributes=ThreatActorModelFactory.build(
            targeted_regions_hierarchy=[
                TargetedRegionFactory.build(
...
@pytest.fixture
def threat_actor_with_country_without_iso() -> GTIThreatActorData:
    """Fixture for GTI threat actor with country but no ISO code."""
    return GTIThreatActorDataFactory.build(
        attributes=ThreatActorModelFactory.build(
            targeted_regions_hierarchy=[
                TargetedRegionFactory.build(
...
@pytest.fixture
def threat_actor_with_mixed_valid_invalid_countries() -> GTIThreatActorData:
    """Fixture for GTI threat actor with mixed valid and invalid countries."""
    return GTIThreatActorDataFactory.build(
        attributes=ThreatActorModelFactory.build(
            targeted_regions_hierarchy=[
                TargetedRegionFactory.build(
...
@pytest.fixture
def threat_actor_with_empty_country_data() -> GTIThreatActorData:
    """Fixture for GTI threat actor with empty country data."""
    return GTIThreatActorDataFactory.build(
        attributes=ThreatActorModelFactory.build(
            targeted_regions_hierarchy=[
                TargetedRegionFactory.build(
...
@pytest.mark.order(1)
def test_gti_threat_actor_to_stix_location_country_regions(
    threat_actor_with_country_regions, mock_organization, mock_tlp_marking
):
    """Test conversion of GTI threat actor with country regions to STIX locations."""
    # GIVEN: A GTI threat actor containing both targeted and source countries
    # with valid country names and ISO codes for location creation
    mapper = _given_gti_threat_actor_location_mapper(
        threat_actor_with_country_regions, mock_organization, mock_tlp_marking
    )

    # WHEN: Converting the GTI threat actor data to STIX location objects
...
@pytest.mark.order(1)
def test_gti_threat_actor_to_stix_location_multiple_countries(
    threat_actor_with_multiple_countries, mock_organization, mock_tlp_marking
):
    """Test conversion of GTI threat actor with multiple countries."""
    # GIVEN: A GTI threat actor containing multiple targeted and source countries
    # with valid country names and ISO codes for location creation
    mapper = _given_gti_threat_actor_location_mapper(
        threat_actor_with_multiple_countries, mock_organization, mock_tlp_marking
    )
...
@pytest.mark.order(1)
def test_gti_threat_actor_to_stix_location_without_regions(
    threat_actor_without_regions, mock_organization, mock_tlp_marking
):
    """Test conversion of GTI threat actor without regions."""
    # GIVEN: A GTI threat actor with no targeted or source regions defined
...
@pytest.mark.order(1)
def test_gti_threat_actor_to_stix_location_empty_regions(
    threat_actor_with_empty_regions, mock_organization, mock_tlp_marking
):
    """Test conversion of GTI threat actor with empty regions."""
    # GIVEN: A GTI threat actor with empty lists for targeted and source regions
...
@pytest.mark.order(1)
def test_gti_threat_actor_to_stix_location_without_attributes(
    ...
    # making it impossible to access any threat actor data including regions
    mapper = _given_gti_threat_actor_location_mapper(
        threat_actor_without_attributes, mock_organization, mock_tlp_marking
    )

    # WHEN: Attempting to convert the GTI threat actor data to STIX location objects
...
@pytest.mark.order(1)
def test_gti_threat_actor_to_stix_location_region_only(
    threat_actor_with_region_only, mock_organization, mock_tlp_marking
):
    """Test conversion of GTI threat actor with region only (no countries)."""
    # GIVEN: A GTI threat actor containing only region-level data
...
@pytest.mark.order(1)
def test_gti_threat_actor_to_stix_location_country_without_iso(
    threat_actor_with_country_without_iso, mock_organization, mock_tlp_marking
):
    """Test conversion of GTI threat actor with country without ISO code."""
    # GIVEN: A GTI threat actor containing country names but missing ISO codes
...
@pytest.mark.order(1)
def test_gti_threat_actor_to_stix_location_mixed_valid_invalid_countries(
    threat_actor_with_mixed_valid_invalid_countries, mock_organization, mock_tlp_marking
):
    """Test conversion of GTI threat actor with mixed valid and invalid countries."""
    # GIVEN: A GTI threat actor containing both valid and invalid country data
    # to test selective processing of only valid geographical information
    mapper = _given_gti_threat_actor_location_mapper(
        threat_actor_with_mixed_valid_invalid_countries,
        mock_organization,
        mock_tlp_marking,
    )
...
@pytest.mark.order(1)
def test_gti_threat_actor_to_stix_location_empty_country_data(
    threat_actor_with_empty_country_data, mock_organization, mock_tlp_marking
):
    """Test conversion of GTI threat actor with empty country data."""
    # GIVEN: A GTI threat actor containing region entries with no country data
...
def test_create_country_from_targeted_with_valid_data(
    ...
    # GIVEN: A GTI threat actor location mapper with valid country data
    mapper = _given_gti_threat_actor_location_mapper(
        threat_actor_with_country_regions, mock_organization, mock_tlp_marking
    )
    region_data = (
        threat_actor_with_country_regions.attributes.targeted_regions_hierarchy[0]
    )
...
def test_create_country_from_source_with_valid_data(
    ...
    # GIVEN: A GTI threat actor location mapper with valid source country data
    mapper = _given_gti_threat_actor_location_mapper(
        threat_actor_with_country_regions, mock_organization, mock_tlp_marking
    )
    region_data = threat_actor_with_country_regions.attributes.source_regions_hierarchy[
        0
    ]
...
def test_create_country_without_country_name(mock_organization, mock_tlp_marking):
    ...
    # GIVEN: A GTI threat actor location mapper and region data without country name
    ...
    region_data = threat_actor.attributes.targeted_regions_hierarchy[0]
...
def test_create_country_without_iso_code(mock_organization, mock_tlp_marking):
    ...
    # GIVEN: A GTI threat actor location mapper and region data without ISO code
    threat_actor = GTIThreatActorDataFactory.build(
        attributes=ThreatActorModelFactory.build(
            targeted_regions_hierarchy=[
                TargetedRegionFactory.build(
...
@pytest.mark.order(1)
def test_gti_threat_actor_location_mapper_initialization(
    threat_actor_with_country_regions, mock_organization, mock_tlp_marking
):
    """Test GTIThreatActorToSTIXLocation mapper initialization."""
    ...
    # WHEN: Creating a new GTIThreatActorToSTIXLocation mapper instance
    mapper = GTIThreatActorToSTIXLocation(
        threat_actor=threat_actor_with_country_regions,
    ...
    assert mapper.threat_actor == threat_actor_with_country_regions  # noqa: S101
    assert mapper.organization == mock_organization  # noqa: S101
    assert mapper.tlp_marking == mock_tlp_marking  # noqa: S101


def _given_gti_threat_actor_location_mapper(
    threat_actor: GTIThreatActorData,
    organization: Identity,
    tlp_marking: MarkingDefinition,
) -> GTIThreatActorToSTIXLocation:
    """Create a GTIThreatActorToSTIXLocation mapper instance."""
    return GTIThreatActorToSTIXLocation(
...
def _when_convert_to_stix(mapper: GTIThreatActorToSTIXLocation) -> list:
    """Convert GTI threat actor to STIX locations."""
    return mapper.to_stix()


def _when_convert_to_stix_raises_error(
    mapper: GTIThreatActorToSTIXLocation, error_type: type, error_message: str
):
