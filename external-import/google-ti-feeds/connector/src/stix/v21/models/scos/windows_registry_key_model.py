...
def test_windows_registry_key_model() -> None:
    """Test function to demonstrate the usage of WindowsRegistryKeyModel."""
    from datetime import UTC, datetime
    from uuid import uuid4

    now = datetime.now(UTC)

    # === Minimal Registry Key ===
    minimal = WindowsRegistryKeyModel(
        type="windows-registry-key",
        spec_version="2.1",
        id=f"windows-registry-key--{uuid4()}",
        key="HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Windows\\CurrentVersion\\Run",
    )

    print("=== MINIMAL WINDOWS REGISTRY KEY ===")  # noqa: T201
    print(minimal.to_stix2_object().serialize(pretty=True))  # noqa: T201

    # === Full Registry Key ===
    full = WindowsRegistryKeyModel(
        type="windows-registry-key",
        spec_version="2.1",
        id=f"windows-registry-key--{uuid4()}",
        key="HKEY_CURRENT_USER\\Software\\Hydra\\Agent",
        values=[
            WindowsRegistryValueModel(
                name="InstallPath",
                data="C:\\Users\\ghost\\AppData\\Roaming\\Hydra",
                data_type=WindowsRegistryDatatypeOV.REG_SZ,
            ),
            WindowsRegistryValueModel(
                name="Persistence",
                data="true",
                data_type=WindowsRegistryDatatypeOV.REG_DWORD,
            ),
        ],
        modified_time=now,
    ...
    print("\n=== FULL WINDOWS REGISTRY KEY ===")  # noqa: T201
    print(full.to_stix2_object().serialize(pretty=True))  # noqa: T201


if __name__ == "__main__":
    test_windows_registry_key_model()
