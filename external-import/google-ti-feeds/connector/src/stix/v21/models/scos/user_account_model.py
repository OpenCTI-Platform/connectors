...
def test_user_account_model() -> None:
    """Test function to demonstrate the usage of UserAccountModel."""
    ...
    minimal = UserAccountModel(
        type="user-account",
        spec_version="2.1",
        id=f"user-account--{uuid4()}",
        account_login="root",
    )

    print("=== MINIMAL USER ACCOUNT ===")  # noqa: T201
    print(minimal.to_stix2_object().serialize(pretty=True))  # noqa: T201

    # === Full User Account ===
    full = UserAccountModel(
        type="user-account",
        spec_version="2.1",
        id=f"user-account--{uuid4()}",
        user_id="1001",
        credential="hunter2",
        account_login="ghost.ops",
        account_type=AccountTypeOV.UNIX,
        display_name="Ghost Operations",
        is_service_account=False,
        is_privileged=True,
        can_escalate_privs=True,
        is_disabled=False,
        account_created=now,
        account_expires=now.replace(year=now.year + 1),
        ...
        extensions={
            "unix-account-ext": {
                "gid": 1001,
                "shell": "/bin/bash",
                "home_dir": "/home/ghost.ops",
            }
        },
    )

    print("\n=== FULL USER ACCOUNT ===")  # noqa: T201
