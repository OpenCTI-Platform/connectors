...
def test_x509_certificate_model() -> None:
    """Test function to demonstrate the usage of X509CertificateModel."""
    ...
    # === Minimal X.509 Cert ===
    minimal = X509CertificateModel(
        type="x509-certificate",
        spec_version="2.1",
        id=f"x509-certificate--{uuid4()}",
        hashes={
            "SHA-256": "d4c7e9d8a08907a50dbeb4bc5830dcd353482ad4d937b8c7c8d21a7be81b127b"
        },
    )

    print("=== MINIMAL X509 CERTIFICATE ===")  # noqa: T201
    print(minimal.to_stix2_object().serialize(pretty=True))  # noqa: T201

    # === Full X.509 Cert ===
    full = X509CertificateModel(
        type="x509-certificate",
        spec_version="2.1",
        id=f"x509-certificate--{uuid4()}",
        is_self_signed=True,
        hashes={
            "SHA-1": "3f1fa508e0c98d96a6edfa273f2e6c98b9f4f412",
            "SHA-256": "cb478abcde932ddde123456789abcdef11223344556677889900aabbccddeeff",
        },
        version="3",
        serial_number="04:3A:D5:EF:4F",
        signature_algorithm="sha256WithRSAEncryption",
        issuer="CN=Hydra Ops CA, O=BlackSector, C=US",
        validity_not_before=now,
        validity_not_after=now.replace(year=now.year + 2),
        subject="CN=Hydra Beacon, O=BlackSector",
        subject_public_key_algorithm="rsaEncryption",
        subject_public_key_modulus="00c1a7d3ff88be2c4d229...",
        subject_public_key_exponent=65537,
        x509_v3_extensions=X509V3ExtensionsTypeModel(
            basic_constraints="CA:TRUE, pathlen:0",
            key_usage="Digital Signature, Key Encipherment",
            extended_key_usage="TLS Web Server Authentication, TLS Web Client Authentication",
            subject_key_identifier="12:34:56:78:90:AB:CD:EF:00:11",
            authority_key_identifier="AA:BB:CC:DD:EE:FF:00:11:22:33",
            subject_alternative_name="DNS:beacon.shadowops.com, DNS:hydra.local",
            crl_distribution_points="http://crl.blacksector.org/hydra.crl",
        ),
    )

    print("\n=== FULL X509 CERTIFICATE ===")  # noqa: T201
    print(full.to_stix2_object().serialize(pretty=True))  # noqa: T201


if __name__ == "__main__":
    test_x509_certificate_model()
