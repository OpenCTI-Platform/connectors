...
def add_to_refs(context_key: str, linking_method: Any) -> Any:
    ...
    def postprocess(stix_output: Any) -> Any:
        ...
        try:
            ...
            logger.warning(f"{LOG_PREFIX} Error adding refs: {str(e)}")
...
def create_relationship(
    relationship_type: str, source_context: str, mapper_class: Any
) -> Any:
    ...
    def postprocess(stix_output: Any) -> Any:
        source = get_context(source_context)
        if not source:
            return stix_output
        ...
        try:
            targets = stix_output if isinstance(stix_output, list) else [stix_output]
            for target in targets:
                if hasattr(target, "id") and not hasattr(target, "relationship_type"):
                    if hasattr(mapper_class, "create_relationship"):
                        rel = mapper_class.create_relationship(
                            source, relationship_type, target
                        )
                        result.append(rel)
                        logger.debug(
                            f"{LOG_PREFIX} Created {relationship_type}: {source.id} → {target.id}"
                        )

        except Exception as e:
            logger.warning(f"{LOG_PREFIX} Error creating {relationship_type}: {str(e)}")
        ...
        try:
            if operation == "set" and context_key:
                ...
                if entity:
                    ...
                    logger.debug(f"{LOG_PREFIX} Set {context_key} context")

            elif operation == "clear" and context_key:
                clear_context(context_key)
                logger.debug(f"{LOG_PREFIX} Cleared {context_key} context")

            elif operation == "clear_all":
                clear_all_contexts()
                logger.debug(f"{LOG_PREFIX} Cleared all contexts")

        except Exception as e:
            logger.warning(f"{LOG_PREFIX} Error managing context: {str(e)}")
...
def uses_relationship(mapper_class: Any, source_context: str = "intrusion_set") -> Any:
    """Create 'uses' relationships from source context."""
    return create_relationship("uses", source_context, mapper_class)


def targets_relationship(
    mapper_class: Any, source_context: str = "intrusion_set"
) -> Any:
    """Create 'targets' relationships from source context."""
    return create_relationship("targets", source_context, mapper_class)
...
def related_to_relationship(
    ...
    return create_relationship("related-to", source_context, mapper_class)
...
def set_malware_context() -> Any:
    """Store malware in context for later use."""
...
GTI_THREAT_ACTOR_MALWARE_CONVERTER_CONFIG = GenericConverterConfig(
    ...
    display_name_singular="malware family",
    validate_input=True,
    postprocessing_function=uses_relationship(GTIMalwareFamilyToSTIXMalware),
)
...
def intrusion_set_to_report() -> Any:
    ...
    def postprocess(stix_output: Any) -> Any:
        intrusion_set = get_context("intrusion_set")
        ...
        try:
            report = None
            for obj in stix_output if isinstance(stix_output, list) else [stix_output]:
                if hasattr(obj, "type") and obj.type == "report":
                    report = obj
                    break

            if report and hasattr(intrusion_set, "id"):
                rel = GTIThreatActorToSTIXIntrusionSet.create_relationship(
                    intrusion_set, "related-to", report
                )
                result.append(rel)

                GTIReportToSTIXReport.add_object_refs([intrusion_set.id], report)

                logger.debug(
                    f"{LOG_PREFIX} Linked intrusion set {intrusion_set.id} to report {report.id}"
                )

        except Exception as e:
            logger.warning(
                f"{LOG_PREFIX} Error linking intrusion set to report: {str(e)}"
            )
...
GTI_THREAT_ACTOR_ATTACK_TECHNIQUE_CONVERTER_CONFIG = GenericConverterConfig(
    ...
    display_name_singular="attack technique",
    validate_input=True,
    postprocessing_function=uses_relationship(GTIAttackTechniqueToSTIXAttackPattern),
)
...
GTI_THREAT_ACTOR_VULNERABILITY_CONVERTER_CONFIG = GenericConverterConfig(
    ...
    display_name_singular="vulnerability",
    validate_input=True,
    postprocessing_function=targets_relationship(GTIVulnerabilityToSTIXVulnerability),
)
...
GTI_THREAT_ACTOR_DOMAIN_CONVERTER_CONFIG = GenericConverterConfig(
    ...
    display_name_singular="domain",
    validate_input=True,
    postprocessing_function=related_to_relationship(GTIDomainToSTIXDomain),
)
...
GTI_THREAT_ACTOR_FILE_CONVERTER_CONFIG = GenericConverterConfig(
    ...
    display_name_singular="file",
    validate_input=True,
    postprocessing_function=related_to_relationship(GTIFileToSTIXFile),
)
...
GTI_THREAT_ACTOR_URL_CONVERTER_CONFIG = GenericConverterConfig(
    ...
    display_name_singular="URL",
    validate_input=True,
    postprocessing_function=related_to_relationship(GTIUrlToSTIXUrl),
)
...
GTI_THREAT_ACTOR_IP_CONVERTER_CONFIG = GenericConverterConfig(
    ...
    display_name_singular="IP address",
    validate_input=True,
    postprocessing_function=related_to_relationship(GTIIPToSTIXIP),
)
