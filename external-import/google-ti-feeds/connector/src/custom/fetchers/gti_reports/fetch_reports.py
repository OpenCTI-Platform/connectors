...
class FetchReports(BaseFetcher):
    ...
    async def _fetch_reports(self) -> None:
        ...
        self._logger.info(
            f"[Fetcher Reports] Fetching reports from endpoint: {endpoint}"
        )
        ...
        while current_url:
            ...
            if not gti_response:
                self._logger.error(
                    f"[Fetcher Reports] API call to {endpoint} did not return a valid GTIReportResponse object. Stopping pagination."
                )
                break
            ...
            if gti_response.data:
                ...
                self._logger.info(
                    f"[Fetcher Reports] Fetched {len(gti_response.data)} reports from current page, {retrieved_reports_count}/{total_reports_count} reports."
                )

                last_report_in_page = gti_response.data[-1]
                last_modification_timestamp = (
                    last_report_in_page.attributes.last_modification_date
                )
                date_from_timestamp = datetime.fromtimestamp(
                    last_modification_timestamp
                )
                self._logger.info(
                    f"[Fetcher Reports] Last report in page: {last_report_in_page.id.strip()}, last modification date: {date_from_timestamp}"
                )

                await self._publish_reports(
                    last_modification_timestamp, gti_response.data
                )
                await self._orchestrate_subfetches(gti_response.data)
            else:
                self._logger.info(
                    f"[Fetcher Reports] No report data in the current page from {endpoint}."
                )

            if (
                gti_response.meta
                and gti_response.meta.cursor
                and gti_response.links
                and gti_response.links.next
            ):
                current_url = gti_response.links.next
                self._logger.info("[Fetcher Reports] Preparing to fetch next page.")
            else:
                self._logger.info(
                    "[Fetcher Reports] No more pages to fetch (cursor/next link criteria not met or end of data)."
                )
                current_url = None

    async def _publish_reports(
        self, last_modification_timestamp: int, reports: List[GTIReportData]
    ) -> None:
        """Publish the fetched reports into the pubsub broker for further processing."""
        await broker.publish(
            f"{REPORTS_BROKER}", (last_modification_timestamp, reports)
        )
        self._logger.info(
            f"[Fetcher Reports] {len(reports)} reports published to broker for processing."
        )
    ...
    async def _publish_sentinels(self) -> None:
        ...
        await broker.publish(f"{REPORTS_BROKER}", (None, SENTINEL))
        self._logger.info(
            "[Fetcher Reports] Ends of data fetched, Sentinels published to broker."
        )
