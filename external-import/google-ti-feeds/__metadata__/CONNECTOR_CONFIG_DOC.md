...
| GTI_SOFTWARE_TOOLKIT_ORIGINS | `array` |  | string | `["google threat intelligence"]` | Comma-separated list of software toolkit origins to import, or 'All' for all origins. Allowed values: All, partner, crowdsourced, google threat intelligence | ```All```, ```partner```, ```google threat intelligence```, ```crowdsourced``` |
