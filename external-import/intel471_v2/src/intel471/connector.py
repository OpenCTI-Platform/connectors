...
class Intel471Connector:
    ...
    def __init__(
        ...
        api_key = self.config.intel471.api_key
        ...
        for stream_class in (
            ...
            if interval := self.config.intel471.__dict__[
                f"interval_{stream_class.group_label}"
            ]:
                self.out_queues[stream_class.label] = Queue()
                initial_history = self.config.intel471.__dict__[
                    f"initial_history_{stream_class.group_label}"
                ]
                self.add_job(
                    stream_class(
                        self.helper,
