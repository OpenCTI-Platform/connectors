...
class ShadowserverStixTransformation:
    ...
    def create_stix_objects(self):
        ...
        if self.object_refs:
            if self.incident.get("create", False):
                ...
                LOGGER.info("Not creating incident because 'create' is set to False.")
    ...
    def handle_stix_object_creation(
        ...
        if value:
            LOGGER.debug(f"Creating {key} STIX object: {value}")
            stix_object = create_method(value=value, labels=labels)

        if stix_object:
            LOGGER.debug(f"Created {key} STIX object: {stix_object.id}")
            ...
            LOGGER.warning(f"Failed to create {key}" | {"value": value})
    ...
    def get_published_date(self, report_list):
        ...
        """
        LOGGER.debug("Getting published date.")
        try:
            if (
                report_list
    ...
    def create_external_reference(self):
        ...
        """
        LOGGER.info(f"Creating external reference: ({self.url}).")
    ...
    def create_opencti_case(self, labels: list = [], severity: str = "low"):
        ...
        Returns:
            None
        """
        LOGGER.debug(f"Creating OpenCTI case: {self.report.get('id')}")
        ...
        opencti_obj = pycti.CustomObjectCaseIncident(**kwargs)
    ...
    def upload_stix2_artifact(self, report_list):
        ...
        Returns:
            None
        """
        LOGGER.debug("Uploading Shadowserver report as artifact.")
        ...
        LOGGER.info(f"Uploading Shadowserver report as artifact: {self.report}.")
    ...
    def create_author(self):
        ...
        LOGGER.debug("Creating author: Shadowserver Connector")
        kwargs = {
        ...
        author = stix2.Identity(
            id=pycti.Identity.generate_id("Shadowserver Connector", "Organization"),
            **kwargs,
        )
    ...
    def map_to_stix(self, element):
        ...
        LOGGER.debug("Mapping Shadowserver report to STIX.")
        observed_data_list = []
        ...
        if observed_data_list:
            ...
            LOGGER.error(f"Unable to create observed data for element: {element}")
        ...
        if element.get("tag"):
            ...
            for label in custom_labels:
                if label.upper().startswith("CVE"):
                    LOGGER.debug(f"Label is CVE: {label}")
                    self.create_vulnerability(label)
            labels_list.extend(custom_labels)
        LOGGER.debug(f"Labels: {labels_list}")
        return labels_list
    ...
    def create_vulnerability(self, name: str):
        ...
        LOGGER.debug(f"Creating vulnerability STIX object: {name}")
        kwargs = {
            "type": "vulnerability",
            "name": name.upper(),
            "created_by_ref": self.author_id,
        ...
        opencti_obj = stix2.Vulnerability(
            id=pycti.Vulnerability.generate_id(name), **kwargs
        )
    ...
    def create_asn(self, value: int, labels: list = []):
        ...
        LOGGER.debug(f"Creating ASN STIX object: {value}")
        kwargs = {
    ...
    def create_ip(self, value: str, labels: list = []):
        ...
        LOGGER.debug(f"Creating IP STIX object: {value}")
        kwargs = {"value": value}
        self.extend_stix_object(kwargs, labels)
    ...
    def create_hostname(self, value: str, labels: list = []):
        ...
        if not value:
            LOGGER.error("Hostname value is missing.")
            return None

        LOGGER.debug(f"Creating hostname STIX object: {value}")
    ...
    def create_mac_address(self, value: str, labels: list = []):
        ...
        if not value:
            LOGGER.error("MAC address value is missing.")
            return None

        LOGGER.debug(f"Creating MAC address STIX object: {value}")
    ...
    def create_network_traffic(
        ...
        LOGGER.debug("Creating network traffic STIX object.")
        kwargs = {
        ...
        if stix_object:
            LOGGER.debug(f"Created network traffic STIX object: {stix_object.id}")
    ...
    def create_x509_certificate(self, data: dict, labels: list = []):
        ...
        stix_object_id = str()
        LOGGER.debug(f"Creating X509 certificate STIX object: {data}")
    ...
    def create_observed_data(
        ...
        LOGGER.debug(f"Creating observed data STIX object: {observables_list}")
        try:
            ...
            stix_object = stix2.ObservedData(
                id=pycti.ObservedData.generate_id(observables), **kwargs
            )
    ...
    def create_stix_note_from_data(self, labels: list = []):
        ...
        for element in self.report_list:
            ...
            timestamp = (
                element.get("timestamp", "") if isinstance(element, dict) else ""
            )
            abstract = f"Shadowserver {self.type} Report {self.report_id} - {timestamp}"
            created = note_timestamp_to_datetime(timestamp) if timestamp else None
            kwargs = {
                "abstract": abstract,
                "content": content,
                "created_by_ref": self.author_id,
            ...
            if kwargs["object_refs"]:
                stix_object = stix2.Note(
                    id=pycti.Note.generate_id(created=created, content=content),
                    allow_custom=True,
                    **kwargs,
                )
