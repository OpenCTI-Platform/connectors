...
class TestShadowServerAPI:
    ...
    def test_init_marking_refs(shadow_server_api):
        assert shadow_server_api.marking_refs == "TLP:WHITE"

    def test_init_marking_refs_invalid(self, shadow_server_api):
        with pytest.raises(ValueError):
            ShadowServerAPI(api_key="test_api_key", api_secret="test_api_secret", marking_refs="invalid_marking_refs")
    ...
    def test_get_report_list(self, shadow_server_api, mocker):
        ...
        self.shadowserver_fixture("report_list.json", fixture_mocker=mocker, shadow_server_api=shadow_server_api)
    ...
    def test_get_subscriptions(self, shadow_server_api, mocker):
        ...
        self.shadowserver_fixture("subscriptions.json", fixture_mocker=mocker, shadow_server_api=shadow_server_api)
        subscriptions = shadow_server_api.get_subscriptions()
        assert len(subscriptions) > 0
        assert len(subscriptions) == 13

    def test_get_reports(self, shadow_server_api, mocker):
        """Test the get_reports method."""
        self.shadowserver_fixture("report_type_blocklist.json", fixture_mocker=mocker, shadow_server_api=shadow_server_api)
        reports = shadow_server_api.get_report(report_id="test_report_id", report="blocklist")
        assert len(reports) > 0
        assert len(reports) == 5

    def test_get_report_id_invalid(self, shadow_server_api, mocker):
        """Test the get_reports method."""
        # patch request with 404 error
        ...
        with pytest.raises(requests.exceptions.HTTPError):
            shadow_server_api.get_report(report_id="invalid_report_id", report="blocklist")
    ...
    def test_get_report_type_invalid(self, shadow_server_api, mocker):
        ...
        mock_request.side_effect = requests.exceptions.HTTPError
        with pytest.raises(requests.exceptions.HTTPError):
            shadow_server_api.get_report(report_id="test_report_id", report="invalid_report")
