...
HASH_TYPE_MAPPING = {
    "CRC32": {
        "normalized": HashTypes.CRC32.value,
        "supported": False
    }, 
    "MD5": {
        "normalized": HashTypes.MD5.value,
        "supported": True
    },
    "SHA1": {
        "normalized": HashTypes.SHA1.value,
        "supported": True
    },
    "SHA256": {
        "normalized": HashTypes.SHA256.value,
        "supported": True
    },
    "SHA384": {
        "normalized": HashTypes.SHA384.value,
        "supported": False
    },
    "SHA512": {
        "normalized": HashTypes.SHA512.value,
        "supported": True
    },
    "SHA3256": {
        "normalized": HashTypes.SHA3256.value,
        "supported": True
    },
    "SHA3512": {
        "normalized": HashTypes.SHA3512.value,
        "supported": True
    },
    "SSDEEP": {
        "normalized": HashTypes.SSDEEP.value,
        "supported": True
    },
    "TLSH": {
        "normalized": HashTypes.TLSH.value,
        "supported": True
    },
    "RIPEMD160": {
        "normalized": HashTypes.RIPEMD160.value,
        "supported": False
    },
    # Add more hash type variations as needed
}

HASH_IDENTIFICATION_BY_LENGTH = {
    32: HashTypes.MD5,
    40: HashTypes.SHA1
}
...
def identify_hash(hash_value: str, hash_type: str = None) -> str:
    ...
    if hash_type:
        ...
        if hash_info['supported']:
            logger.debug(f"Mapped hash_type ({hash_info['normalized']}) for ({hash_value}).")
            return hash_info['normalized']
        else:
            logger.debug(f"Mapped unsupported hash_type ({hash_info['normalized']}) for ({hash_value}).")
