...
class RansomwareAPIConnector:
    ...
    def stix_object_generator(self,item):
        ...
        for field in ['screenshot', 'website', 'post_url']:
            ...
            if item.get(field):
                external_reference = ExternalReference(source_name="ransomware.live",url=item[field],description=f"This is the {field} for the ransomware campaign.")
        ...
        Target_attribution=Relationship(relationship_type="attributed-to",source_ref=threat_actor.id,target_ref=identity.id)
        Target_relation=Relationship(relationship_type="targets",source_ref=threat_actor.id,target_ref=identity.id)
        Report_attribution=Relationship(relationship_type="attributed-to", source_ref=report.id, target_ref=identity.id)
        Report_relation=Relationship(relationship_type="related-to", source_ref=report.id, target_ref=threat_actor.id)
        Threat_relation=Relationship(relationship_type="related-to", source_ref=threat_actor.id, target_ref=report.id)

        # Creating Location object
        if(item['country']!=""):
            location= Location(name=item['country'],country=item['country'])
            bundle = Bundle(objects=[report,identity,threat_actor,location,Target_attribution,Target_relation,Report_attribution,Report_relation,Threat_relation], allow_custom=True)
        else:
            bundle = Bundle(objects=[report,identity,threat_actor,Target_attribution,Target_relation,Report_attribution,Report_relation,Threat_relation], allow_custom=True)
                ...
                try:
                    if response.status_code == 200:
                        ...
                        for item in response_json:
                            ...
                            bundle= self.stix_object_generator(item) #calling the stix_object_generator method to create stix objects
            ...
            try:
                for item in response_json:
                    ...
                    if last_run is None:
                        ...
                        time_diff = int(datetime.timestamp(created))-(int(last_run)-30) # 30 seconds is added to avoid missing any data that might have caused due to code execution time
                ...
                self.helper.log_info(f"Sending {stix_objects} STIX objects to OpenCTI...")
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                self.get_historic = os.environ.get("CONNECTOR_PULL_HISTORY", None).lower()
                self.get_historic_year = os.environ.get("CONNECTOR_HISTORY_START_YEAR", None).lower()
