...
| HTTP Authorization Header              | misp_feed.http_authorization_header               | `MISP_FEED_HTTP_AUTHORIZATION_HEADER`                          |               | No        | Optional static value of the HTTP `Authorization` header (e.g. `Basic <base64(user:password)>` or `Bearer <token>`). |
