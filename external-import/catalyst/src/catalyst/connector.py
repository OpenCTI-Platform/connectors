...
class CatalystConnector:
    ...
    def _collect_intelligence(self) -> list:
        ...
        self.helper.connector_logger.debug(f"Received entities: {entities}")

        for entity in entities:
            if hasattr(entity, "type"):
                self.helper.connector_logger.debug(
                    f"Adding STIX object of type: {entity.type}"
                )
                stix_objects.append(entity)

            elif isinstance(entity, dict):
                self.helper.connector_logger.error(
                    f"Unexpected entity type: {type(entity)}. Expected a STIX object or dictionary."
                )
                raise ValueError(
                    f"Unexpected entity type: {type(entity)}. Expected a STIX object or dictionary."
                )
            else:
                # Entity is neither a recognized STIX object nor a dictionary
