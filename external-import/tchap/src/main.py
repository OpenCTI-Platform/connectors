...
class TchapConnector:
    def __init__(self):
        """Initialization of the connector"""
        # super().__init__()
        self.update_existing_data = True
        self.helper = OpenCTIConnectorHelper({})
        self.tchap_device_name = os.environ.get("TCHAP_DEVICE_NAME", "octi_bot")
        self.tchap_server = os.environ.get(
            "TCHAP_SERVER", "https://matrix.agent.tchap.gouv.fr"
        )
        self.tchap_password = os.environ.get("TCHAP_PASSWORD")
        self.tchap_user_id = os.environ.get("TCHAP_USER_ID")
        self.tchap_debug = (
            False if (os.environ.get("TCHAP_DEBUG", "false") == "false") else True
        )
        self.tchap_tlp = get_tlp(os.environ.get("TCHAP_TLP", "AMBER"))
        self.helper.log_debug(
            f"Init TchapConnector for user {self.tchap_user_id} /password {self.tchap_password} to {self.tchap_server}"
        )
        ...
        try:
            ...
            if len(channel_list) == 0:
                x_channel = Channel(
                    ...
                    object_marking_refs=self.tchap_tlp,
                    channel_types=["Tchap"],
                )
    ...
    async def run_client(self):
        self.helper.log_debug("Creating Tchap client...")
        self.client = AsyncClient(
            homeserver=self.tchap_server,
            user=self.tchap_user_id,
            config=self.client_config,
        )
        self.helper.log_debug("Login...")
        await self.client.login(
            password=self.tchap_password, device_name=self.tchap_device_name, token=None
        )
