...
class StubReport(Report):
    ...
    @property
    def related_tags(self) -> typing.Generator[Tag, None, None]:
        yield from [stub_valid_tag()]

    @property
    def related_indicators(
        self,
...
@pytest.mark.parametrize(
    "instance",
    [
        pytest.param(stub_valid_tag(), id="Tag"),
        pytest.param(stub_valid_indicator(), id="Indicator"),
        pytest.param(stub_valid_report(), id="Report"),
    ],
)
def tests_implemented_interface_attributes_are_read_only(instance):
...
def test_tag_should_raise_data_retrieval_error_with_incorrect_attribute():
    ...
    with pytest.raises(ReportRetrievalError):
        _ = Tag(value=(44.5, 5.2), type="Geolocation")  # type: ignore[arg-type]
...
def test_indicator_should_cast_datetime():
    ...
    # Given a StubIndicator definition respecting interface types
    # When instantiating the StubIndicator
    indicator = stub_valid_indicator()
    # Then the indicator should have time aware datetime attributes
    assert (  # noqa: S101 we indeed call assert in test
        indicator.first_seen
        == datetime.datetime(1970, 1, 1, 0, 0, 0, tzinfo=datetime.timezone.utc)
        and indicator.last_seen
        == datetime.datetime(1970, 1, 2, 0, 0, 0, tzinfo=datetime.timezone.utc)
    )
