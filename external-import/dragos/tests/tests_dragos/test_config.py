...
from dragos.config import Config, ConfigRetrievalError
...
def fixture_mocked_environ(mocker: MockerFixture, config_dict: dict) -> None:
    environ = deepcopy(os.environ)
    for key, value in config_dict.items():
        for sub_key, sub_value in value.items():
            if sub_value:
                environ[f"{key.upper()}_{sub_key.upper()}"] = str(sub_value)
    environ["CONNECTOR_SCOPE"] = '["ChangeMe"]'
