...
| Threat Actor to Intrusion Set           | recorded_future.ta_to_intrusion_set            | `RECORDED_FUTURE_TA_TO_INTRUSION_SET`                     |                     | No        | List of submodules where Threat Actors are mapped to Intrusion Sets. Available values: `risk_list`, `analyst_notes`. `true` still supported for backward compatibility act as 'all'|
