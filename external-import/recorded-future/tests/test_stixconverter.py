...
class TestStixObjects:
    ...
    def test_note_creation(self, opencti_helper, tas):
        note = StixNote(opencti_helper, tas)
    ...
    def test_ta_note_creation(self, opencti_helper, tas):
        note = StixNote(opencti_helper, tas)
    ...
    def test_ta_org_note_creation(self, opencti_helper, tas):
        note = StixNote(opencti_helper, tas)
    ...
    def test_ta_org_note_creation_flag(self, opencti_helper, tas):
        note = StixNote(opencti_helper, tas, ta_to_intrusion_set=True)
        with open(os.path.join(CWD, 'inputs/taorgnote.json'), 'r') as file:
            raw = json.load(file)
        note.from_json(raw)
        assert len(note.objects) > 2
    ...
    def test_person_note_creation(self, opencti_helper, tas):
        note = StixNote(opencti_helper, tas)
    ...
    def test_person_note_creation_flag(self, opencti_helper, tas):
        note = StixNote(opencti_helper, tas, person_to_ta=True)
        with open(os.path.join(CWD, 'inputs/personnote.json'), 'r') as file:
            raw = json.load(file)
        note.from_json(raw)
        assert len(note.objects) > 2
    ...
    def test_vuln_note_creation(self, opencti_helper, tas):
        note = StixNote(opencti_helper, tas)
    ...
    def test_yara_note_creation(self, opencti_helper, tas):
        note = StixNote(opencti_helper, tas)
    ...
    def test_report_type_conversion(self, opencti_helper, tas):
        topics = [
            {
                "id": "ZjnoP2",
