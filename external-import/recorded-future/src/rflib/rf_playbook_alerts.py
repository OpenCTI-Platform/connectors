...
class RecordedFuturePlaybookAlertConnector(threading.Thread):
    def __init__(self, helper):
        ...
        self.connector_interval = pycti.get_config_variable(
            "RECORDED_FUTURE_INTERVAL", ["connector", "interval"], config, required=True
        )
        self.threshold_domain_abuse = pycti.get_config_variable(
            "RECORDED_FUTURE_PLAYBOOK_ALERT_SEVERITY_THRESHOLD_DOMAIN_ABUSE",
            ["rf", "playbook_alert_severity_threshold_domain_abuse"],
            config,
            required=False,
            default="Informational",
        )
        self.threshold_identity_novel_exposure = pycti.get_config_variable(
            "RECORDED_FUTURE_PLAYBOOK_ALERT_SEVERITY_THRESHOLD_IDENTITY_NOVEL_EXPOSURES",
            ["rf", "playbook_alert_severity_threshold_identity_novel_exposures"],
            config,
            required=False,
            default="Informational",
        )
        self.threshold_code_repo_leakage = pycti.get_config_variable(
            "RECORDED_FUTURE_PLAYBOOK_ALERT_SEVERITY_THRESHOLD_CODE_REPO_LEAKAGE",
            ["rf", "playbook_alert_severity_threshold_code_repo_leakage"],
            config,
            required=False,
            default="Informational",
        )
        self.debug_var = pycti.get_config_variable(
            "RECORDED_FUTURE_DEBUG",
            ["rf", "recorded_future_debug"],
            config,
            required=False,
            default=False,
        )
    ...
    def run(self):
        while True:
            ...
            self.work_id = self.helper.api.work.initiate_work(
                ...
                "Recorded Future Playbook Alert run @ "
            ...
            for playbook_type in playbook_types:
                ...
                if (
                    ...
                    self.api_recorded_future.get_playbook_id(
                        ...
                        trigger_from=current_state_datetime.strftime(
                            "%Y-%m-%dT%H:%M:%S"
                        ),
                ...
                for plb_alert in self.api_recorded_future.playbook_alerts_summaries:
                    try:
                        ...
                        playbook_alert = (
                            self.api_recorded_future.get_complete_playbook_alert(
                                plb_alert
                            )
                        )
                        if (
                            playbook_alert["data"]["panel_status"]["status"]
                            != "Dismissed"
                        ):
            ...
            next_time = timestamp + datetime.timedelta(
                minutes=int(self.connector_interval)
            )
            waiting_time = (
