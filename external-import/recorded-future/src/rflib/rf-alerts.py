...
class RecordedFutureAlertConnector(threading.Thread):
    def __init__(self, helper):
        ...
        self.helper.log_info('Starting Recorded Future Alert connector module initialization')

        # Additional configuration.
        self.opencti_url = pycti.get_config_variable('OPENCTI_URL', ['opencti', 'url'], config, required=True)
        self.opencti_token = pycti.get_config_variable('OPENCTI_TOKEN', ['opencti', 'token'], config, required=True)
        
        self.recordedfuture_x_rf_token = pycti.get_config_variable('RECORDED_FUTURE_XRFTOKEN', ['rf', 'token'], config, required=True)
        self.connector_interval = pycti.get_config_variable('CONNECTOR_INTERVAL', ['connector', 'download_path'], config, required=True)
        
        self.opencti_incident_vocabulary_name = pycti.get_config_variable('OPENCTI_INCIDENT_VOCABULARY_NAME', ['alert', 'incident_vocabulary_name'], config, required=True)
        self.opencti_channels_vocabulary_name = pycti.get_config_variable('OPENCTI_CHANNELS_VOCABULARY_NAME', ['alert', 'channels_vocabulary_name'], config, required=True)
        self.opencti_default_marking = pycti.get_config_variable('OPENCTI_DEFAULT_MARKING', ['alert', 'default_marking'], config, required=True)
        self.opencti_connector_author_name = pycti.get_config_variable('OPENCTI_CONNECTOR_AUTHOR_NAME', ['alert', 'connector_author_name'], config, required=True)
        self.opencti_incident_static_description = pycti.get_config_variable('OPENCTI_INCIDENT_STATIC_DESCRIPTION', ['alert', 'incident_static_description'], config, required=True)
        self.connector_initial_pull_off = pycti.get_config_variable('CONNECTOR_INITIAL_PULL_OFF', ['alert', 'initial_pull_off'], config, required=False, default=1)        
        self.connector_download_path = pycti.get_config_variable('CONNECTOR_DOWNLOAD_PATH', ['alert', 'download_path'], config, required=True)
        self.recordedfuture_logo_abuse_rule_id = pycti.get_config_variable('RECORDED_FUTURE_LOGO_ABUSE_RULE_ID', ['alert', 'logo_abuse_rule_id'], config, required=True)
        ...
        for vocab in existing_vocabulary:
            self.vocabulary_list.append(Vocabulary(vocabulary_id=vocab['id'], vocabulary_name=vocab['name']))
        
        self.api_recorded_future = RecordedFutureApiClient(x_rf_token=self.recordedfuture_x_rf_token, connector_download_path=self.connector_download_path)
        ...
        for newrule in self.api_recorded_future.unfound_rf_rules_in_vocabularies :
            self.opencti_api_client.vocabulary.create(
                **{
                    'name' : str(self.api_recorded_future.nomenclature + newrule.rule_name),
        ...
        self.stix_object_default_marking = self.opencti_api_client.marking_definition.read(
            **{
                'filters' : {
                    'mode': 'and', 
                    'filterGroups' : [], 
        ...
        self.stix_author = (self.opencti_api_client.identity.list(
            **{
                'filters': {
                    ...
                    'filters':[{
                        'key' : 'name',
                        'values': self.opencti_connector_author_name
                    }]
                }
            }
        ))
    ...
    def run_from_date_to_date(self, from_date, to_date):
        assert isinstance(from_date, str), 'Date must be a string with the following format : yyyy-MM-dd'
        assert isinstance(to_date, str), 'Date must be a string with the following format : yyyy-MM-dd'
    ...
    def run_for_time_period(self, trigger):
        assert isinstance(trigger, str), 'Date must be a string with any format between : yyyy to yyyy-MM-dd\'T\'HH:mm:ss\'Z\''
        ...
        for alert in self.api_recorded_future.alerts :
            ...
            stix_external_references = self.opencti_api_client.external_reference.create(
                **{
            ...
            for hit in alert.alert_hits :
                ...
                if fragment is None :   
                    ...
                    fragment = hit['fragment'].replace(str(fragment[0]), str(' ![](' +fragment[0]+ ') '))
                for entity in hit['entities'] :
                    if entity['type'] == 'Image' :
                        image_presence, image_path, image_name = self.api_recorded_future.get_image_and_save_temp_file(entity['name'])
                        if image_presence :
                            stix_images.append(
                                ImageToShowInNote(
                                    hit['id'],
                    ...
                    entities_string += str('entity ' + str(entity_count) + ' - ' + entity['type'] + ' : ' + entity['name'] + '  \n  ')
                    if entity['type'] == 'URL':
                        observable_type = 'url'
                ...
                for channel in self.stix_channels :
                    if hit['document']['source']['name'] is not None :
                        if str(channel['name']).lower() in str(hit['document']['source']['name']).lower() :
                            value = search(
                ...
                if alert.alert_rf_rule.rule_id == self.recordedfuture_logo_abuse_rule_id :
                    if hit['document']['url'] is not None :
                        ...
                        if parsed_url_from_title is not None :
                            ...
                            parsed_url_from_parsed_url = search(
                                '^(https?:\/\/)?(www\.)?([^\/]+)',
                                parsed_url_from_title
                            )
                            if parsed_url_from_parsed_url is not None :
                                parsed_url_from_parsed_url = parsed_url_from_parsed_url[0]
                                composed_url_from_title = 'https://urlscan.io/domain/' + str(parsed_url_from_parsed_url)
                                stix_external_ref_to_add.append(composed_url_from_title)
                                composed_image_from_url = 'https://urlscan.io/liveshot/?url=https://' + str(parsed_url_from_title)
                                fragment = fragment + ' LIVESHOT : ![Liveshot not available](' + str(composed_image_from_url) + ') '
                ...
                if observable_type == 'url' and twitter_account_page is not None :
                    twitter_account_name = (search(
                        '@([A-Za-z0-9_]+)',
                        twitter_account_page[0]
                    ))[0]
                    stix_observable = self.opencti_api_client.stix_cyber_observable.create(
                        **{
                            'observableData' : {
                ...
                stix_note = self.opencti_api_client.note.create(
                    **{
                        'abstract' : str( '[' + alert.alert_id + ']' + '[' + str(fragment_count) + '] ' + alert.alert_title),
                        'content' : fragment,
                        'objectMarking' : self.stix_object_default_marking['id'],
                ...
                stix_notes.append(NoteWithCraftedImage(
                    hit['id'],
                    stix_note,
                    str( '[' + alert.alert_id + ']' + '[' + str(fragment_count) + '] ' + alert.alert_title)
                ))
                ...
                if len(hit['document']['authors']) > 0 :
                    for author in hit['document']['authors'] :
                        if author['name'] not in self.stix_anonymous_user_account :
                            ...
                            striped_account_login = str(author['name']).replace('Create by: ', '')
                            stix_user_account = self.opencti_api_client.stix_cyber_observable.create(
                                **{
                                    'observableData' : {
                                        'account_type' : hit['document']['source']['name'], 
                                        'type': 'user-account',
            ...
            for stix_external_reference_url in stix_external_ref_to_add :
                stix_external_reference = self.opencti_api_client.external_reference.create(
                    **{
                        'created' : alert.alert_date,
            ...
            stix_incident = self.opencti_api_client.incident.create(
                **{
                    ...
                    'incident_type' : str(self.api_recorded_future.nomenclature +  alert.alert_rf_rule.rule_name),
                    'created' : alert.alert_date,
            ...
            for stix_image in stix_images :
                stix_image_result = self.opencti_api_client.stix_domain_object.add_file(
                    **{
                        'id':stix_incident['id'],
                        'file_name' : stix_image.image_path
                    }
                )
            for stix_note in stix_notes :
                ...
                for stix_image in stix_images :
                    if stix_image.image_alert_hit_id == stix_note.note_alert_hit_id :
                        new_fragment = '![Image Not Found](' + self.opencti_url + '/storage/view/import%2FIncident%2F' + stix_incident['id'] + '%2F' + stix_image.image_name + ')'
