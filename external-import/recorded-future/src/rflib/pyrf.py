...
class Alert:
    def __init__(self, alert_id, alert_url, alert_date, alert_title, alert_ai_insight, alert_rf_rule, alert_hits):
...
class RecordedFutureApiClient:
    def __init__(self, x_rf_token, connector_download_path, helper, base_url='https://api.recordedfuture.com/', nomenclature = ''):
    ...
    def print_all(self):
        for each_playbook_alert in self.playbook_alerts:
            self.helper.log_info(each_playbook_alert.playbook_alert_id + ' - ' + each_playbook_alert.playbook_alert_title + ' - ' + each_playbook_alert.playbook_alert_priority)
    
    def get_data_from_playbook_id(self, playbook_alert):
        assert self.x_rf_token is not None, 'You must provide an XRF-Token.'
        assert playbook_alert.playbook_alert_id is not None, 'You must provide an XRF-Token'
        
        response = requests.post( 
            str(self.base_url + 'playbook-alert/' +  playbook_alert.playbook_alert_category + '/' + (playbook_alert.playbook_alert_id).replace(':', '%3A')),
            headers = {
        ...
        data = response.json()
        assert response.status_code == 200, 'Unexpected status code from ApiRecordedFuture: ' + str(response.status_code)
        assert response.headers.get('Content-Type') == 'application/json', 'Unexpected Content-Type from ApiRecordedFuture: ' + response.headers.get('Content-Type')
        assert data['status']['status_code'] == 'Ok', 'Unexpected response status from ApiRecordedFuture : ' + data['status']['status_code']
        assert data['status']['status_message'] == 'Playbook alert single lookup successful.', 'Unexpected response message from ApiRecordedFuture : ' + data['status']['status_message']
        return data['data']['panel_evidence_summary'], data['data']['panel_evidence_dns'], data['data']['panel_evidence_whois'], data['data']['panel_log']
    ...
    def get_image_and_save_temp_file(self, image_id):
        regexed_image_id = search('[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}', image_id)
        assert regexed_image_id is not None, 'You must provide a valide uuid v4 image id'
        try :
            ...
            assert response.status_code == 200, 'Unexpected status code from ApiRecordedFuture: ' + str(response.status_code)
            assert response.headers.get('Content-Type') == 'image/png;charset=utf-8', 'Unexpected Content-Type from ApiRecordedFuture: ' + response.headers.get('Content-Type')
            ...
            self.helper.log_info('Exception occured when trying to reach RecordedFuture\'s API : ' + str(e))
        ...
        for priorited_rule in self.priorited_rules :
            try :
                ...
                while int(from_api) < int(self.alert_count) :
                    ...
                    assert response.status_code == 200, 'Unexpected status code from ApiRecordedFuture: ' + str(response.status_code)
                    assert response.headers.get('Content-Type') == 'application/json;charset=utf-8', 'Unexpected Content-Type from ApiRecordedFuture: ' + response.headers.get('Content-Type')
                    data = response.json()
                    ...
                    assert 'counts' in data, 'Response does not contain mandatory counts field'
                    if from_api == 0 :
                        self.helper.log_info('RF Rule : ' +  str(data['counts']['total'])  + ' - ' +  priorited_rule.rule_name)
                    if data['counts']['total'] == 0 :
                        self.alert_count = 0
                        from_api = 1
                    else :
                ...
                raise RuntimeError('Exception occured when trying to reach RecordedFuture\'s API : ' + str(e))
            except :
                raise RuntimeError('Unexpected error')

    def compare_rf_rules_and_vocabularies(self, found_vocabulary):
        for rule in self.priorited_rules :
            if not any(str(self.nomenclature + rule.rule_name) == vocab.vocabulary_name for vocab in found_vocabulary) :
